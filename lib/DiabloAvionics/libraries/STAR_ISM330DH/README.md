STAR ISM330DH (ISM330DHCX) — Arduino library
============================================

**This is an improved STAR fork of the [SparkFun 6DoF ISM330DHCX Arduino Library](https://github.com/sparkfun/SparkFun_6DoF_ISM330DHCX_Arduino_Library).** It keeps the same STMicro `ism330dhcx` C driver and the SparkFun-derived I²C/SPI glue, and adds UC Berkeley STAR extensions (for example wake-up routing on INT1 and `serviceWakeUpInterrupt()` so application code does not parse raw interrupt status bytes). Use `#include <STAR_ISM330DH.h>` and classes **`ISM330DH`** (I²C) / **`ISM330DH_SPI`** (SPI). SparkFun hardware and their hookup guide still apply.

---

The remainder of this README is largely unchanged from SparkFun’s original documentation.

SparkFun 6DoF ISM330DHCX Arduino Library (upstream reference)
===============================================================
