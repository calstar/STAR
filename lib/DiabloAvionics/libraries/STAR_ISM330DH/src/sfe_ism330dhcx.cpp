...
// Register addresses used by the extensions below (not exposed by the ST shim).
static constexpr uint8_t ISM_REG_ALL_INT_SRC  = 0x1A;
static constexpr uint8_t ISM_REG_FIFO_STATUS1 = 0x3A;
static constexpr uint8_t ISM_REG_FIFO_STATUS2 = 0x3B;
static constexpr uint8_t ISM_REG_FIFO_DATA_OUT_TAG = 0x78; // TAG byte (0x78); data follows at 0x79-0x7E
static constexpr uint8_t ISM_REG_INT2_CTRL         = 0x0E; // INT2_FIFO_TH = bit 3 (0x08)

//////////////////////////////////////////////////////////////////////////////
// getAllIntSrc()
//
// Reads the ALL_INT_SRC register (0x1A). Motion/event flags only — not data-ready.
// Bit 0 = FF_IA (free-fall), bit 1 = WU_IA (wake-up), bit 2 = SINGLE_TAP,
// bit 3 = DOUBLE_TAP, bit 4 = D6D_IA (orientation change),
// bit 5 = SLEEP_CHANGE_IA, bit 6 = (reserved), bit 7 = TIMESTAMP_ENDCOUNT.
//
uint8_t QwDevISM330DHCX::getAllIntSrc()
{
	uint8_t val = 0;
	readRegisterRegion(ISM_REG_ALL_INT_SRC, &val, 1);
	return val;
}

bool QwDevISM330DHCX::serviceWakeUpInterrupt(sfe_ism330_wake_event_t *out)
{
	if (!out)
		return false;
	out->axisX = false;
	...
	// ALL_INT_SRC: WU_IA is bit 1 (see ST ism330dhcx_all_int_src_t, little-endian bitfield order).
	const uint8_t all = getAllIntSrc();
	if ((all & 0x02U) == 0)
		return false;

	// WAKE_UP_SRC: z_wu bit0, y_wu bit1, x_wu bit2 (little-endian layout in register byte).
	uint8_t wus = 0;
	readRegisterRegion(ISM330DHCX_WAKE_UP_SRC, &wus, 1);
	out->axisZ = (wus & 0x01U) != 0;
	out->axisY = (wus & 0x02U) != 0;
	out->axisX = (wus & 0x04U) != 0;
	return true;
}

bool QwDevISM330DHCX::configureWakeUpOnInt1(uint8_t wkThreshold, uint8_t wakeDur,
                                            bool routeAccelGyroDrdyToInt1)
{
	if (wkThreshold == 0)
		wkThreshold = 1;
	if (wkThreshold > 63)
	...
	if (ism330dhcx_wkup_ths_weight_set(&sfe_dev, ISM330DHCX_LSb_FS_DIV_64) != 0)
		return false;
	if (ism330dhcx_wkup_threshold_set(&sfe_dev, wkThreshold) != 0)
		return false;
	if (ism330dhcx_wkup_dur_set(&sfe_dev, wakeDur) != 0)
		return false;

	ism330dhcx_pin_int1_route_t route{};
	if (ism330dhcx_pin_int1_route_get(&sfe_dev, &route) != 0)
		return false;

	route.md1_cfg.int1_wu = PROPERTY_ENABLE;
	if (!routeAccelGyroDrdyToInt1) {
		route.int1_ctrl.int1_drdy_xl = PROPERTY_DISABLE;
		route.int1_ctrl.int1_drdy_g = PROPERTY_DISABLE;
	}

	return ism330dhcx_pin_int1_route_set(&sfe_dev, &route) == 0;
}

//////////////////////////////////////////////////////////////////////////////
// configureSensor()
//
// One-call init that mirrors the commonInit() pattern from the bring-up script:
//   - Block data update on
//   - Device config on (enables auto-increment)
//   - Accel: 104 Hz, 2 g
//   - Gyro:  104 Hz, 250 dps
//   - Routes DRDY_XL and DRDY_G to INT1
//
bool QwDevISM330DHCX::configureSensor()
{
	if (!setBlockDataUpdate(true))    return false;
	if (!setDeviceConfig(true))       return false;
	if (!setAccelDataRate(ISM_XL_ODR_104Hz))  return false;
	if (!setAccelFullScale(ISM_2g))           return false;
	if (!setGyroDataRate(ISM_GY_ODR_104Hz))   return false;
	if (!setGyroFullScale(ISM_250dps))        return false;
	if (!setAccelStatustoInt1(true))  return false;
	if (!setGyroStatustoInt1(true))   return false;
	return true;
}
...
// getFifoLevel()
//
// Returns the number of unread samples currently in the FIFO by reading
// FIFO_STATUS1 (lower 8 bits) and the two LSBs of FIFO_STATUS2.
//
uint16_t QwDevISM330DHCX::getFifoLevel()
{
	uint8_t buf[2] = {0};
	readRegisterRegion(ISM_REG_FIFO_STATUS1, buf, 2);
...
// readFifoSample()
//
// Reads one 7-byte FIFO word from FIFO_DATA_OUT_TAG (0x78).
// Each word = 1 TAG byte + 6 data bytes (X/Y/Z as 16-bit little-endian pairs).
// The TAG byte identifies the sensor:
//   0x01 = gyro, 0x02 = accel, 0x04 = temperature (per datasheet Table 105).
//
// The TAG is returned via *tag_out. Callers should check TAG before interpreting
// the data — gyro and accel words have the same layout but different scale factors.
//
// Returns false if the bus read fails.
//
bool QwDevISM330DHCX::readFifoSample(sfe_ism_raw_data_t* gyro, sfe_ism_raw_data_t* accel, uint8_t* tag_out)
{
	// Read TAG + 6 data bytes as one 7-byte burst starting at 0x78.
	uint8_t buf[7] = {0};
	if (readRegisterRegion(ISM_REG_FIFO_DATA_OUT_TAG, buf, 7) != 0)
		return false;

	if (tag_out)
		*tag_out = buf[0];

	// buf[1..6] = X_L, X_H, Y_L, Y_H, Z_L, Z_H
	sfe_ism_raw_data_t* dst = (buf[0] == 0x01) ? gyro : accel;
	dst->xData = (int16_t)(buf[1] | ((uint16_t)buf[2] << 8));
	dst->yData = (int16_t)(buf[3] | ((uint16_t)buf[4] << 8));
	dst->zData = (int16_t)(buf[5] | ((uint16_t)buf[6] << 8));
...
// setFifoWtmToInt2()
//
// Routes (or removes) the FIFO watermark interrupt to the INT2 pin via
// INT2_FIFO_TH (bit 3) of INT2_CTRL (0x0E). Call after setFifoWatermark()
// and setFifoMode().
//
bool QwDevISM330DHCX::setFifoWtmToInt2(bool enable)
{
	uint8_t val = 0;
	if (readRegisterRegion(ISM_REG_INT2_CTRL, &val, 1) != 0)
		return false;
	if (enable)
		val |= 0x08;  // INT2_FIFO_TH = bit 3
	else
		val &= ~0x08;
	return (writeRegisterRegion(ISM_REG_INT2_CTRL, &val, 1) == 0);
}

//////////////////////////////////////////////////////////////////////////////
// calibrateBiases()
//
// Collects `samples` readings from getAccel() and getGyro() while the sensor
// is stationary, then stores the average as internal bias values (mg / mdps).
// Call getAccelCal() / getGyroCal() to retrieve bias-corrected readings.
//
// Note: the device must already be configured and the sensor held still.
//
bool QwDevISM330DHCX::calibrateBiases(uint16_t samples)
{
	...
	for (uint16_t i = 0; i < samples; ++i) {
		if (!getAccel(&a) || !getGyro(&g))
			return false;
		ax += a.xData; ay += a.yData; az += a.zData;
		gx += g.xData; gy += g.yData; gz += g.zData;
		delay(2);  // ~2 ms gap; adequate for ODR ≥ 104 Hz
	}

	_accelBias.xData = (float)(ax / samples);
	_accelBias.yData = (float)(ay / samples);
	_accelBias.zData = (float)(az / samples);
	_gyroBias.xData  = (float)(gx / samples);
	_gyroBias.yData  = (float)(gy / samples);
	_gyroBias.zData  = (float)(gz / samples);
	_biasValid = true;
...
// getAccelCal()
//
// Reads accelerometer data and subtracts the stored bias. Returns false if
// calibrateBiases() has not been called or if the bus read fails.
//
bool QwDevISM330DHCX::getAccelCal(sfe_ism_data_t* accelData)
{
	if (!_biasValid)
		return false;
	if (!getAccel(accelData))
		return false;
	accelData->xData -= _accelBias.xData;
	accelData->yData -= _accelBias.yData;
	accelData->zData -= _accelBias.zData;
	return true;
}
...
// getGyroCal()
//
// Reads gyroscope data and subtracts the stored bias. Returns false if
// calibrateBiases() has not been called or if the bus read fails.
//
bool QwDevISM330DHCX::getGyroCal(sfe_ism_data_t* gyroData)
{
	if (!_biasValid)
		return false;
	if (!getGyro(gyroData))
		return false;
	gyroData->xData -= _gyroBias.xData;
	gyroData->yData -= _gyroBias.yData;
	gyroData->zData -= _gyroBias.zData;
	return true;
}
