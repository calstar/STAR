"""Per-user design documents: working copy + microversions + releases.

The versioned-document API (/api/documents) mirrors the pid-designer
...
A design here is the whole unified config (CAD slice + recovery slice), but
this store is payload-agnostic, so the fixtures below are opaque dicts.
"""
...
#: The ``<app>`` path segment this backend writes under (backend/userdata.py).
APP_SEGMENT = "openrocket"


def test_s3_backend_is_dormant_without_bucket():
    """No OPENROCKET_S3_BUCKET in the environment -> local files, never AWS."""
...
def test_create_lists_and_isolates_until_shared(client):
    ...
    a_list = client.get("/api/documents", headers=A).json()
    b_list = client.get("/api/documents", headers=B).json()
...
def test_rename(client):
    ...
    r = client.patch(f"/api/documents/{doc_id}", headers=A, json={"name": "Renamed"})
    assert r.status_code == 200 and r.json()["name"] == "Renamed"
    assert [c["name"] for c in client.get("/api/documents", headers=A).json()] == ["Renamed"]
...
def test_delete_is_gone(client):
    ...
    grant, not a config."""
    doc_id = _create(client, A)
    assert client.delete(f"/api/documents/{doc_id}", headers=A).status_code == 405
    deletes = {r.path for r in documents.router.routes
               if "DELETE" in getattr(r, "methods", set())}
    assert deletes == {
        "/api/documents/{doc_id}/share/me",
...
def test_autosave_load_and_history(client):
    ...
    assert client.post(f"/api/documents/{doc_id}/autosave",
                       headers=A, json={"config": cfg}).json()["micro"] is True

    # /load returns the freshest working copy.
    assert client.get(f"/api/documents/{doc_id}/load", headers=A).json()["config"] == cfg

    # The autosave recorded a microversion; fetch it back verbatim.
    history = client.get(f"/api/documents/{doc_id}/history", headers=A).json()
    assert len(history) >= 1
    vid = history[0]["versionId"]
    snap = client.get(f"/api/documents/{doc_id}/version/{vid}", headers=A).json()
...
def test_history_needs_a_share(client):
    ...
    client.post(f"/api/documents/{a_id}/autosave", headers=A,
                json={"config": {"secret": 1}})
    assert client.get(f"/api/documents/{a_id}/history", headers=B).status_code == 404
    assert client.get(f"/api/documents/{a_id}/history",
...
def test_history_readable_once_shared(client):
    ...
    _share(client, a_id, [B["X-Auth-Email"]])
    history = client.get(f"/api/documents/{a_id}/history",
...
def test_release_is_immutable_and_listed(client):
    ...
    r = client.post(f"/api/documents/{doc_id}/release",
    ...
    dup = client.post(f"/api/documents/{doc_id}/release",
                      headers=A, json={"label": "0.1", "config": {"v": 2}})
    assert dup.status_code == 409

    assert "0.1" in [r["label"] for r in
                     client.get(f"/api/documents/{doc_id}/releases", headers=A).json()]
    got = client.get(f"/api/documents/{doc_id}/release/0.1", headers=A).json()
...
def test_restore_returns_older_state(client):
    ...
    client.post(f"/api/documents/{doc_id}/autosave", headers=A, json={"config": good})
    client.post(f"/api/documents/{doc_id}/autosave", headers=A,
    ...
    oldest = client.get(
        f"/api/documents/{doc_id}/version/{history[-1]['versionId']}", headers=A
    ).json()
...
def test_share_grants_edit_and_writes_into_the_owners_folder(client, tmp_path):
    ...
    _share(client, a_id, [B["X-Auth-Email"]])
    # Creating a config checks it out to its creator, so Alice hands it over
    ...
    assert (tmp_path / "alice@berkeley.edu" / APP_SEGMENT / a_id / "current.json").is_file()
    assert not (tmp_path / "bob@berkeley.edu" / APP_SEGMENT / a_id).exists()
...
#: Every config-scoped route, as (method, path-suffix, json body). A route added
...
#: Not config-scoped: no doc id, or exists precisely to reach configs you cannot edit.
