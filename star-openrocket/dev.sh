#!/usr/bin/env bash
# star-openrocket/dev.sh — run the backend (:8002) and frontend (:5175) together.
#
# Usage: ./dev.sh    (Ctrl-C stops both)
#
# First-time install is `bash star-openrocket/setup.sh` from the repo root, or
# `./setup.sh --star-openrocket`. This script installs missing dependencies as a
...
if [ -z "${ONSHAPE_ACCESS_KEY:-}" ] || [ -z "${ONSHAPE_SECRET_KEY:-}" ]; then
  ...
  echo "Put a key pair from https://dev-portal.onshape.com in star-openrocket/.env:"
