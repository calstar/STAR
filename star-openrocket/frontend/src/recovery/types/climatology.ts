...
/**
 * One calendar month of wind, as parallel arrays down the shared height grid.
 *
 * Stored as u (east) / v (north) components -- the RocketPy convention and what
 * the drift model consumes -- plus speed quantiles for the day-to-day spread.
 * `spd_p95` is the deterministic "worst-case wind" the Drift tab offers; `dir`
 * is the bearing the mean wind blows FROM.
 */
export interface WindMonth {
  month: number
  n: number
  u: number[]
  v: number[]
...
export interface Climatology {
  ...
  upper: {
    ...
    /** Monthly wind per dataset tag. Optional: generated by wind_profile.py,
     *  which runs separately from the temperature profile, so a tag can have a
     *  temperature profile without a wind one. */
    wind?: Record<string, WindMonth[]>
  }
