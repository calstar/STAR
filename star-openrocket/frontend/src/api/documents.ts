 ...
 * `@stardesign-ui` -- shared with EngineDesign, pid-designer and the recovery
 * calculator, mirroring the shared server router. What is app-specific is only
 * what is below: the route prefix and the payload shape.
 *
 * A design here is the whole unified config -- the CAD/stability/ascent slice
 * and the recovery slice together. The client-side localStorage autosave
 * (`lib/persist.ts`) stays as the instant working cache; this store is the
 * durable, versioned timeline.
 */
...
import { toStoredOrkConfig } from '../lib/persist'
...
const api = createDesignApi<UiConfig>({
  ...
  usersPath: '/api/users',
  codec: {
    // `toStoredOrkConfig` is what keeps view state out of the saved design --
    // collapsing a device card must not count as an edit. See lib/persist.ts.
    toBody: (config) => ({ config: toStoredOrkConfig(config) }),
