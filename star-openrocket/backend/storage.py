...
* **S3Backend** when ``OPENROCKET_S3_BUCKET`` is set. Microversions ride on S3
...
def _make_backend():
    bucket = os.environ.get("OPENROCKET_S3_BUCKET")
    if bucket:
        return S3Backend(bucket, os.environ.get("OPENROCKET_S3_PREFIX", "openrocket"))
    return LocalBackend()
