...
* **S3Backend** when ``RECOVERY_S3_BUCKET`` is set. Microversions ride on S3
...
def _make_backend():
    bucket = os.environ.get("RECOVERY_S3_BUCKET")
    if bucket:
        return S3Backend(bucket, os.environ.get("RECOVERY_S3_PREFIX", "recovery"))
    return LocalBackend()
