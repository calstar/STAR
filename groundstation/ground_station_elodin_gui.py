...
class ElodinClient:
        ...
        try:
            ...
            while len(payload_data) < (response_len - 12):
                chunk = self.socket.recv(
                    min(4096, response_len - 12 - len(payload_data))
                )
                if not chunk:
                    break
                payload_data += chunk
    ...
    def _create_header(
        self, packet_type: PacketType, packet_id: List[int], payload_length: int
    ) -> bytes:
        """Create Elodin packet header"""
        total_length = 12 + payload_length  # Header + payload

        header = struct.pack("<I", total_length)  # len (4 bytes)
        header += struct.pack("<B", packet_type.value)  # ty (1 byte)
        header += struct.pack("<BB", packet_id[0], packet_id[1])  # packet_id (2 bytes)
        header += struct.pack("<I", 0)  # reserved/padding (4 bytes)
        header += struct.pack("<B", 0)  # request_id (1 byte)
...
class ElodinGroundStationGUI(QtWidgets.QMainWindow):
    ...
    def _create_command_panel(self) -> QtWidgets.QWidget:
        ...
        for i, name in enumerate(["LOX Main", "Fuel Main", "LOX Purge", "Fuel Purge"]):
            ...
            send_btn.clicked.connect(
                lambda checked, vid=i, s=slider: self._send_valve_command(
                    vid, s.value()
                )
            )
    ...
    def _handle_telemetry_update(self, packet_type: str, data: List[Dict]):
        ...
        for record in data:
            ...
            if packet_type == "PT_DATA":
                ...
                self.engine_state_display.setText(
                    f"Engine: {EngineState(int(state)).name}"
                )
    ...
    def _add_plot_data(self, plot_type: str, key: str, timestamp: float, value: float):
        ...
        if key not in self.plot_data[plot_type]:
            self.plot_data[plot_type][key] = {
                "time": deque(maxlen=self.max_buffer_size),
    ...
    def _send_abort(self):
        ...
        self.elodin.send_command(CommandType.ENGINE_ABORT.value, {})
    ...
    def _thrust_changed(self, value: int):
        ...
        self.elodin.send_command(
            CommandType.SET_THRUST.value, {"thrust_percent": float(value)}
        )
    ...
    def _update_pressure_plot(self):
        ...
        for key, data in self.plot_data["pressure"].items():
            if key not in self.pressure_curves:
                self.pressure_curves[key] = self.pressure_plot.plot(
                    pen=pg.mkPen(width=2), name=key
                )
    ...
    def _update_temperature_plot(self):
        ...
        for key, data in self.plot_data["temperature"].items():
            if key not in self.temperature_curves:
                self.temperature_curves[key] = self.temperature_plot.plot(
                    pen=pg.mkPen(width=2), name=key
                )
