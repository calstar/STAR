...
void DiabloNavigationFilter::reset() {
    ...
    state_.altitude_variance_m2 = config_.initial_altitude_uncertainty * 
                                  config_.initial_altitude_uncertainty;
    state_.velocity_variance_m2s2 = config_.initial_velocity_uncertainty * 
                                    config_.initial_velocity_uncertainty;
...
bool DiabloNavigationFilter::isOutlier(double measurement, double predicted, double variance) const {
...
void DiabloNavigationFilter::adaptNoiseBasedOnQuality(double sensor_quality) {
    if (!config_.enable_adaptive_noise) return;
...
DiabloNavigationFilter::FilterState DiabloNavigationFilter::update(
    ...
    auto time_diff = std::chrono::duration_cast<std::chrono::milliseconds>(
        measurement.timestamp - state_.timestamp).count() / 1000.0;
    ...
    if (time_diff > 0.0 && time_diff < 10.0) {  // Reasonable time difference
        ...
        if (measurement.altitude_m > 0.0) {
            ...
            double innovation_variance = predicted.altitude_variance_m2 + 
                                        measurement.uncertainty_altitude_m * 
                                        measurement.uncertainty_altitude_m;
        ...
        if (measurement.thrust_estimated_n >= 0.0) {
            ...
            state_.thrust_n = alpha * measurement.thrust_estimated_n + 
                             (1.0 - alpha) * state_.thrust_n;
            state_.thrust_variance_n2 = measurement.uncertainty_thrust_n * 
                                       measurement.uncertainty_thrust_n;
        }
