...
Navigation::Navigation(Config config)
    ...
    if (is_hitl) {
        std::cout << "[Navigation] HITL is enabled; velocity and acceleration will be simulated"
                  << std::endl;
    }
...
void Navigation::populate_navigation_message_ned() {
    ...
    // Update global navigation message
    {
        ...
        set_navigation_message(navigation_message, time_monotonic, quat_ned_to_body, pos_lla,
                               vel_ned, accel_ned);
    }
...
void Navigation::update() {
    ...
    while (true) {
        ...
        // Break if abort mode
        if (static_cast<EngineMode>(_state_machine_output.template getField<1>()) ==
            EngineMode::ABORT) {
            break;
        }
