...
static std::string getTomlValue(const std::string& content,
...
static bool parseActuatorRole(const std::string& val, ActuatorRole& out,
    ...
    try {
        std::string ch_str = trimVal(val.substr(c1 + 1,
            (c2 != std::string::npos) ? c2 - c1 - 1 : std::string::npos));
        out.channel = std::stoi(ch_str);

        if (c2 != std::string::npos) {
            int bid = std::stoi(trimVal(val.substr(c2 + 1)));
            auto it = board_id_to_ip.find(bid);
            out.board_ip = (it != board_id_to_ip.end())
                               ? it->second
                               : "192.168.2." + std::to_string(bid);
        } else {
...
// load()
// ─────────────────────────────────────────────────────────────────────────────
bool ActuatorCommander::load(const std::string& config_content,
    ...
    // -- Actuator roles from [actuator_roles] section --
    {
        ...
        while (std::getline(cfg, line)) {
            ...
            if (current_section != "actuator_roles") continue;
            auto eq = line.find('=');
            if (eq == std::string::npos) continue;

            std::string role_name = trimVal(line.substr(0, eq));
            ActuatorRole role;
            if (!parseActuatorRole(trimVal(line.substr(eq + 1)), role, board_id_to_ip)) continue;
            roles_[role_name] = role;
        }
    }
    std::cout << "[ActuatorCommander] Loaded " << roles_.size()
              << " actuator roles from config" << std::endl;
    ...
    if (!f.is_open()) {
        for (const char* fb : fallbacks) {
            if (std::string(fb) == csv_path) continue;
            f.open(fb);
            if (f.is_open()) { used_csv = fb; break; }
        }
    }
    if (!f.is_open()) {
    ...
    while (std::getline(f, line)) {
        ...
        for (size_t col = 1; col < headers.size() && col < cells.size(); ++col) {
            if (headers[col].empty()) continue;
            std::string val = cells[col];
            std::transform(val.begin(), val.end(), val.begin(), ::toupper);
            int pos = -1;
            if (val == "OPEN")                pos = 1;
            else if (val == "CLOSE" || val == "CLOSED") pos = 0;
            if (pos < 0) continue;
    ...
    loaded_ = true;
    std::cout << "[ActuatorCommander] Loaded " << state_actuators_.size()
              << " states from " << used_csv << std::endl;
    return true;
}
...
// ─────────────────────────────────────────────────────────────────────────────
bool ActuatorCommander::sendUDP(
    const std::string& board_ip,
    const std::vector<std::pair<uint8_t, uint8_t>>& id_state_pairs)
{
    if (id_state_pairs.empty()) return true;
...
void ActuatorCommander::applyForState(State state) {
    ...
    for (const auto& [act_name, logical_pos] : it->second) {
        ...
        if (is_fire && role.is_pwm) continue; // skipped: controlled by controller_service

        // Manual override takes precedence
        int pos = logical_pos;
        auto ov = manual_overrides_.find(act_name);
        if (ov != manual_overrides_.end()) pos = ov->second;
    ...
    for (const auto& [ip, cmds] : by_board) {
        if (sendUDP(ip, cmds))
            std::cout << "[ActuatorCommander] Sent " << cmds.size()
                      << " commands to " << ip << " for state " << state_name << std::endl;
        else
...
bool ActuatorCommander::sendSingleActuator(const std::string& name, int pos) {
    ...
    if (it == roles_.end()) {
        ...
        for (auto jt = roles_.begin(); jt != roles_.end(); ++jt) {
            if (toLower(jt->first) == lower) { it = jt; break; }
        }
    }
    if (it == roles_.end()) {
