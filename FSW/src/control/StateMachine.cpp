...
std::string StateMachine::name(State s) {
    switch (s) {
        ...
        case State::FUEL_FILL:       return "Fuel Fill";
        case State::OX_FILL:         return "Ox Fill";
        case State::PRESS_STANDBY:   return "Press Standby";
        case State::GN2_LOW_PRESS:   return "GN2 Low Press";
        case State::GN2_VENT:        return "GN2 Low Vent";
        case State::FUEL_PRESS:      return "Fuel Press";
        case State::FUEL_VENT:       return "Fuel Vent";
        case State::OX_PRESS:        return "Ox Press";
        case State::OX_VENT:         return "Ox Vent";
        case State::GN2_HIGH_PRESS:  return "GN2 High Press";
        case State::GN2_HIGH_VENT:   return "GN2 High Vent";
        case State::VENT:            return "Vent";
        case State::CALIBRATE:       return "Calibrate";
        case State::READY:           return "Ready";
        case State::FIRE:            return "Fire";
        case State::ENGINE_ABORT:    return "Engine Abort";
        case State::GSE_ABORT:       return "GSE Abort";
        case State::EMERGENCY_ABORT: return "Emergency Abort";
...
bool StateMachine::load(const std::string& csv_path) {
    ...
    std::vector<State> column_states;
    {
        ...
        while (std::getline(iss, cell, ',')) {
            if (first) { first = false; continue; } // skip leading empty cell
    ...
    while (std::getline(f, line)) {
        ...
        std::istringstream iss(line);
        std::string cell;
        while (std::getline(iss, cell, ',')) cells.push_back(trimCell(cell));
        if (cells.empty() || cells[0].empty()) continue;

        State from = fromName(cells[0]);
        if (from == State::UNKNOWN) continue;
...
std::vector<State> StateMachine::allowedFrom(State state) const {
    ...
    if (it != transitions_.end()) {
        for (State s : it->second) result.push_back(s);
    }
    return result;
}
