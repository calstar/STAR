...
bool SequencerService::init(const std::string& config_path) {
    ...
    if (!state_machine_.load(sm_csv)) {
        std::cerr << "[SequencerService] Failed to load state_transitions.csv" << std::endl;
        return false;
    }

    // Actuator commander
    std::string act_csv = "external/DiabloAvionics/test_guis/state_machine_actuators.csv";
    if (!actuator_commander_.load(config_content_, act_csv)) {
        std::cerr << "[SequencerService] Failed to load state_machine_actuators.csv" << std::endl;
        return false;
    }
