...
private:
    ...
    std::map<uint8_t, std::vector<std::pair<double, double>>>
        calibration_data_;  // voltage, reference_pressure
...
public:
    ...
    void startCalibrationForChannel(uint8_t channel_id) {
        ...
        if (active_calibration_sessions_.find(channel_id) != active_calibration_sessions_.end()) {
            std::cout << "⚠️  Calibration already active for channel "
                      << static_cast<int>(channel_id) << std::endl;
            return;
        }

        // Create calibration procedure
        auto procedure = calibration_tool_->createCalibrationProcedure(
            "live_calibration_ch" + std::to_string(channel_id), 0.0,
            300000.0,  // 0 to 300 kPa range
            10,        // 10 data points
        ...
        std::string session_id =
            calibration_tool_->startCalibrationSession(channel_id, channel_id, procedure);
    ...
    void provideReferencePressure(uint8_t channel_id, double reference_pressure_pa) {
        ...
        if (it == active_calibration_sessions_.end()) {
            std::cout << "⚠️  No active calibration session for channel "
                      << static_cast<int>(channel_id) << std::endl;
            return;
        }
        ...
        if (data_it == calibration_data_.end() || data_it->second.empty()) {
            std::cout << "⚠️  No recent voltage data for channel " << static_cast<int>(channel_id)
                      << std::endl;
            return;
        }
        ...
        calibration_tool_->addCalibrationDataPoint(session_id, voltage, reference_pressure_pa,
                                                   environment);

        std::cout << "✓ Added calibration point: " << std::fixed << std::setprecision(2) << voltage
                  << "V → " << reference_pressure_pa << "Pa" << std::endl;

        // Stream human feedback to Elodin DB
        calibration_monitor_->processHumanFeedback("PT_" + std::to_string(channel_id),
                                                   reference_pressure_pa, voltage,
                                                   true  // accepted
        );
    ...
    void completeCalibrationForChannel(uint8_t channel_id) {
        ...
        std::string session_id = it->second;
        std::cout << "\n🔄 Completing calibration for channel " << static_cast<int>(channel_id)
                  << "..." << std::endl;
        ...
        try {
            ...
            if (session.calibration_successful) {
                std::cout << "✅ Calibration successful for channel "
                          << static_cast<int>(channel_id) << "!" << std::endl;

                // Stream calibration results to Elodin DB
                calibration_monitor_->processCalibrationUpdate("PT_" + std::to_string(channel_id),
                ...
                std::cout << "❌ Calibration failed for channel " << static_cast<int>(channel_id)
                          << std::endl;
            }
...
private:
    void onSensorData(uint8_t sensor_id, double raw_voltage_v, uint64_t timestamp,
        ...
        if (sensor_id == 2 || sensor_id == 3) {
            calibration_data_[sensor_id].push_back(
                {raw_voltage_v, 0.0});  // reference pressure will be added later
        ...
        calibration_monitor_->processPTMeasurement(pt_message.get(),
                                                   0.0);  // No reference pressure yet
        ...
        if (sensor_id == 2 || sensor_id == 3) {
            std::cout << "📡 Ch" << static_cast<int>(sensor_id) << ": " << std::fixed
                      << std::setprecision(4) << raw_voltage_v << "V" << " [STREAMED TO ELODIN DB]";

            if (active_calibration_sessions_.find(sensor_id) !=
                active_calibration_sessions_.end()) {
    ...
    void processingLoop() {
        ...
        while (running_) {
            ...
            if (std::chrono::duration_cast<std::chrono::seconds>(now - last_stats_time).count() >=
                10) {
                printStatistics();
                last_stats_time = now;
            }
    ...
    void printStatistics() {
        ...
        for (const auto& pair : packet_counts_) {
            ...
            if (last_seen_it != last_seen_.end()) {
                auto age = std::chrono::duration_cast<std::chrono::milliseconds>(
                               std::chrono::steady_clock::now() - last_seen_it->second)
                               .count();

                std::cout << "Ch" << static_cast<int>(sensor_id) << ": " << count << " packets, "
                          << age << "ms ago";
        ...
        std::cout << "Active calibration sessions: " << active_calibration_sessions_.size()
                  << std::endl;
        std::cout << "================================" << std::endl;
    }
};
...
void printUsage() {
    ...
    std::cout << "  ref_pressure_2 <pressure> - Provide reference pressure for channel 2"
              << std::endl;
    std::cout << "  ref_pressure_3 <pressure> - Provide reference pressure for channel 3"
              << std::endl;
