...
public:
    ...
    std::shared_ptr<PTMessage> generatePTData(uint8_t sensor_id, double reference_pressure_pa, 
        ...
        auto hours_since_install = std::chrono::duration_cast<std::chrono::hours>(now - chars.install_time).count();
        chars.accumulated_aging = chars.aging_rate * hours_since_install;
        
            // Calculate environmental effects
            double temp_effect = chars.temperature_coeff * (environment.temperature - 25.0); // 25°C reference
...
public:
    ...
    void saveCalibration(const CalibrationSession& session, const CalibrationParameters& params) {
        ...
        std::cout << "Saved calibration for " << session.sensor_id 
                  << " (calibration #" << entry.calibration_count << ")" << std::endl;
    }
    ...
    void printCalibrationStatistics() const {
        ...
        for (const auto& pair : calibrations_) {
            ...
            std::cout << "Sensor " << entry.sensor_id 
                      << ": " << entry.calibration_count << " calibrations, "
                      << "confidence: " << std::fixed << std::setprecision(3) << entry.confidence_score
                      << ", last: " << formatTimestamp(entry.timestamp) << std::endl;
        }
        std::cout << "=======================================\n" << std::endl;
    }
...
private:
    ...
    void saveCalibrationToFile(const CalibrationEntry& entry, const std::string& filename) {
        ...
        file << "  \"confidence_score\": " << std::fixed << std::setprecision(6) << entry.confidence_score << ",\n";
        ...
        for (int i = 0; i < entry.theta.size(); ++i) {
            ...
            if (i < entry.theta.size() - 1) file << ", ";
        }
        file << "],\n";
        file << "  \"quality_metrics\": {\n";
        file << "    \"nrmse\": " << std::fixed << std::setprecision(6) << entry.quality.nrmse << ",\n";
        file << "    \"coverage_95\": " << std::fixed << std::setprecision(6) << entry.quality.coverage_95 << ",\n";
        file << "    \"extrapolation_confidence\": " << std::fixed << std::setprecision(6) << entry.quality.extrapolation_confidence << "\n";
        file << "  }\n";
        file << "}\n";
...
public:
    FakeDataTestSystem() 
        : data_generator_(),
          calibration_storage_(),
          calibration_tool_("standard") {
    ...
    void runTestScenario() {
        ...
        for (int sensor_id = 0; sensor_id < 3; ++sensor_id) { // Test first 3 sensors
            ...
            bool has_existing = calibration_storage_.loadCalibration("PT_" + std::to_string(sensor_id), existing_cal);
            
            if (has_existing) {
                std::cout << "Found existing calibration (confidence: " 
                          << std::fixed << std::setprecision(3) << existing_cal.confidence_score << ")" << std::endl;
            } else {
    ...
    void runCalibrationForSensor(uint8_t sensor_id, const std::vector<double>& test_pressures) {
        ...
        for (size_t i = 0; i < test_pressures.size(); ++i) {
            ...
            if (pt_message) {
                ...
                elodin_interface_->streamEnvironmentalConditions(sensor_name, &environment, 
                    std::chrono::duration_cast<std::chrono::nanoseconds>(
                        std::chrono::system_clock::now().time_since_epoch()).count());
                
                // Add calibration data point
                calibration_tool_.addCalibrationDataPoint(session_id, voltage, pressure, environment);
        ...
        try {
            ...
            if (session.calibration_successful) {
                ...
                calibration_monitor_->processCalibrationUpdate(sensor_name, 
                    session.calibration_result, getCalibrationCount(sensor_name) + 1);
                ...
                std::cout << "  Quality - Calibration Quality: " << std::fixed << std::setprecision(4) 
                          << session.calibration_result.calibration_quality << std::endl;
