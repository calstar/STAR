...
void dummy_tcp_server(int& out_port, std::vector<uint8_t>& out_data, bool& run) {
    ...
    if (new_socket >= 0) {
        ...
        while (run) {
            ...
            if (valread <= 0) break;
            for(int i=0; i<valread; ++i) out_data.push_back(buffer[i]);
        }
        close(new_socket);
    }
...
int main() {
    ...
    std::thread server_thread(dummy_tcp_server, std::ref(elodin_port), std::ref(received_data), std::ref(run_server));
    
    // Wait for OS to assign port
    while(elodin_port == 0) std::this_thread::sleep_for(std::chrono::milliseconds(10));
    ...
    if (!seq.init(test_config)) {
        std::cerr << "❌ FAILED: Sequencer init failed (ensure you are running from repository root where config/ and external/ exist)\n";
        run_server = false;
        server_thread.join();
        return 1;
    }
    ...
    // The VTable schema setup for SequencerState and StateTransition is roughly ~500 bytes and the 2 actual publishes are another ~35 bytes.
    if (received_data.size() < 100) {
        std::cerr << "❌ FAILED: Received unexpectedly small amount of bytes (" << received_data.size() << ") for schemas and data\n";
        return 1;
    }
    
    std::cout << "✅ SUCCESS: Elodin mock server gracefully received " << received_data.size() << " bytes of serialized schemas and state transitions.\n";
    return 0;
}
