...
std::map<std::string, double> generateEngineStatus() {
    ...
    status["chamber_pressure"] =
        g_current_state == EngineState::STEADY_STATE ? 300.0 + g_current_thrust * 7.0 : 0.0;
...
bool handleEngineStart(const GroundStationInterface::Command& cmd) {
    ...
    // Check preconditions
    if (g_current_state != EngineState::STANDBY && g_current_state != EngineState::IGNITION_PREP) {
...
int main(int argc, char** argv) {
    ...
    gs_interface->registerCommandHandler(GroundStationInterface::CommandType::ENGINE_START,
                                         handleEngineStart);

    gs_interface->registerCommandHandler(GroundStationInterface::CommandType::ENGINE_STOP,
                                         handleEngineStop);

    gs_interface->registerCommandHandler(GroundStationInterface::CommandType::ENGINE_ABORT,
                                         handleAbort);

    gs_interface->registerCommandHandler(GroundStationInterface::CommandType::SET_THRUST,
                                         handleSetThrust);

    gs_interface->registerCommandHandler(GroundStationInterface::CommandType::VALVE_CONTROL,
                                         handleValveControl);
