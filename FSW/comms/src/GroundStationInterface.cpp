...
void GroundStationInterface::commandProcessLoop() {
    ...
    while (running_) {
        ...
        // Wait for commands
        commands_cv_.wait_for(lock, std::chrono::milliseconds(100), 
                            [this] { return !incoming_commands_.empty() || !running_; });
...
void GroundStationInterface::telemetrySendLoop() {
    ...
    while (running_) {
        ...
        // Wait for telemetry
        telemetry_cv_.wait_for(lock, std::chrono::milliseconds(100),
                              [this] { return !outgoing_telemetry_.empty() || !running_; });
...
std::vector<uint8_t> GroundStationInterface::serializeTelemetry(const TelemetryPacket& telemetry) {
    ...
    *reinterpret_cast<uint16_t*>(packet.data() + 6) = static_cast<uint16_t>(telemetry.sequence_number);
...
GroundStationInterface::Command GroundStationInterface::deserializeCommand(const std::vector<uint8_t>& data) {
...
void FSWGroundStationBridge::updateSensorTelemetry(const std::map<std::string, double>& sensor_data) {
...
void FSWGroundStationBridge::startTelemetryStreaming(std::chrono::milliseconds interval) {
    ...
    std::cout << "🚀 Telemetry streaming started (" << interval.count() << " ms interval)" << std::endl;
}
