...
ElodinCommandHandler::ElodinCommandHandler(const Config& config)
    ...
    std::cout << "  Validation enabled: " << (config_.enable_validation ? "yes" : "no")
              << std::endl;
}
...
void ElodinCommandHandler::executeCommand(const Command& cmd) {
    ...
    CommandHandler handler;
    {
        ...
        if (it == handlers_.end()) {
            std::cerr << "❌ No handler registered for command type: " << static_cast<int>(cmd.type)
                      << std::endl;
            commands_failed_++;
...
ElodinCommandHandler::CommandType ElodinCommandHandler::parseCommandType(
...
bool ElodinFSWIntegration::start() {
    ...
    command_handler_->registerHandler(ElodinCommandHandler::CommandType::ENGINE_START,
                                      [this](const auto& cmd) {
    ...
    command_handler_->registerHandler(ElodinCommandHandler::CommandType::ENGINE_STOP,
                                      [this](const auto& cmd) {
    ...
    command_handler_->registerHandler(ElodinCommandHandler::CommandType::ENGINE_ABORT,
                                      [this](const auto& cmd) {
                                          if (abort_handler_) {
    ...
    command_handler_->registerHandler(ElodinCommandHandler::CommandType::SET_THRUST,
                                      [this](const auto& cmd) {
                                          if (thrust_handler_) {
    ...
    command_handler_->registerHandler(
        ElodinCommandHandler::CommandType::VALVE_CONTROL, [this](const auto& cmd) {
            if (valve_handler_) {
    ...
    command_handler_->registerHandler(
        ElodinCommandHandler::CommandType::STATE_TRANSITION, [this](const auto& cmd) {
