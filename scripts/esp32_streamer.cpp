...
void generateESP32PTData() {
    ...
    while (running) {
        ...
        try {
            ...
            std::cout << "PT Ch2: P=" << pressure_ch2 << " Pa, T=" << temperature_ch2 << " C [ESP32 DATA SENT]" << std::endl;
        ...
        try {
            write_to_elodindb(packet_id, pt_msg);
            LocalSock->flush_elodin(); // Flush buffer to ensure data is sent
            std::cout << "PT Ch3: P=" << pressure_ch3 << " Pa, T=" << temperature_ch3 << " C [ESP32 DATA SENT]" << std::endl;
        } catch (const std::exception& e) {
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        // Generate database configuration (send vtable schemas) - EXACTLY like fake_sensor_generator
        cppGenerateDBConfig();
