...
CMD_WEB_BACKEND="printf '\n  ══ LEGACY BACKEND — WS :8081 (server-legacy.ts) ══\n\n' && sleep 5 && cd $PROJECT/web-gui/backend && $ACTUATOR_SVC_ENV ELODIN_RELAY_WS_URL=ws://localhost:9090 USE_DIRECT_DAQ=false USE_CALIBRATION_SERVICE_CALIBRATED=false USE_CPP_CONTROLLER=true npx tsx watch src/server-legacy.ts 2>&1"
