#!/bin/bash
# Full stack with thin Node backend (server-thin.ts), matching scripts/test/test_integration.sh defaults.
# Same pipeline as start_tmux_dev.sh; only the web-gui backend pane differs (relay URL env + entrypoint).
#
...
pkill -f "tsx watch.*server-thin\.ts" 2>/dev/null || true
pkill -f "tsx.*server-thin\.ts" 2>/dev/null || true
...
# Sequencer service: state machine + actuator UDP (same TCP :9998 text protocol as server-thin.ts)
...
CMD_WEB_BACKEND="printf '\n  ══ THIN BACKEND — HTTP+WS :${THIN_WS_PORT} (server-thin.ts) ══\n\n' && sleep 5 && cd $PROJECT/web-gui/backend && WS_PORT=$THIN_WS_PORT ELODIN_RELAY_URL=$THIN_RELAY_URL ACTUATOR_SERVICE_PORT=$THIN_ACT_PORT npx tsx watch src/server-thin.ts 2>&1"
...
echo "┌─────────────────────────────────────────────────────────────┐"
echo "│  Pipeline: UDP → daq_bridge → DB → relay → thin backend → UI │"
