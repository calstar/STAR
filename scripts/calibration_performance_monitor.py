...
class PerformanceMonitor:
    ...
    def _monitor_loop(self):
        ...
        while self._running:
            try:
                ...
                # Average times
                avg_consensus_ms = sum(self.consensus_times) / len(self.consensus_times) if self.consensus_times else 0
                avg_calibration_ms = sum(self.calibration_times) / len(self.calibration_times) if self.calibration_times else 0
    ...
    def get_summary(self) -> Dict:
        ...
        return {
            ...
            'avg_calibration_ms': sum(m.calibration_time_ms for m in recent) / len(recent),
    ...
    def print_summary(self):
        ...
        logger.info(f"Memory: {summary['avg_memory_mb']:.1f} MB avg, {summary['max_memory_mb']:.1f} MB max")
        logger.info(f"Consensus: {summary['avg_consensus_ms']:.1f} ms avg, {summary['max_consensus_ms']:.1f} ms max")
        logger.info(f"Calibration: {summary['avg_calibration_ms']:.1f} ms avg, {summary['max_calibration_ms']:.1f} ms max")
