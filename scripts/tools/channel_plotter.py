...
try:
    from calibration_robustness import (
        ...
        HealthMetrics, ValidationResult
    )
...
try:
    from autonomous_calibration_engine import (
        AutonomousCalibrationEngine, CalibrationRequest,
        OnlineBayesianLearner, ActiveLearningAgent
    )
...
class VoltageFilter:
    ...
    def update(self, new_voltage: float) -> float:
        ...
        self.ema_value = self.alpha_ema * new_voltage + (1 - self.alpha_ema) * self.ema_value
        
        # Low-Pass Filter on top of EMA
        self.lpf_value = self.alpha_lpf * self.ema_value + (1 - self.alpha_lpf) * self.lpf_value
...
class MultivariateBayesianCalibration:
    ...
    def __init__(self, num_sensors: int = 16, order: int = 3):
        ...
        self.filter_stats = {i: {'outliers_rejected': 0, 'rate_limits_applied': 0} for i in range(num_sensors)}
        ...
        self.population_mean[1] = 200.0  # Linear term (200 PSI/V for typical 1000 PSI transducer)
        ...
        covariance_diagonal[0] = 10000.0    # Intercept: very uncertain initially (±100 PSI)
        ...
        for i in range(6, self.n_params):
            covariance_diagonal[i] = 1000.0  # Environmental interaction terms: moderate uncertainty
        ...
        self.pt_covariances = {i: np.copy(self.population_covariance) for i in range(num_sensors)}
        self.pt_precisions = {i: np.copy(self.population_precision) for i in range(num_sensors)}
    ...
    def filter_voltage(self, sensor_id: int, raw_voltage: float, dt: float = 0.1) -> float:
        """
    ...
    def reset_voltage_filter(self, sensor_id: int):
        ...
        if sensor_id in self.voltage_filters:
            ...
            self.filter_stats[sensor_id] = {'outliers_rejected': 0, 'rate_limits_applied': 0}
    ...
    def compute_consensus_pressure(self, voltages: Dict[int, float], env_state: EnvironmentalState) -> ConsensusState:
        """
    ...
    def _compute_consensus_pressure_unsafe(self, voltages: Dict[int, float], env_state: EnvironmentalState) -> ConsensusState:
        ...
        for sensor_id, voltage in voltages.items():
            ...
            if np.all(mean == self.population_mean) and np.allclose(cov, self.population_covariance):
                continue
        ...
        self._update_uncertainty_inflation(sensor_ids, residuals, uncertainties, agreement_score)
    ...
    def _update_uncertainty_inflation(self, sensor_ids: List[int], residuals: np.ndarray, 
                                     uncertainties: np.ndarray, agreement_score: float):
        """
        ...
        for i, sensor_id in enumerate(sensor_ids):
            ...
            disagreement_threshold = max(1.5, 3.0 - self.population_strength * 0.2)  # 3.0 → 1.5
            agreement_threshold = max(0.3, 0.7 - self.population_strength * 0.1)     # 0.7 → 0.3
            ...
            if normalized_residual > disagreement_threshold:
                ...
                self.uncertainty_inflation_factor[sensor_id] = min(2.0, self.uncertainty_inflation_factor[sensor_id])  # Cap at 2x (was 10x)
                
                # DON'T reset autonomy - keep the calibration knowledge
                # self.autonomy_levels[sensor_id] *= 0.95  # DISABLED
                
                logger.info(f"PT{sensor_id} harness change detected (residual: {normalized_residual:.2f}σ), "
                           f"uncertainty: {self.uncertainty_inflation_factor[sensor_id]:.2f}x - calibration RETAINED")
                             
            elif normalized_residual < agreement_threshold and agreement_score > min_agreement:
                # CONSISTENT AGREEMENT: DEFLATE uncertainty (build confidence)
                deflation_rate = 0.98 - min(0.03, self.population_strength * 0.01)  # 0.98 → 0.95
                self.uncertainty_inflation_factor[sensor_id] *= deflation_rate
                self.uncertainty_inflation_factor[sensor_id] = max(0.3, self.uncertainty_inflation_factor[sensor_id])
                
                # Boost autonomy slightly (system is more confident)
                self.autonomy_levels[sensor_id] = min(1.0, self.autonomy_levels[sensor_id] * 1.01)
                
                if normalized_residual < 0.2:  # Very good agreement
                    logger.info(f"PT{sensor_id} excellent agreement: {normalized_residual:.2f}σ, "
    ...
    def _update_consensus_confidence(self, agreement_score: float, num_sensors: int):
        ...
        total_human_points = sum(
            sum(1 for pt in self.calibration_points[sensor_id] 
        ...
        human_boost = min(0.8, total_human_points * 0.15)  # 15% per human point, max 80%
        ...
        self.consensus_confidence = (
            0.40 * human_boost +           # Human ground truth (dominant)
            0.30 * agreement_factor +      # Sensor agreement
            0.20 * sensor_factor +         # Number of participating sensors
            0.10 * population_factor       # Population learning strength
        )
    ...
    def _estimate_observation_noise(self, sensor_id: int, new_point: CalibrationPoint) -> float:
        """
        ...
        if hasattr(new_point, 'is_human_input') and new_point.is_human_input:
            ...
            logger.info(f"PT{sensor_id} HUMAN INPUT (ground truth): obs_variance={obs_variance:.8f}")
            return obs_variance
        ...
        if hasattr(new_point, 'is_consensus') and new_point.is_consensus:
            ...
            logger.info(f"PT{sensor_id} CONSENSUS INPUT: obs_variance={obs_variance:.6f}")
            return obs_variance
    ...
    def multivariate_calibration_update(self, sensor_id: int, calibration_point: CalibrationPoint):
        """
        ...
        try:
            if not (0 <= sensor_id < self.num_sensors):
                raise ValueError(f"Invalid sensor_id: {sensor_id}, must be 0-{self.num_sensors-1}")
            
            if not np.isfinite(calibration_point.voltage):
                raise ValueError(f"Non-finite voltage: {calibration_point.voltage}")
            
            if calibration_point.pressure is not None and not np.isfinite(calibration_point.pressure):
            ...
            if calibration_point.voltage < 0 or calibration_point.voltage > 15.0:
                logger.warning(f"PT{sensor_id} voltage out of normal range: {calibration_point.voltage}V")
    ...
    def _multivariate_calibration_update_unsafe(self, sensor_id: int, calibration_point: CalibrationPoint):
        """Internal implementation without locking"""
        # Add to history
        self.calibration_points[sensor_id].append(calibration_point)
        
        # Get design matrix for this observation
        phi = self._design_matrix(calibration_point.voltage, calibration_point.environmental_state, sensor_id)
        ...
        if hasattr(calibration_point, 'is_human_input') and calibration_point.is_human_input:
            ...
            logger.info(f"PT{sensor_id} STRONG HUMAN UPDATE: precision boost = {human_precision/obs_precision:.0f}x")
        ...
        new_mean = self.pt_covariances[sensor_id] @ (old_precision @ old_mean + innovation_term)
        ...
        all_precisions = np.array([np.trace(self.pt_precisions[i]) for i in range(self.num_sensors)])
        ...
        logger.info(f"PT{sensor_id} multivariate update: population_strength={self.population_strength:.2f}, "
    ...
    def _validate_calibration_quality(self, sensor_id: int):
        ...
        logger.info(f"PT{sensor_id} quality: RMSE={rmse:.3f}, MAE={mae:.3f}, points={len(points)}")
        ...
        for i in range(self.num_sensors):
            ...
            if updated_sensor not in recent_predictions:
                ...
                correlation = self._compute_pressure_correlation(
                    recent_predictions[updated_sensor],
                    recent_predictions[i]
                )
    ...
    def _compute_pressure_correlation(self, pred1: List[Tuple[float, float, float]], 
                                     pred2: List[Tuple[float, float, float]]) -> float:
        """
        ...
        overlap = min(np.max(pressures1), np.max(pressures2)) - max(np.min(pressures1), np.min(pressures2))
        total_range = max(np.max(pressures1), np.max(pressures2)) - min(np.min(pressures1), np.min(pressures2))
        ...
        logger.debug(f"Pressure correlation: overlap={overlap_fraction:.2f}, corr={correlation:.2f}")
    ...
    def _update_sensor_confidence(self, sensor_id: int):
        ...
        human_points = sum(1 for pt in self.calibration_points[sensor_id] 
                          if hasattr(pt, 'is_human_input') and pt.is_human_input)
        
        # COUNT CONSENSUS/AUTO POINTS
        consensus_points = sum(1 for pt in self.calibration_points[sensor_id] 
        ...
        consensus_factor = min(0.6, consensus_points * 0.2)  # 20% per consensus point, max 60%
        ...
        # WEIGHTED COMBINATION - Human inputs dominate
        confidence = (
            0.40 * human_factor +        # Human ground truth (dominant)
            0.20 * consensus_factor +    # Consensus agreement
            0.15 * data_factor +         # Total data amount
            0.15 * precision_factor +    # Model precision
            0.10 * population_factor     # Population learning
        )
        ...
        if abs(self.confidence_scores[sensor_id] - current_confidence) > 0.05:
            logger.info(f"PT{sensor_id} confidence: {current_confidence:.2f} → {self.confidence_scores[sensor_id]:.2f} "
    ...
    def _update_consensus_confidence_from_calibration(self, sensor_id: int):
        ...
        # COUNT HUMAN INPUTS across all sensors
        total_human_points = sum(
            sum(1 for pt in self.calibration_points[sid] 
                if hasattr(pt, 'is_human_input') and pt.is_human_input)
        ...
        participating_sensors = sum(
            1 for sid in range(self.num_sensors) 
        ...
        new_consensus_confidence = (
            0.50 * human_boost +           # Human ground truth (dominant)
            0.30 * sensor_factor +         # Number of participating sensors
            0.20 * population_factor       # Population learning strength
        )
        ...
        if new_consensus_confidence > self.consensus_confidence:
            ...
            logger.info(f"Consensus confidence updated: {self.consensus_confidence:.3f} "
                       f"(human_points={total_human_points}, sensors={participating_sensors}, "
                       f"population={self.population_strength:.2f})")
    
    def _propagate_to_other_sensors(self, source_sensor: int, calibration_point: CalibrationPoint):
        """
        ...
        for target_sensor in range(self.num_sensors):
            ...
            propagation_strength = correlation * min(1.0, self.population_strength / 10.0)
            ...
            self.pt_means[target_sensor] = (1 - alpha) * current_mean + alpha * target_mean
            ...
            self.pt_covariances[target_sensor] = (1 - alpha) * current_cov + alpha * target_cov
            self.pt_precisions[target_sensor] = np.linalg.inv(self.pt_covariances[target_sensor])
    ...
    def _update_autonomy_levels(self):
        ...
        for sensor_id in range(self.num_sensors):
            ...
            consensus_points = sum(1 for pt in self.calibration_points[sensor_id] 
                                  if hasattr(pt, 'is_consensus') and pt.is_consensus)
            consensus_factor = min(0.7, consensus_points * 0.25)  # 25% per consensus point, max 70%
            ...
            if len(self.calibration_points[sensor_id]) > 0:
                ...
                for other_id in range(self.num_sensors):
                    if other_id != sensor_id and len(self.calibration_points[other_id]) > 0:
                        correlations.append(self.cross_sensor_covariance[sensor_id, other_id])
                if correlations:
                    correlation_factor = np.mean(correlations)
            
            # WEIGHTED COMBINATION - Human inputs dominate
            autonomy = (
                0.35 * human_factor +        # Human ground truth (dominant)
                0.20 * consensus_factor +    # Consensus agreement
                0.15 * precision_factor +    # Model precision
                0.15 * population_factor +   # Population learning
                0.10 * system_consensus +    # System agreement
                0.05 * correlation_factor    # Cross-sensor help
            )
            ...
            if abs(self.autonomy_levels[sensor_id] - current_autonomy) > 0.1:
                logger.info(f"PT{sensor_id} autonomy: {current_autonomy:.2f} → {self.autonomy_levels[sensor_id]:.2f} "
                           f"(human={human_points}, consensus={consensus_points}, precision={precision_factor:.2f}, "
                           f"population={population_factor:.2f})")
    
    def _design_matrix(self, voltage: float, env_state: EnvironmentalState, sensor_id: int) -> np.ndarray:
    ...
    def automatic_pressure_detection(self, voltages: Dict[int, float], env_state: EnvironmentalState) -> Optional[float]:
        """
        ...
        if (consensus.agreement_score > min_agreement and 
            len(consensus.participating_sensors) >= min_sensors and 
            self.consensus_confidence > min_confidence):
            
            logger.info(f"AUTO-DETECTED PRESSURE: {consensus.pressure:.1f} PSI "
    ...
    def self_calibrate_with_confidence(self, voltages: Dict[int, float], env_state: EnvironmentalState) -> bool:
        """
        ...
        for sensor_id, voltage in voltages.items():
            ...
            if (autonomy > 0.6 and 
                ...
                logger.info(f"SELF-CALIBRATED PT{sensor_id}: {detected_pressure:.1f} PSI at {voltage:.3f}V "
    ...
    def save_population_prior(self, filepath: str = "population_prior.json"):
        ...
        logger.info(f"💾 Saved population prior to {filepath} (strength={self.population_strength:.2f})")
    ...
    def load_population_prior(self, filepath: str = "population_prior.json"):
        ...
        try:
            ...
            logger.info(f"✅ Loaded population prior from {filepath} (strength={self.population_strength:.2f})")
            logger.info(f"🎯 ALL PTs initialized with accumulated knowledge from previous sessions!")
            return True
        except Exception as e:
...
class RobustCalibrationFramework:
    ...
    """
    def __init__(self, sensor_id: int, global_system: Optional[MultivariateBayesianCalibration] = None, order: int = 3):
    ...
    def _design_matrix(self, voltage: float, env_state: EnvironmentalState) -> np.ndarray:
        if self.standalone:
    ...
    def _perform_robust_calibration(self):
        ...
        try:
            ...
            for i, point in enumerate(self.calibration_points):
                ...
                sigma_env_sq = self._compute_environmental_uncertainty(env_states[i])  # Environmental uncertainty
                
                sigma_total_sq[i] = sigma_gauge_sq + sigma_meas_sq + sigma_temp_sq + sigma_aging_sq + sigma_v_sq + sigma_env_sq
            ...
            self.covariance_matrix = np.linalg.inv(Sigma_prior_inv + Sigma_likelihood_inv)
            
            # Posterior mean (paper equation 138)
            self.coefficients = self.covariance_matrix @ (
                Sigma_prior_inv @ self.pop_mean + 
                X.T @ W @ pressures
            )
    ...
    def _compute_environmental_uncertainty(self, env_state: EnvironmentalState) -> float:
        """Compute environmental uncertainty from paper equation 192-199"""
        # Environmental variance model
        env_vector = np.array([env_state.temperature, env_state.humidity, 
                              env_state.vibration, env_state.aging_factor, env_state.mounting_torque])
        
        # Base environmental uncertainty
        sigma_env_sq = 0.01**2  # Base environmental variance
        
        # Environmental interaction terms
        sigma_env_sq += np.dot(env_vector, np.dot(self.env_covariance_matrix, env_vector)) if self.env_covariance_matrix is not None else 0
        
        return sigma_env_sq
    
    def _validate_calibration_quality(self, X: np.ndarray, pressures: np.ndarray, sigma_total_sq: np.ndarray):
        ...
        sigma_total_sq_safe = np.nan_to_num(sigma_total_sq, nan=1e-6, posinf=1e-6, neginf=1e-6)
        ...
        # Log quality metrics
        logger.info(f"PT{self.sensor_id} calibration quality: NRMSE={nrmse:.4f}, Coverage={coverage:.2f}")
        ...
        if nrmse > 0.1 or coverage < 0.8:
            logger.warning(f"PT{self.sensor_id} calibration quality is poor - may need more data points")
    ...
    def _run_rls_update(self, new_point: CalibrationPoint):
        ...
        if self.theta_rls is None or self.P_rls is None:
            ...
            self.theta_rls = self.coefficients if self.coefficients is not None else np.zeros(self.order + 1)
            self.P_rls = self.covariance_matrix if self.covariance_matrix is not None else np.eye(self.order + 1) * 100.0
        ...
        self.P_rls = (self.P_rls - np.outer(K_k, np.dot(phi_k, self.P_rls))) / self.forgetting_factor
        ...
        if self.covariance_matrix is None:
            ...
            self.covariance_matrix = (1 - alpha) * self.covariance_matrix + alpha * self.P_rls

    def _run_glr_test(self):
        # COMPLETE GENERALIZED LIKELIHOOD RATIO TEST from paper equation 235-248
        if len(self.calibration_points) < self.glr_window_size or self.coefficients is None:
            return False
        ...
        X_recent = np.array([self._design_matrix(v, e) for v, e in zip(recent_voltages, recent_env_states)])
        ...
        log_likelihood_null = -0.5 * np.sum(residuals**2 / sigma_sq) - 0.5 * len(residuals) * np.log(2 * np.pi * sigma_sq)
        ...
        try:
            ...
            if np.linalg.det(X_recent_T_X_recent) > 1e-10:  # Check for singularity
                theta_alt = np.linalg.solve(X_recent_T_X_recent, X_recent.T @ recent_pressures)
                predictions_alt = X_recent @ theta_alt
                residuals_alt = recent_pressures - predictions_alt
                sigma_sq_alt = np.var(residuals_alt)
                log_likelihood_alt = -0.5 * np.sum(residuals_alt**2 / sigma_sq_alt) - 0.5 * len(residuals_alt) * np.log(2 * np.pi * sigma_sq_alt)
                
                # GLR statistic
                glr_statistic = 2 * (log_likelihood_alt - log_likelihood_null)
                
                # Threshold for drift detection (paper equation 247)
                threshold = 2 * len(recent_points)  # Chi-squared threshold approximation
                
                drift_detected = glr_statistic > threshold
                
                if drift_detected:
                    logger.warning(f"PT{self.sensor_id} drift detected! GLR={glr_statistic:.2f} > {threshold:.2f}")
    ...
    def predict_pressure_with_uncertainty(self, voltage: float, env_state: EnvironmentalState) -> Tuple[float, float]:
        if self.coefficients is None:
        ...
        if self.covariance_matrix is not None:
            ...
            if not self.standalone and self.global_system is not None:
                inflation = self.global_system.uncertainty_inflation_factor[self.sensor_id]
    ...
    def get_confidence_level(self) -> str:
        ...
        _, uncertainty = self.predict_pressure_with_uncertainty(
            self.calibration_points[-1].voltage, self.calibration_points[-1].environmental_state
        )
    ...
    def get_calibration_curve_data(self, voltage_range: Tuple[float, float] = None) -> Tuple[List[float], List[float]]:
    ...
    def _update_environmental_covariance(self, env_states: List[EnvironmentalState]):
        ...
        env_matrix = np.array([
            [e.temperature, e.humidity, e.vibration, e.aging_factor, e.mounting_torque]
            for e in env_states
        ])
    ...
    def update_cross_pt_covariance(self, other_pt_id: int, other_framework: 'RobustCalibrationFramework'):
    ...
    def predict_pressure_with_covariance_mapping(self, voltage: float, env_state: EnvironmentalState, 
                                                reference_pt_id: int, reference_framework: 'RobustCalibrationFramework') -> Tuple[float, float]:
        ...
        if reference_pt_id in self.cross_pt_covariance:
            ...
            ref_prediction, ref_uncertainty = reference_framework.predict_pressure_with_uncertainty(voltage, env_state)
            
            # Covariance mapping factor (from paper equation 354)
            cross_cov = self.cross_pt_covariance[reference_pt_id]
            mapping_factor = np.trace(cross_cov) / (np.trace(reference_framework.covariance_matrix) + 1e-6)
            
            # Weighted combination
            alpha = min(0.3, mapping_factor)  # Limit influence
            corrected_prediction = (1 - alpha) * base_prediction + alpha * ref_prediction
            ...
            total_var = (1 - alpha)**2 * base_uncertainty**2 + alpha**2 * ref_uncertainty**2
            total_var = max(total_var, 1e-10)  # Ensure positive
            total_uncertainty = np.sqrt(total_var)
            total_uncertainty = np.nan_to_num(total_uncertainty, nan=1.0, posinf=1.0, neginf=1.0)
    ...
    def update_autonomy_level(self):
        ...
        if self.coefficients is not None:
            ...
            recent_points = self.calibration_points[-5:] if len(self.calibration_points) >= 5 else self.calibration_points
            avg_uncertainty = 0.0
            for point in recent_points:
                _, uncertainty = self.predict_pressure_with_uncertainty(point.voltage, point.environmental_state)
            ...
            calibration_factor = min(1.0, len(self.calibration_points) / 10.0)  # More points = more autonomous
    ...
    def should_request_human_input(self, voltage: float, env_state: EnvironmentalState) -> bool:
    ...
    def load_calibration_state(self, filepath: str = "calibration_state.json"):
        ...
        try:
            ...
            logger.info(f"Loaded calibration state for PT{self.sensor_id} with {len(self.calibration_points)} points")
    ...
    def perform_self_calibration(self, other_pt_id: int, other_framework: 'RobustCalibrationFramework', 
                                voltage: float, env_state: EnvironmentalState) -> Tuple[float, float]:
        """Perform self-calibration using another PT as reference (from paper Section 6)"""
        if not self.cross_calibration_enabled or self.autonomy_level < self.self_calibration_threshold:
            return self.predict_pressure_with_uncertainty(voltage, env_state)
            
        # Use reference PT's calibration to inform this PT
        ref_prediction, ref_uncertainty = other_framework.predict_pressure_with_uncertainty(voltage, env_state)
        ...
        self_prediction, self_uncertainty = self.predict_pressure_with_uncertainty(voltage, env_state)
        
        # Bayesian combination (from paper equation 354)
        combined_prediction = (1 - self_cal_weight) * self_prediction + self_cal_weight * ref_prediction
        ...
        logger.info(f"PT{self.sensor_id} self-calibrated using PT{other_pt_id} (weight: {self_cal_weight:.2f})")
    ...
    def run(self):
        ...
        while self._running:
            ...
            try:
                ...
                env_state = EnvironmentalState()
                consensus = self.global_system.compute_consensus_pressure(voltages, env_state)
...
class CalibrationWorkerThread(QtCore.QThread):
    ...
    """
    calibration_complete = QtCore.pyqtSignal(int, object)  # sensor_id, CalibrationResult
    ...
    def add_calibration_point(self, sensor_id: int, calibration_point: CalibrationPoint):
    ...
    def run(self):
        ...
        while self._running:
            try:
                ...
                self.global_system.multivariate_calibration_update(sensor_id, calibration_point)
                elapsed = time.time() - start_time
                
                logger.info(f"PT{sensor_id} calibration update completed in {elapsed*1000:.1f}ms")
...
class CalibrationWindow(QtWidgets.QDialog):
    def __init__(self, parent):
        ...
        self.data_table.setHorizontalHeaderLabels(["Voltage (V)", "Pressure (PSI)", "Uncertainty", "Timestamp"])
        ...
        self.variance_table.setHorizontalHeaderLabels(["Timestamp", "RMSE", "MAE", "Autonomy", "Points"])
        ...
        self.covariance_table.setHorizontalHeaderLabels(["Parameter", "Value", "Std Dev"])
    ...
    def add_calibration_point(self):
        try:
            channel = int(self.channel_combo.currentText()[2:])  # Extract number from "CH2"
            ...
            if channel in self.parent.latest_voltages:
                ...
                self.parent.calibration_worker.add_calibration_point(channel, calibration_point)
                
                logger.info(f"Submitted calibration point for CH{channel}: {pressure} PSI at {voltage:.3f}V")
                
                # Clear input
                self.pressure_input.clear()
                
                # Update calibration system immediately for UI
                self.parent.global_calibration_system.multivariate_calibration_update(channel, calibration_point)
                
                # AUTONOMOUS LEARNING: Update self-improving engine
                if self.parent.autonomous_engine:
                    # Get design vector (basis functions)
                    design_vector = self.parent.global_calibration_system._design_matrix(
                    ...
                    logger.info(f"🤖 Autonomous engine updated from PT{channel} calibration")
                    
                    # Save evolved prior periodically
                    if self.parent.autonomous_engine.total_calibrations % 5 == 0:
                        learned_prior = self.parent.autonomous_engine.export_learned_prior()
                        with open("learned_prior.json", 'w') as f:
                            json.dump(learned_prior, f, indent=2)
                        logger.info(f"💾 Saved evolved prior (confidence={learned_prior['confidence']:.3f})")
                ...
                if abs(pressure) < 10.0:  # Within 10 PSI of zero
                    logger.info(f"🎯 ZERO-POINT CALIBRATION detected for PT{channel} at {pressure:.1f} PSI")
                    self._propagate_zero_point_calibration(channel, voltage, pressure, env_state)
                ...
                QtWidgets.QMessageBox.warning(self, "No Data", f"No voltage data available for CH{channel}")
                
        except ValueError:
            QtWidgets.QMessageBox.warning(self, "Invalid Input", "Please enter a valid pressure value")
    
    def _propagate_zero_point_calibration(self, source_channel: int, voltage: float, pressure: float, env_state: EnvironmentalState):
        """
        ...
        """
        logger.info(f"🌐 Propagating zero-point calibration from PT{source_channel} to all other PTs...")
        ...
        for target_channel in range(1, 11):  # Channels 1-10
            ...
            if target_channel in self.parent.latest_voltages:
                ...
                self.parent.calibration_worker.add_calibration_point(target_channel, zero_point)
                self.parent.global_calibration_system.multivariate_calibration_update(target_channel, zero_point)
                
                propagated_count += 1
                logger.info(f"  ✓ Propagated to PT{target_channel} at {target_voltage:.3f}V")
        
        logger.info(f"✅ Zero-point calibration propagated to {propagated_count} PTs")
        QtWidgets.QMessageBox.information(
            self, "Zero-Point Propagation", 
    ...
    def clear_calibration_points(self):
        ...
        self.update_calibration_data()
        self.update_calibration_plot()
        logger.info(f"✅ CLEARED all calibration points for CH{channel} - reset to population prior")
    ...
    def update_display(self):
        ...
        if channel in self.parent.latest_voltages:
            ...
            env_state = EnvironmentalState()
            _, uncertainty = framework.predict_pressure_with_uncertainty(voltage, env_state)
            inflation = self.parent.global_calibration_system.uncertainty_inflation_factor[channel]
        ...
        self.consensus_confidence_label.setText(f"{global_system.consensus_confidence*100:.1f}%")
        
        # Show consensus state
        if self.parent.last_consensus_state is not None:
            consensus = self.parent.last_consensus_state
            self.consensus_pressure_label.setText(f"{consensus.pressure:.1f} PSI (±{consensus.uncertainty:.2f})")
            self.agreement_label.setText(f"{consensus.agreement_score*100:.1f}% ({len(consensus.participating_sensors)} sensors)")
        else:
    ...
    def update_calibration_data(self):
        try:
            ...
            global_points = self.parent.global_calibration_system.calibration_points.get(channel, [])
            
            logger.info(f"Updating calibration data for CH{channel}: {len(global_points)} points")
            ...
            for i, point in enumerate(global_points):
                ...
                if hasattr(point, 'is_human_input') and point.is_human_input:
                    brush = QtGui.QBrush(QtGui.QColor(255, 200, 200))  # Light red for human input
                else:
                ...
                pressure_item = QtWidgets.QTableWidgetItem(f"{point.pressure:.1f}" if point.pressure is not None else "N/A")
                pressure_item.setBackground(brush)
                self.data_table.setItem(i, 1, pressure_item)
                
                uncertainty_item = QtWidgets.QTableWidgetItem(f"{point.uncertainty:.3f}")
                uncertainty_item.setBackground(brush)
                self.data_table.setItem(i, 2, uncertainty_item)
                
                timestamp_item = QtWidgets.QTableWidgetItem(time.strftime("%H:%M:%S", time.localtime(point.timestamp)))
        ...
        quality_history = self.parent.global_calibration_system.quality_history.get(channel, [])
        self.variance_table.setRowCount(len(quality_history))
        
        for i, record in enumerate(quality_history):
            self.variance_table.setItem(i, 0, QtWidgets.QTableWidgetItem(time.strftime("%H:%M:%S", time.localtime(record['timestamp']))))
            self.variance_table.setItem(i, 1, QtWidgets.QTableWidgetItem(f"{record.get('rmse', 0):.4f}"))
            self.variance_table.setItem(i, 2, QtWidgets.QTableWidgetItem(f"{record.get('mae', 0):.4f}"))
            self.variance_table.setItem(i, 3, QtWidgets.QTableWidgetItem(f"{record.get('autonomy', 0):.2f}"))
            self.variance_table.setItem(i, 4, QtWidgets.QTableWidgetItem(f"{record.get('n_points', 0)}"))
        
        # Update covariance matrix
        if framework.covariance_matrix is not None and framework.coefficients is not None:
            param_names = [f"θ{i}" for i in range(len(framework.coefficients))]
            self.covariance_table.setRowCount(len(param_names))
            
            for i, (name, coeff, std_dev) in enumerate(zip(param_names, framework.coefficients, np.sqrt(np.diag(framework.covariance_matrix)))):
                self.covariance_table.setItem(i, 0, QtWidgets.QTableWidgetItem(name))
                self.covariance_table.setItem(i, 1, QtWidgets.QTableWidgetItem(f"{coeff:.6f}"))
                self.covariance_table.setItem(i, 2, QtWidgets.QTableWidgetItem(f"{std_dev:.6f}"))
        else:
    ...
    def update_calibration_plot(self):
        try:
            ...
            valid_points = [(p.voltage, p.pressure) for p in global_points if p.pressure is not None]
            ...
            logger.info(f"Pressure range: {min(pressures):.2f} - {max(pressures):.2f} PSI")
            
            # Plot calibration points (HUMAN INPUT = RED, LARGE)
            self.plot_widget.plot(voltages, pressures, pen=None, symbol='o', symbolSize=12, 
                                symbolBrush=(255, 0, 0), symbolPen='w', name='Human Calibration')
            ...
            if len(valid_points) >= 2:
                try:
                    ...
                    v_curve, p_curve = framework.get_calibration_curve_data((v_curve_min, v_curve_max))
                    
                    if v_curve and p_curve and len(v_curve) > 0:
                        # Plot prediction curve
                        self.plot_widget.plot(v_curve, p_curve, pen=pg.mkPen('b', width=2), name='Bayesian Fit')
                        ...
                        for v in v_curve:
                            p_pred, uncertainty = framework.predict_pressure_with_uncertainty(v, env_state)
                            pressures_upper.append(p_pred + 2 * uncertainty)
                            pressures_lower.append(p_pred - 2 * uncertainty)
                        
                        # Plot uncertainty bounds
                        self.plot_widget.plot(v_curve, pressures_upper, pen=pg.mkPen('gray', width=1, style=QtCore.Qt.PenStyle.DashLine))
                        self.plot_widget.plot(v_curve, pressures_lower, pen=pg.mkPen('gray', width=1, style=QtCore.Qt.PenStyle.DashLine))
                        
                        logger.info(f"Plotted calibration curve for CH{channel}")
                    else:
                        logger.warning(f"Could not generate calibration curve for CH{channel}")
                        
                except Exception as e:
                    logger.error(f"Error plotting calibration curve for CH{channel}: {e}")
...
class App(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.global_calibration_system = MultivariateBayesianCalibration(num_sensors=16, order=8)
        ...
        if ROBUSTNESS_AVAILABLE:
            ...
            logger.info(f"🛡️  Robustness Manager active: mode={self.robustness_manager.config.mode.value}")
                ...
                if recovered_data and 'population_prior' in recovered_data:
                    ...
                    try:
                        ...
                        if 'population_mean' in pp:
                            self.global_calibration_system.population_mean = np.array(pp['population_mean'])
                        if 'population_covariance' in pp:
                            self.global_calibration_system.population_covariance = np.array(pp['population_covariance'])
                        if 'population_strength' in pp:
                            self.global_calibration_system.population_strength = float(pp['population_strength'])
        ...
        if AUTONOMOUS_LEARNING_AVAILABLE:
            ...
            if os.path.exists("learned_prior.json"):
                try:
                    ...
                    logger.info("✅ Loaded previously learned prior from autonomous learning")
                except Exception as e:
                    logger.warning(f"Failed to load learned prior: {e}")
            
            logger.info("🤖 Autonomous Learning Engine active: system will evolve priors automatically")
        else:
        ...
        self.calibration_worker = CalibrationWorkerThread(self.global_calibration_system)
    ...
    def _on_sample(self, t_wall, rec):
        ...
        predicted_pressure, uncertainty = framework.predict_pressure_with_uncertainty(filtered_volts, env_state)
        ...
        if framework.autonomy_level > 0.8 and framework.autonomy_level < 0.81:
            logger.info(f"PT{ch} achieving high autonomy: {framework.autonomy_level:.2f}, "
    ...
    def _on_consensus_ready(self, consensus: ConsensusState):
        ...
        if self.global_calibration_system.consensus_confidence > 0.7:
            ...
            if auto_pressure is not None:
                ...
                self.global_calibration_system.self_calibrate_with_confidence(voltages_copy, env_state)
    ...
    def _on_calibration_complete(self, sensor_id: int, result):
        ...
        logger.info(f"PT{sensor_id} calibration complete - autonomy: {framework.autonomy_level:.2f}, "
                   f"pop_strength: {self.global_calibration_system.population_strength:.2f}")
    
    def _auto_calibrate_from_consensus(self, consensus_pressure: float, env_state: EnvironmentalState):
        """
        ...
        for sensor_id, voltage in voltages_copy.items():
            ...
            logger.info(f"AUTO-CALIBRATING PT{sensor_id} from consensus: {consensus_pressure:.1f} PSI at {voltage:.3f}V")
    ...
    def _find_best_reference_pt(self, current_pt: int) -> Optional[int]:
        ...
        for pt_id, framework in self.calibration_frameworks.items():
            ...
            if framework.coefficients is not None and len(framework.calibration_points) >= 3:
    ...
    def _save_all_calibration_states(self):
        ...
        logger.info(f"Saved population prior (strength={self.global_calibration_system.population_strength:.2f})")
        
        # ROBUSTNESS: Automatic backup if enabled
        if self.robustness_manager and self.robustness_manager.backup_manager.should_backup():
            try:
                # Prepare backup data
                population_prior = {
                    ...
                    'population_strength': float(self.global_calibration_system.population_strength)
                }
                ...
                for pt_id in range(16):
                    pt_states[pt_id] = {
                        ...
                        'covariance': self.global_calibration_system.pt_covariances[pt_id].tolist(),
                        'num_points': len(self.global_calibration_system.calibration_points[pt_id])
                    }
                ...
                success = self.robustness_manager.backup_manager.backup_calibration_state(
    ...
    def closeEvent(self, event: QtGui.QCloseEvent):
        try:
            ...
            self.calibration_worker.wait(2000)  # Give more time for calibration to finish
