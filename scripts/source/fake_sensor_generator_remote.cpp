...
#include "../../daq_comms/include/comms/BarometerMessage.hpp"
#include "../../daq_comms/include/comms/GPSMessage.hpp"
#include "../../daq_comms/include/comms/IMUMessage.hpp"
#include "../../daq_comms/include/comms/PTMessage.hpp"
#include "../../daq_comms/include/comms/RTDMessage.hpp"
#include "../../daq_comms/include/comms/TCMessage.hpp"
...
bool refreshConnection() {
    ...
    try {
        ...
        auto new_socket =
            std::make_unique<Socket>(groundstation_ip_global.c_str(), groundstation_port_global);
        ...
        std::cout << "✅ Database connection refreshed successfully! (Refresh #"
                  << connection_refreshes << ")" << std::endl;
        return true;
        ...
        try {
            ...
            if (total_packets_sent % 50 == 0) {
                ...
                std::cout << "📊 [" << elapsed << "s] Total packets: " << total_packets_sent
                          << ", Errors: " << total_write_errors
                          << ", Refreshes: " << connection_refreshes << std::endl;
            }

            std::cout << "PT: P=" << pressure << " Pa, T=" << temperature << " C [SENT #"
                      << total_packets_sent << "]" << std::endl;
        } catch (const std::exception& e) {
            total_write_errors++;
...
void generateRTDData() {
    ...
    while (running) {
        ...
        std::cout << "RTD: T=" << temperature << " C, R=" << resistance << " Ohm [SENT]"
                  << std::endl;
...
void generateIMUData() {
    ...
    while (running) {
        ...
        std::array<double, 3> accel = {
            0.0 + accel_dist(gen) + noise_dist(gen), 0.0 + accel_dist(gen) + noise_dist(gen),
            9.81 + accel_dist(gen) + noise_dist(gen)  // Gravity
        };
        std::array<double, 3> gyro = {gyro_dist(gen) + noise_dist(gen),
        ...
        std::cout << "IMU: Accel=[" << accel[0] << ", " << accel[1] << ", " << accel[2]
                  << "], Gyro=[" << gyro[0] << ", " << gyro[1] << ", " << gyro[2] << "] [SENT]"
                  << std::endl;
...
void generateGPSData() {
    ...
    while (running) {
        ...
        set_gps_position_measurement(gps_pos_msg, time_gps, 1, latitude, longitude, altitude, 2.5,
                                     3.0, 8, time_ns);
        ...
        set_gps_velocity_measurement(gps_vel_msg, time_gps, velocity_x, velocity_y, velocity_z, 0.5,
                                     time_ns);
        ...
        std::cout << "GPS: Lat=" << latitude << ", Lon=" << longitude << ", Alt=" << altitude
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "   Groundstation: " << groundstation_ip_global << ":" << groundstation_port_global
              << std::endl;
    ...
    std::cout << "✅ Connected to groundstation database. Starting fake sensor generators..."
              << std::endl;
    ...
    std::thread heartbeat_thread([]() {
        ...
        while (running) {
            ...
            if (elapsed > 0 && elapsed % 70 == 0) {
                std::cout << "🔄 [" << elapsed
                          << "s] Connection refresh time! Refreshing database connection..."
                          << std::endl;
                if (refreshConnection()) {
                    std::cout << "✅ [" << elapsed << "s] Connection refreshed successfully!"
                              << std::endl;
                } else {
            ...
            if (current_count == last_count && elapsed > 10) {
                std::cerr << "⚠️  [" << elapsed << "s] WARNING: No packets sent in last 10 seconds! "
                          << "Total: " << current_count << ", Errors: " << total_write_errors
                          << std::endl;
            } else {
                std::cout << "💓 [" << elapsed << "s] Heartbeat: " << current_count
                          << " packets sent, " << (current_count - last_count) << " in last 10s"
                          << std::endl;
            }
