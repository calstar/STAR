...
import time
import logging
from scripts.channel_plotter import MultivariateBayesianCalibration, CalibrationPoint, EnvironmentalState
...
def test_human_input_system():
    ...
    logger.info("\n--- Final Results ---")
    logger.info(f"PT0 - Autonomy: {system.autonomy_levels[0]:.3f}, Confidence: {system.confidence_scores[0]:.3f}")
    logger.info(f"PT1 - Autonomy: {system.autonomy_levels[1]:.3f}, Confidence: {system.confidence_scores[1]:.3f}")
    ...
    logger.info(f"\nExpected vs Actual:")
    logger.info(f"PT0 Autonomy: Expected ~{expected_pt0_autonomy:.1f}, Actual {system.autonomy_levels[0]:.3f}")
    logger.info(f"PT0 Confidence: Expected ~{expected_pt0_confidence:.1f}, Actual {system.confidence_scores[0]:.3f}")
    logger.info(f"Consensus Confidence: Expected ~{expected_consensus:.1f}, Actual {system.consensus_confidence:.3f}")
    ...
    if system.consensus_confidence < 0.4:
        logger.error(f"Consensus confidence too low: {system.consensus_confidence:.3f} < 0.4")
        success = False
    else:
        logger.info(f"✅ Consensus confidence good: {system.consensus_confidence:.3f}")
    
    if system.population_strength < 0.8:
        logger.error(f"Population strength too low: {system.population_strength:.3f} < 0.8")
        success = False
    else:
        logger.info(f"✅ Population strength good: {system.population_strength:.3f}")
    
    if success:
        logger.info("✅ All tests passed! Human inputs properly treated as ground truth.")
    else:
