...
(cd "$REPO_ROOT/web-gui/frontend" && npm install)
...
mkdir -p scripts/calibration/calibrations
mkdir -p scripts/calibration/calibrations/pt
mkdir -p scripts/calibration/calibrations/tc
mkdir -p scripts/calibration/calibrations/rtd
mkdir -p scripts/calibration/calibrations/lc
...
Or for minimal (DB + DAQ):
  ...
  ${CYAN}./build/FSW/daq_bridge config/config.toml${NC}
