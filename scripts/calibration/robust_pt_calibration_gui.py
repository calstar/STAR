...
class BayesianPTCalibration:
    ...
    def add_calibration_point(self, voltage, reference_pressure, env_state):
        ...
        self.calibration_data.append((voltage, reference_pressure, env_state, time.time()))
    ...
    def _update_calibration(self):
        ...
        try:
            ...
            self.theta = self.Sigma_theta @ (Sigma_prior_inv @ self.theta + Phi.T @ R_inv @ pressures)
...
class CalibrationManager:
    ...
    def save_calibrations(self):
        ...
        data = {
            ...
            'calibrations': {
                str(sid): cal.to_dict() 
                for sid, cal in self.calibrations.items()
            }
        }
    ...
    def load_calibrations(self):
        ...
        try:
            ...
            print(f"✅ Loaded {len(self.calibrations)} calibrations from {CALIBRATION_FILE}")
...
class SerialReader(QtCore.QThread):
    sample = QtCore.pyqtSignal(float, int, int, float, int, int)  # t_wall, ch, raw, volts, read_us, conv_us
    ...
    def _resync(self):
        ...
        while True:
            ...
            try:
                magic, version, flags, count, _, _, _ = HEADER_STRUCT.unpack_from(self.buf, 0)
            except struct.error:
                return False
    ...
    def _drain_synced(self):
        ...
        while True:
            ...
            for i in range(count):
                ...
                out.append((time.monotonic(), int(ch), int(raw), float(volts), int(read_dur), int(conv_dur)))
...
class RobustPTCalibrationGUI(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        # Data storage
        self.t0 = None
        self.channel_data = defaultdict(lambda: {'t': deque(maxlen=1000), 'v': deque(maxlen=1000), 'raw': deque(maxlen=1000)})
    ...
    def _add_channel(self, ch):
        ...
        # Add to plot
        color = [(255, 0, 0), (0, 255, 0), (0, 0, 255), (255, 165, 0), (128, 0, 128), 
                 (0, 255, 255), (255, 192, 203), (128, 128, 0), (0, 128, 128), (255, 255, 0)][ch % 10]
        ...
        if sensor_text:
            ...
            if ch in self.channel_data and self.channel_data[ch]['v']:
                ...
                if cal.human_input_count >= 2:
                    ...
                    self.lbl_prediction.setText("Prediction: Need ≥2 calibration points")
