...
try:
    from calibration_robustness import RobustnessManager, SystemConfig, OperationMode
...
def check_system_health() -> bool:
    ...
    if ROBUSTNESS_AVAILABLE:
        ...
        logger.warning(
            "⚠️  Robustness module not available (system will run with limited features)"
        )
...
def main():
    ...
    if config:
        ...
        print(
            f"Health Monitoring: {'ENABLED' if config.health_monitoring_enabled else 'DISABLED'}"
        )
