...
class SystemConfig:
    ...
    def load(cls, filepath: str = "system_config.json") -> 'SystemConfig':
        ...
        try:
            ...
            logger.info(f"System configuration loaded from {filepath}: mode={config.mode.value}")
            return config
...
class BackupManager:
    ...
    def should_backup(self) -> bool:
        ...
        if not self.config.auto_backup_enabled:
            return False
        return (time.time() - self.last_backup_time) >= self.config.backup_interval_seconds
    
    def backup_calibration_state(self, 
    ...
    def restore_from_backup(self, backup_file: Path) -> Optional[Dict]:
        ...
        try:
            ...
            if 'population_prior' in data:
                ...
                if 'population_covariance' in pp and isinstance(pp['population_covariance'], list):
...
class CalibrationValidator:
    ...
    def validate_voltage(self, voltage: float, sensor_id: int) -> ValidationResult:
        ...
        if not np.isfinite(voltage):
            return ValidationResult(False, f"PT{sensor_id}: Non-finite voltage {voltage}", "error")
    ...
    def validate_pressure(self, pressure: float, sensor_id: int) -> ValidationResult:
        ...
        if not np.isfinite(pressure):
            return ValidationResult(False, f"PT{sensor_id}: Non-finite pressure {pressure}", "error")
    ...
    def validate_uncertainty(self, uncertainty: float, sensor_id: int) -> ValidationResult:
        """Validate uncertainty value"""
        if not self.config.validation_enabled:
            return ValidationResult(True)
        
        if not np.isfinite(uncertainty) or uncertainty < 0:
            return ValidationResult(False, f"PT{sensor_id}: Invalid uncertainty {uncertainty}", "error")
    ...
    def validate_calibration_coefficients(self, coefficients: np.ndarray, sensor_id: int) -> ValidationResult:
        """Validate calibration coefficients"""
        if not self.config.validation_enabled:
            return ValidationResult(True)
        
        if coefficients is None:
            return ValidationResult(False, f"PT{sensor_id}: Coefficients are None", "error")
        ...
        if not np.all(np.isfinite(coefficients)):
            return ValidationResult(False, f"PT{sensor_id}: Non-finite coefficients", "error")
    ...
    def validate_covariance_matrix(self, covariance: np.ndarray, sensor_id: int) -> ValidationResult:
        """Validate covariance matrix"""
        if not self.config.validation_enabled:
            return ValidationResult(True)
        
        if covariance is None:
            return ValidationResult(False, f"PT{sensor_id}: Covariance is None", "error")
        ...
        if not np.all(np.isfinite(covariance)):
            return ValidationResult(False, f"PT{sensor_id}: Non-finite covariance", "error")
        ...
        try:
            ...
            return ValidationResult(False, f"PT{sensor_id}: Covariance matrix computation failed", "error")
        
        return ValidationResult(True)
    
    def validate_consensus_state(self, consensus_pressure: float, 
                                 agreement_score: float,
                                 num_sensors: int) -> ValidationResult:
        ...
        if not (0 <= agreement_score <= 1):
            return ValidationResult(False, f"Agreement score {agreement_score} out of [0,1]", "warning")
...
class HealthMonitor:
    ...
    def is_healthy(self) -> bool:
        ...
        if latest.max_uncertainty > self.config.max_uncertainty:
            logger.warning(f"Maximum uncertainty {latest.max_uncertainty} exceeds limit")
            return False
...
class AnomalyDetector:
    ...
    def __init__(self, config: SystemConfig):
        self.config = config
        self.voltage_history: Dict[int, deque] = {i: deque(maxlen=100) for i in range(config.num_sensors)}
        self.pressure_history: Dict[int, deque] = {i: deque(maxlen=100) for i in range(config.num_sensors)}
    ...
    def detect_voltage_anomaly(self, sensor_id: int, voltage: float) -> bool:
        ...
        if z_score > 3.0:
            ...
            logger.warning(f"PT{sensor_id} voltage anomaly: {voltage:.3f}V (z-score: {z_score:.2f})")
            return True
    ...
    def detect_pressure_anomaly(self, sensor_id: int, pressure: float) -> bool:
        ...
        if len(history) >= 2:
            ...
            if rate_of_change > 100.0:  # > 100 PSI change in one sample
                logger.warning(f"PT{sensor_id} rapid pressure change: {rate_of_change:.1f} PSI/sample")
                return True
...
class RobustnessManager:
    ...
    def __init__(self, config: Optional[SystemConfig] = None):
        ...
        logger.info(f"🛡️  Robustness Manager initialized: mode={self.config.mode.value}, "
    ...
    def set_mode(self, mode: OperationMode):
        ...
        if mode == OperationMode.FLIGHT:
            ...
            logger.warning("⚠️  FLIGHT MODE: Consensus disabled, measurements are independent")
