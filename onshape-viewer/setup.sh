#!/usr/bin/env bash
# onshape-viewer/setup.sh — per-project setup for the Onshape CM Viewer.
#
# Standalone: `bash onshape-viewer/setup.sh` from the repo root prepares both
# ends of the app. Also invoked by top-level `setup.sh --onshape-viewer`.
#
# What it installs:
#   1. Python venv at onshape-viewer/.venv + pip install -r requirements.txt
...
OS="$(detect_os)"
step "onshape-viewer setup — OS: $OS"
...
# ─── 2. Python venv ──────────────────────────────────────────────────────────
step "Python venv (onshape-viewer/.venv)"
...
if [ "$DO_FRONTEND" = "1" ]; then
  ...
  if [ -d "$VIEWER_DIR/frontend" ]; then
    ...
    warn "onshape-viewer/frontend not found — skipping"
  fi
fi
...
# comment block at the top of .github/workflows/onshape-viewer-ci.yml.
if [ "$CI_MODE" = "0" ]; then
...
EOF
    ...
    warn "Fill in onshape-viewer/.env before building a model."
  fi
fi

step "onshape-viewer setup complete"

cat <<EOF

Next steps:

  1. Put your Onshape key pair in onshape-viewer/.env
