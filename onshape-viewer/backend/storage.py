"""Version storage for recovery configs: microversions + immutable releases.

Ported from pid-designer/backend/storage.py -- same two-backend design, same
methods. The payload here is a recovery config blob (``{"config": <UiConfig>}``),
but this module is payload-agnostic: it stores and returns whatever JSON dict it
is given. The *working copy* (``current.json`` on the volume) is owned by the
router (backend/routers/documents.py); here we only keep version history.

Two backends, chosen at import time by environment:

* **S3Backend** when ``ONSHAPE_VIEWER_S3_BUCKET`` is set. Microversions ride on S3
...
class LocalBackend:
    """Filesystem version history under ``<root>/<user>/recovery/<id>/``."""

    def _doc_dir(self, user: str, doc_id: str) -> Path:
        return userdata.user_dir(user) / doc_id
    ...
    def delete_doc(self, user: str, doc_id: str) -> None:
        shutil.rmtree(self._doc_dir(user, doc_id), ignore_errors=True)
...
class S3Backend:
    ...
    def delete_doc(self, user: str, doc_id: str) -> None:
        # Best-effort: remove every version of every object under the document.
        prefix = f"{self.prefix}/{user}/{doc_id}/"
...
def _make_backend():
    bucket = os.environ.get("ONSHAPE_VIEWER_S3_BUCKET")
    if bucket:
        return S3Backend(bucket, os.environ.get("ONSHAPE_VIEWER_S3_PREFIX", "viewer"))
    return LocalBackend()
