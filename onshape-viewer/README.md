...
```bash
# From the repo root
./setup.sh --onshape-viewer      # or: bash onshape-viewer/setup.sh
```

Then put an Onshape key pair (https://dev-portal.onshape.com) into the
gitignored `onshape-viewer/.env`:
...
[`.github/workflows/onshape-viewer-ci.yml`](../.github/workflows/onshape-viewer-ci.yml)
runs on changes under `onshape-viewer/`:
