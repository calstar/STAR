...
int main() {
    ...
    for (int i = 0; i < repeats; ++i) {
        ...
        double check = 0.0;
        for (size_t i = 0; i < out_width * out_height; i++) check += out[i];
        sink = check;        // observable side effect
    }
    double avg_nanoseconds = total_nanoseconds / repeats;
    std::cout << "matmul(" << out_height << "x" << out_width << " * " << out_height << "x" << out_width << "): " << avg_nanoseconds << " nanoseconds (avg of " << repeats / 1000 << "k runs)\n";
    ...
    if (std::all_of(out, out + 4, [](double v) { return std::fabs(v - 3.0) < 1e-9; })) {
