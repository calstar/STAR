...
function buildActuatorChannelMap() {
    const byName = {};
    try {
        const config = readConfig();
...
function buildActuatorEntityMap() {
    const map = {};
    try {
        const config = readConfig();
        const roles = config.actuator_roles || {};
...
export function parseStateActuatorsCSV(csvPath) {
    const result = {};
    try {
        // Load actuator channel mappings and role names from config.toml (UI is driven by config roles)
        let configActuatorChannels = {};
        let configActuatorNames = new Set();
        try {
            const config = readConfig();
...
export function getNumActuatorsFromCSV() {
