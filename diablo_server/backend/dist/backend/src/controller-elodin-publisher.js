...
function appendStateToCsvFallback(timestampMs, toState) {
    try {
        ...
        if (!exists)
            fs.writeFileSync(STATE_CSV, 'timestamp_ms,to_state\n');
        fs.appendFileSync(STATE_CSV, line);
    }
    catch (_) { /* ignore */ }
}
function getPublishTarget(elodin, relay) {
 ...
 */
export function encodeControllerActuation(timestampNs, dutyF, dutyO, uFOn, uOOn, valid) {
 ...
 */
export function encodeControllerDiagnostics(timestampNs, fRef, mrRef, fEstimated, mrEstimated, pCh, cost, safetyFiltered, cutoffActive, solverIters) {
 ...
 */
export function publishControllerActuation(elodin, dutyF, dutyO, uFOn, uOOn, valid) {
 ...
 */
export function publishControllerDiagnostics(elodin, fRef, mrRef, fEstimated, mrEstimated, pCh, cost, safetyFiltered, cutoffActive, solverIters) {
    if (!elodin.isConnected()) {
        return false;
    }
    try {
        const timestampNs = BigInt(Date.now()) * BigInt(1_000_000); // Convert ms to ns
        const payload = encodeControllerDiagnostics(timestampNs, fRef, mrRef, fEstimated, mrEstimated, pCh, cost, safetyFiltered, cutoffActive, solverIters);
 ...
 */
export function publishControllerStateTransition(elodin, fromState, toState, reason = 0, relay = null) {
 ...
 */
export function publishActuatorStateToElodin(elodin, channelId, actuatorState, relay = null) {
    const target = getPublishTarget(elodin, relay);
    if (!target)
        return false;
