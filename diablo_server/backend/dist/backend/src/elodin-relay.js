...
function main() {
            ...
            try {
                ...
                if (msg.type === 'publish' && Array.isArray(msg.packetId) && msg.packetId.length === 2 && typeof msg.payload === 'string') {
                    ...
                    if (elodin.isConnected()) {
                        ...
                        console.log(`[Relay] Publish [0x${msg.packetId[0].toString(16)},0x${msg.packetId[1].toString(16)}] → Elodin: ${ok ? 'OK' : 'FAIL'}`);
                    }
                    else {
    ...
    elodin.on('packet', (header, payload) => {
        ...
        wss.clients.forEach((client) => {
            if (client.readyState === 1) {
                try {
                    client.send(broadcastBuffer);
                }
                catch (_) { /* ignore closed client */ }
            }
        });
        tcpClients.forEach((sock) => {
            if (sock.writable) {
                try {
                    sock.write(broadcastBuffer);
                }
                catch (_) { /* ignore */ }
            }
        });
    ...
    elodin.on('connected', () => {
        ...
        registerControllerVTables(elodin).then((ok) => {
            if (ok)
                console.log('[Relay] Controller VTables registered (CONTROLLER.state etc.)');
        ...
        registerActuatorCommandedVTables(elodin, actuatorMap).then((ok) => {
            if (ok)
                console.log('[Relay] Actuator Commanded VTables registered');
