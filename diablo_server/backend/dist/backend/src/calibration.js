 ...
 */
export function validateCalibrationCoefficients(coeffs, typicalAdcRange = [0, 4000000000]) {
 ...
 */
export function calculatePressure(adcCode, coeffs, _env // Reserved for future use (robust calibration)
) {
    ...
    const result = polyCoeffs != null && polyCoeffs.length > 0
 ...
 */
export function inversePressureToAdc(targetPsi, coeffs) {
    const { A, B, C, D } = coeffs;
    const f = (x) => A * (x ** 3) + B * (x ** 2) + C * x + D - targetPsi;
    let lo = 0;
    let hi = MAX_ADC;
    const fLo = f(lo);
    const fHi = f(hi);
 ...
 */
function loadCalibrationJSON(jsonPath) {
    ...
    try {
        ...
        const polyCoeffsMap = data.calibration_poly_coeffs;
        const adcNormMinMap = data.calibration_adc_norm_min;
        const adcNormScaleMap = data.calibration_adc_norm_scale;
        if (data.calibration_polynomials) {
            for (const [sensorIdStr, coeffs] of Object.entries(data.calibration_polynomials)) {
                ...
                if (Array.isArray(poly) && poly.length > 0 && poly.some(c => c !== 0)) {
                    const entry = { A: 0, B: 0, C: 0, D: 0, polyCoeffs: poly };
                    if (adcNormMinMap?.[sensorIdStr] != null)
                        entry.adcNormMin = adcNormMinMap[sensorIdStr];
                    if (adcNormScaleMap?.[sensorIdStr] != null)
                        entry.adcNormScale = adcNormScaleMap[sensorIdStr];
                    calMap.set(sensorId, entry);
                }
                else if (Array.isArray(coeffs) && coeffs.length >= 4 && coeffs.some(c => c !== 0)) {
                    const entry = { A: coeffs[0], B: coeffs[1], C: coeffs[2], D: coeffs[3] };
                    if (adcNormMinMap?.[sensorIdStr] != null)
                        entry.adcNormMin = adcNormMinMap[sensorIdStr];
                    if (adcNormScaleMap?.[sensorIdStr] != null)
                        entry.adcNormScale = adcNormScaleMap[sensorIdStr];
                    ...
                    if (Array.isArray(fallbackPoly) && fallbackPoly.length > 0 && fallbackPoly.some(c => c !== 0)) {
                        const entry = { A: 0, B: 0, C: 0, D: 0, polyCoeffs: fallbackPoly };
                        if (adcNormMinMap?.[sensorIdStr] != null)
                            entry.adcNormMin = adcNormMinMap[sensorIdStr];
                        if (adcNormScaleMap?.[sensorIdStr] != null)
                            entry.adcNormScale = adcNormScaleMap[sensorIdStr];
...
function getCoeffVector(c) {
    if (c.polyCoeffs && c.polyCoeffs.length >= 4)
        return c.polyCoeffs.slice(0, 4);
 ...
 */
function fillUncalibratedChannelsFromAgreeing(calMap) {
    ...
    if (vectors.length < MIN_AGREEING)
        return;
    ...
    for (const channelId of toFill) {
        ...
        if (C <= 0)
            C = mean[2]; // keep positive slope for sensible PT
...
export function loadPTCalibration(overridePath) {
    ...
    for (const calibrationDir of candidateDirs) {
        ...
        const latest = jsonFiles.sort((a, b) => fs.statSync(b).mtimeMs - fs.statSync(a).mtimeMs)[0];
