...
struct CommsMessage {
    ...
    void serialize(uint8_t* buffer) const {
        std::size_t offset = 0;
        std::apply([&](const auto&... field_vals) {
...
private:
    ...
    void serialize_field(uint8_t* buffer, std::size_t& offset, const T& field) const {
        if constexpr (is_comms_message_v<T>) {
            // Nested CommsMessage - flatten by writing each field individually (matching FSW's Serializer::write)
            std::apply([&](const auto&... nested_fields) {
                ((serialize_field(buffer, offset, nested_fields)), ...);
            }, field.fields);
        } else if constexpr (is_std_array_v<T>) {
            // std::array - write each element individually (matching FSW's Serializer::write for std::array)
            for (const auto& item : field) {
                serialize_field(buffer, offset, item);
            }
        } else {
            // Primitive type - memcpy directly (matching FSW's Serializer::write for TrivialSerializable)
