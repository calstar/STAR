...
Configs are shared: one lives in its creator's folder but is editable by anyone
...
def test_delete_is_gone(client):
    ...
    assert deletes == {
        "/api/recovery/documents/{doc_id}/share/me",
        # Releasing a checkout is the other DELETE, and it drops a write token,
        # not a config.
        "/api/recovery/documents/{doc_id}/checkout",
    }
...
def test_history_needs_a_share(client):
    ...
    assert client.get(f"/api/recovery/documents/{a_id}/history", headers=B).status_code == 404
    assert client.get(f"/api/recovery/documents/{a_id}/history",
...
def test_history_readable_once_shared(client):
    ...
    _share(client, a_id, [B["X-Auth-Email"]])
    history = client.get(f"/api/recovery/documents/{a_id}/history",
...
def test_share_grants_edit_and_writes_into_the_owners_folder(client, tmp_path):
    ...
    assert (tmp_path / "alice@berkeley.edu" / "recovery" / a_id / "current.json").is_file()
    assert not (tmp_path / "bob@berkeley.edu" / "recovery" / a_id).exists()
...
def test_legacy_records_without_sharedwith_still_work(client, tmp_path):
    ...
    index = tmp_path / "alice@berkeley.edu" / "recovery" / "index.json"
