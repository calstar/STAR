...
Sharing
-------
A config lives in its creator's folder, but that is only *where it lives* -- it
is not a privilege level. The creator and everyone in the record's ``sharedWith``
list are equally editors; there is no owner-only operation. Any config can also
be read and copied by anyone, which is what the view-only tree in the UI browses.

Every config-scoped handler resolves through :func:`_resolve_doc`, which is the
...
There is deliberately no delete endpoint: a config is editable by more than
...
router = APIRouter(prefix="/api/recovery/documents", tags=["documents"])
...
MICRO_INTERVAL = int(os.environ.get("RECOVERY_MICRO_INTERVAL", "300"))
...
# Keyed on the config's *owner*, not the editor: two people editing one shared
# config must share a throttle clock, or it snapshots at twice the intended rate.
...
# ── sharing: who may edit which config ───────────────────────────────────────

@dataclass(frozen=True)
class DocRef:
    """A config the caller is allowed to act on.

    ``owner`` is the folder the config lives in -- not a privilege level. It only
    differs from ``viewer`` when someone is editing a config shared with them.
    """
...
def _shared_with(record: dict) -> list[str]:
    """The record's editor list. Absent on every config created before sharing
...
def _resolve_doc(
    ...
    *,
    ...
    """Resolve ``(?owner=, doc_id)`` into a config the caller may act on.
...
def _scan_all() -> Iterator[tuple[str, dict]]:
    """``(owner, record)`` for every config on the shared volume.
...
async def list_documents(request: Request):
    """Configs the caller may edit -- their own plus any shared with them."""
    viewer = userdata.current_user(request)
...
async def browse_documents(request: Request):
    ...
    Configs the caller can already edit are left out: they are in the editable
...
def _create(user: str, name: str, data: dict) -> dict:
    """Add a config to ``user``'s index and seed its working copy."""
    now = _now_iso()
    with _index_lock(user):
...
async def create_document(request: Request, payload: CreatePayload):
    ...
    return _create(user, payload.name.strip() or "Untitled", {"config": payload.config or {}})


@router.post("/copy")
async def copy_document(request: Request, payload: CopyPayload):
    """Copy any config -- yours or anyone else's -- into the caller's own list.

    Deliberately not gated on sharing: every config is viewable and copyable by
    anyone, which is what the view-only tree offers. What a copy does *not*
    inherit is history, releases, or the share list. It is a new config that
    ...
    if data is None:
        data = storage.backend.latest_micro(ref.owner, ref.doc_id) or {"config": {}}
    names = directory.display_names(request)
...
async def share_document(
    ...
    """Replace the config's editor list.
    ...
    New -- and a removed editor can still read and copy the config anyway, so
...
@router.delete("/{doc_id}/share/me")
async def leave_document(request: Request, doc_id: str, owner: str | None = None):
    """Remove yourself from a config someone shared with you.

    With delete gone this is the only thing that shrinks your list, and it
    destroys nothing: the config stays exactly where it is and you can copy it
    back out of the view-only tree whenever you like. The owner cannot leave --
    the config would end up in nobody's editable list.
    """
    ref = _resolve_doc(request, owner, doc_id)
    if ref.owner == ref.viewer:
        raise HTTPException(
            status_code=400,
            detail="You own this config, so you cannot leave it.",
        )
    remaining = [
...
async def autosave_document(
    request: Request, doc_id: str, payload: ConfigPayload, owner: str | None = None
):
    """Write the working copy; snapshot a microversion only once per MICRO_INTERVAL.

    A config shared with several people is last-write-wins here, on purpose --
    checkouts are the next piece of work. A client that is autosaving a config
...
async def create_release(
    ...
    if payload.config is not None:
        ...
        data = _read_working(ref.owner, ref.doc_id) or {"config": {}}
