...
if __name__ == "__main__" and __package__ is None:
    # Allow `python3 pad_state.py --elev 610` from inside physics/, not
    # just `python3 -m physics.pad_state`. The CLI is the point of this
    # module on launch day; requiring -m would be a regression.
    sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

from physics.atmosphere import geopotential, refit_lapse  # noqa: E402,F401
...
# Names kept for the CLI's own use and for site-climatology, which imports
# this module rather than keeping a second copy of §5.
L0 = L0_ISA
INHG = INHG_TO_PA
EXP = -G0 / (RD * L0_ISA)  # 5.255877, the layer-0 pressure exponent
...
# --- atmosphere ------------------------------------------------------------
#
# Eqs (1), (5) and (7) live in atmosphere.py and are imported above. Only the
# two pad-pressure inversions are here, because they are about decoding a
# *reported* number rather than about the profile.
...
def column_ratio(H):
    ...
    would decode with a different function than the station encoded with, and
    would *introduce* an error where none existed.
    """
    return (1.0 + L0_ISA * H / T0) ** EXP
...
def density(p, T):
    """Eq (5), dry air. Takes p and T directly; Atmosphere.rho takes altitude."""
    return p / (RD * T)
...
def fetch_metar(station, hours=3.0, timeout=30.0):
    ...
    `timeout` is generous by default because a CLI run has nobody waiting on
    it. A request handler does: the browser gives up first and reports a
    frontend timeout, which blames the wrong service. Callers with a deadline
    of their own must pass one shorter than it.
    """
