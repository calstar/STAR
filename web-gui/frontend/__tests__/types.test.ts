...
describe('ActuatorId enum', () => {
  it('should have correct integer values', () => {
    expect(ActuatorId.LOX_MAIN).toBe(0);
    expect(ActuatorId.FUEL_MAIN).toBe(1);
    expect(ActuatorId.LOX_VENT).toBe(2);
    expect(ActuatorId.FUEL_VENT).toBe(3);
    expect(ActuatorId.LOX_PRESS).toBe(4);
    expect(ActuatorId.FUEL_PRESS).toBe(5);
    expect(ActuatorId.GSE_LOW_VENT).toBe(6);
    expect(ActuatorId.FUEL_FILL_VENT).toBe(7);
    expect(ActuatorId.FUEL_FILL_PRESS).toBe(8);
    expect(ActuatorId.LOX_FILL).toBe(9);
    expect(ActuatorId.LOX_DUMP).toBe(10);
  });
});
