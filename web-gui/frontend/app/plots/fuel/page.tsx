...
export default function FuelGraphsPage() {
  ...
  const fuelMainAdc = useSensorValue('ACT.ACT_CH7', 'raw_adc_counts');
  const fuelMainOpen = (fuelMainAdc ?? 0) > 0;
  ...
  return (
    <main className="h-full bg-background text-text flex flex-col overflow-hidden p-3 gap-2">
      ...
      <div className="flex items-center justify-between flex-shrink-0">
        ...
        <span className={`text-xs font-mono font-bold px-2 py-0.5 rounded ${fuelMainOpen ? 'bg-green-900/50 text-green-400' : 'bg-red-900/50 text-red-400'}`}>
          FUEL MAIN: {fuelMainOpen ? 'OPEN' : 'CLOSED'}
        </span>
      </div>
