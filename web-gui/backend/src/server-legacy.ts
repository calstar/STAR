/**
 * DEPRECATED — server-legacy.ts (formerly server.ts)
 *
 * Monolithic WebSocket server that handled everything: Elodin TCP, actuator UDP,
 * calibration, config packets, state machine, etc. Replaced by server.ts (the
 * thin relay) which delegates heavy logic to C++ and Python services.
 *
 * Use server.ts instead. This file is kept for reference and rollback only.
 *
 * To run intentionally:  npx tsx src/server-legacy.ts
 */

console.warn('\n' +
  '╔══════════════════════════════════════════════════════════════╗\n' +
  '║  WARNING: You are running server-legacy.ts                  ║\n' +
  '║  The default server is now server.ts (thin relay).          ║\n' +
  '║  This legacy server is kept for reference/rollback only.    ║\n' +
