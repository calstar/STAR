...
export async function registerVTables(client: ElodinClient): Promise<boolean> {
  ...
  try {
    ...
    const subscriptions: Array<[number, number]> = [
      ...
      // PT Calibrated (0x20, 0x11-0x1E) NOT subscribed — backend derives pressure_psi from raw
      // only. Subscribing to both caused duplicate/zero updates and value/0/value/0 in plots.
      // TC Raw (0x21, 0x01–0x14) / Cal (0x21, 0x11–0x24) — include CH5+ so config active_connectors [2,3,4,5] etc. get data
      ...
      // LC Raw/Cal 1–4
      [0x23, 0x01], [0x23, 0x02], [0x23, 0x03], [0x23, 0x04],
      [0x23, 0x11], [0x23, 0x12], [0x23, 0x13], [0x23, 0x14],
