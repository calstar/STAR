"""Per-user engine designs with microversion history + named releases.

Ported from pid-designer/backend/routers/pid.py. A pid *diagram* is an engine
*design* here; the payload is a config blob (``{"config": <dict>}``) instead of
``{nodes, edges}``. Three tiers of durability:
...
Every design-scoped handler resolves through :func:`_resolve_doc`, which is the
...
router = APIRouter(prefix="/api/engine/documents", tags=["documents"])
...
# Keyed on the design's *owner*, not the editor: two people editing one shared
# design must share a throttle clock, or it snapshots at twice the intended rate.
...
def _unique_id(user: str, name: str) -> str:
    base = userdata.slugify(name) or "design"
