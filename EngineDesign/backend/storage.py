...
methods. The payload here is an engine design blob (``{"config": <dict>}``), but
this module is payload-agnostic: it stores and returns whatever JSON dict it is
given. The *working copy* (``current.json`` on the volume) is owned by the
router (backend/routers/documents.py); here we only keep version history.

Two backends, chosen at import time by environment:

* **S3Backend** when ``ENGINE_S3_BUCKET`` is set. Microversions ride on S3 object
  *versioning* of a per-document HEAD key (``<prefix>/<user>/<id>/current.json``):
...
class LocalBackend:
    """Filesystem version history under ``<root>/<user>/engine/<id>/``."""
...
def _make_backend():
    bucket = os.environ.get("ENGINE_S3_BUCKET")
    if bucket:
        return S3Backend(bucket, os.environ.get("ENGINE_S3_PREFIX", "engine"))
    return LocalBackend()
