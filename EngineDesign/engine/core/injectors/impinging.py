...
def impingement_standoff_m(
    ...
    """Axial distance from the injector FACE to where a doublet's two jets meet [m].

    The two streams of an unlike doublet sit on DIFFERENT pitch circles::

        D_pitch = n * spacing / pi          (spacing = arc between adjacent same-stream holes)
        dr      = |D_pitch_O - D_pitch_F| / 2

    ``dr`` is the RADIAL gap between the two rings at the face. Each jet is inclined toward
    the other and closes its share of that gap at rate ``tan(theta)`` per unit axial
    distance, so they meet at::

        L_imp = dr / (tan(theta_O) + tan(theta_F))

    NOTE the offset is ``dr``, not the element spacing. ``spacing`` is the circumferential
    distance between ADJACENT holes of the SAME stream around its own ring -- it says how
    densely one ring is populated and has nothing to do with how far the O and F jets of a
    single doublet are from each other. An earlier version used ``0.5*(s_O + s_F)`` here,
    which is the average ring density: dimensionally a length, physically the wrong one, and
    it under-reported the standoff (2.89 mm vs 4.35 mm on a real design, i.e. L/d 1.59 vs
    2.39) while also shortening the spray length that feeds the residence-time correction.

    Returns NaN when the rings coincide or both jets are axial -- in either case there is no
    crossing point.
    """
    import numpy as _np

    n = max(1.0, float(n_elements))
    dp_O = n * float(spacing_O_m) / _np.pi
    dp_F = n * float(spacing_F_m) / _np.pi
    dr = 0.5 * abs(dp_O - dp_F)
    tan_sum = float(
        _np.tan(_np.deg2rad(float(angle_O_deg))) + _np.tan(_np.deg2rad(float(angle_F_deg)))
    )
    if not _np.isfinite(tan_sum) or tan_sum <= 1e-9 or dr <= 0.0:
        return float("nan")
    val = dr / tan_sum
