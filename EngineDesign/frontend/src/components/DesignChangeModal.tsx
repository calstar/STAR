...
export function DesignChangeModal({
  ...
  return (
    <Modal open={open} onClose={onClose} title="Designs" width="w-[560px]">
      ...
      {tab === 'editable' ? (
        <div className="max-h-[55vh] overflow-y-auto">
          {creating ? (
            ...
            <button
              ...
              onClick={() => { reset(); setCreating(true); setNewName(`Design ${documents.length + 1}`); }}
            >
              + New design
            </button>
          )}
          ...
          {documents.length === 0 && (
            <p className="py-3 text-xs text-[var(--color-text-muted)]">No designs yet.</p>
          )}
        ...
        <div className="max-h-[55vh] overflow-y-auto">
          ...
          {tree?.length === 0 && (
            <p className="py-3 text-xs text-[var(--color-text-muted)]">Nobody else has designs yet.</p>
          )}
          {tree?.map((g) => (
