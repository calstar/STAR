...
void generatePTData() {
    ...
    while (running) {
        ...
        double daily_temp =
            5.0 * sin(time_s * 2 * M_PI / 86400.0 + M_PI / 4);  // 24-hour cycle with offset
...
void generateIMUData() {
    while (running) {
        ...
        std::array<double, 3> gyro = {rotation_x + imu_drift_x + gyro_noise,
...
void generateGPSData() {
    ...
    while (running) {
        ...
        std::cout << "GPS: Lat=" << latitude << ", Lon=" << longitude << ", Alt=" << altitude
                  << ", Vel=[" << velocity_north << ", " << velocity_east << ", " << velocity_up
                  << "]" << std::endl;
