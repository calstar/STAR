...
ElodinDBInterface::ElodinDBInterface(const ElodinConfig& config)
    ...
    std::cout << "ElodinDBInterface initialized for database: " << config_.database_name
              << std::endl;
}
...
void ElodinDBInterface::streamRawADC(const std::string& sensor_id, int32_t raw_adc,
                                     uint64_t timestamp_ns) {
    ElodinDataPoint point(sensor_id, "raw_adc", static_cast<double>(raw_adc), "counts",
                          timestamp_ns);
...
void ElodinDBInterface::streamVoltage(const std::string& sensor_id, double voltage,
                                      uint64_t timestamp_ns) {
...
void ElodinDBInterface::streamPressure(const std::string& sensor_id, double pressure_pa,
                                       uint64_t timestamp_ns) {
...
void ElodinDBInterface::streamCalibrationParameters(const std::string& sensor_id,
    ...
    for (int i = 0; i < params.theta.size(); ++i) {
        ...
        ElodinDataPoint point(sensor_id, param_name, params.theta(i), "dimensionless",
                              timestamp_ns);
    ...
    for (int i = 0; i < params.covariance.rows(); ++i) {
        ...
        ElodinDataPoint point(sensor_id, var_name, params.covariance(i, i), "dimensionless^2",
                              timestamp_ns);
    ...
    ElodinDataPoint quality_point(sensor_id, "calibration_quality", params.calibration_quality,
                                  "dimensionless", timestamp_ns);
    {
        std::lock_guard<std::mutex> lock(queue_mutex_);
...
void ElodinDBInterface::streamCalibrationConvergence(
    const CalibrationConvergenceData& convergence_data) {
...
void ElodinDBInterface::streamEnvironmentalConditions(const std::string& sensor_id,
    ...
    // Stream vibration
    ElodinDataPoint vibration_point(sensor_id, "vibration_level", env->vibration_level,
                                    "normalized", timestamp_ns);

    // Stream aging factor
    ElodinDataPoint aging_point(sensor_id, "aging_factor", env->aging_factor, "dimensionless",
                                timestamp_ns);

    // Stream mounting torque
    ElodinDataPoint torque_point(sensor_id, "mounting_torque", env->mounting_torque, "Nm",
                                 timestamp_ns);
...
void ElodinDBInterface::streamHumanFeedback(const std::string& sensor_id, double reference_pressure,
...
void ElodinDBInterface::createCalibrationPlot(const std::string& sensor_id,
...
void ElodinDBInterface::createConvergencePlot(const std::string& sensor_id,
                                              const std::string& plot_title) {
...
void ElodinDBInterface::createVariancePlot(const std::string& sensor_id,
                                           const std::string& plot_title) {
...
void ElodinDBInterface::streamingThreadFunction() {
    ...
    while (running_) {
        ...
        if (!batch.empty() ||
            (now - last_flush >= std::chrono::milliseconds(config_.flush_interval_ms))) {
            if (!batch.empty()) {
...
bool ElodinDBInterface::sendBatch(const std::vector<ElodinDataPoint>& batch) {
    ...
    if (success) {
        std::cout << "Successfully sent batch of " << batch.size() << " data points to Elodin DB"
                  << std::endl;
    } else {
        std::cerr << "Failed to send batch of " << batch.size() << " data points to Elodin DB"
                  << std::endl;
    }
...
std::string ElodinDBInterface::convergenceDataToJSON(const CalibrationConvergenceData& data) const {
    ...
    json_stream << "  \"confidence_score\": " << std::fixed << std::setprecision(6)
    ...
    for (int i = 0; i < data.theta_parameters.size(); ++i) {
        ...
        if (i < data.theta_parameters.size() - 1)
            json_stream << ", ";
    }
    json_stream << "],\n";
    ...
    for (int i = 0; i < data.parameter_covariance.rows(); ++i) {
        ...
        if (i < data.parameter_covariance.rows() - 1)
            json_stream << ", ";
    }
    json_stream << "],\n";
...
RealTimeCalibrationMonitor::RealTimeCalibrationMonitor(
    std::shared_ptr<ElodinDBInterface> elodin_interface)
...
void RealTimeCalibrationMonitor::processPTMeasurement(const void* pt_message,
    ...
    std::cout << "Streamed PT measurement for " << sensor_name << ": ADC=" << raw_adc
              << ", Voltage=" << std::fixed << std::setprecision(4) << voltage << "V";
    if (reference_pressure > 0.0) {
        std::cout << ", Pressure=" << std::fixed << std::setprecision(0) << reference_pressure
                  << "Pa";
    }
    std::cout << std::endl;
}
...
void RealTimeCalibrationMonitor::processCalibrationUpdate(const std::string& sensor_id,
    ...
    std::cout << "Streamed calibration update for " << sensor_id << " (iteration " << iteration
              << ")" << std::endl;
}
...
void RealTimeCalibrationMonitor::processHumanFeedback(const std::string& sensor_id,
    ...
    elodin_interface_->streamHumanFeedback(sensor_id, reference_pressure, measured_voltage,
...
std::vector<double> RealTimeCalibrationMonitor::getVarianceTrend(const std::string& sensor_id,
...
double RealTimeCalibrationMonitor::calculateConfidenceScore(
