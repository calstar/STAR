...
class FSWCommunicationProtocol:
    ...
    def __init__(
        ...
        command_port: int = 2241,
        telemetry_port: int = 2242,
    ):
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            print(
                f"✅ Connected to FSW command port: {self.fsw_host}:{self.command_port}"
            )
            ...
            print(
                f"✅ Connected to FSW telemetry port: {self.fsw_host}:{self.telemetry_port}"
            )
    ...
    def _command_loop(self):
        ...
        while self.running:
            try:
                ...
                print(
                    f"📤 Sent command: {command.command_type.name} (ID: {command.command_id})"
                )
                ...
                while len(buffer) >= 8:  # Minimum packet size (header)
                    ...
                    try:
                        ...
                        telemetry = self._parse_telemetry(
                            message_type, payload, sequence_num
                        )
    ...
    def _parse_telemetry(
        self, message_type: MessageType, payload: bytes, sequence_num: int
    ) -> TelemetryData:
...
class GroundStationGUI(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.protocol.register_telemetry_callback(
            MessageType.SENSOR_DATA, self._handle_sensor_data
        )
        self.protocol.register_telemetry_callback(
            MessageType.ENGINE_STATUS, self._handle_engine_status
        )
        self.protocol.register_telemetry_callback(
    ...
    def _create_command_panel(self) -> QtWidgets.QWidget:
        ...
        self.estop_btn.setStyleSheet(
            "background-color: darkred; color: white; font-weight: bold; font-size: 14px;"
        )
        ...
        self.mixture_ratio_slider.valueChanged.connect(
            self._update_mixture_ratio_display
        )
        ...
        self.valve_controls = {}
        for i, valve_name in enumerate(
            ["LOX Main", "Fuel Main", "LOX Purge", "Fuel Purge"]
        ):
