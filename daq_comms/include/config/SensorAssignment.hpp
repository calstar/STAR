...
public:
     ...
     */
    std::string assign_board_ip(uint8_t board_id, const std::string& mac_address, SystemState system_state);
     ...
     */
    bool assign_sensors_to_board(uint8_t board_id, const std::vector<std::string>& sensor_ids, uint8_t start_channel = 0);
     ...
     */
    void register_config_update_callback(std::function<void(uint8_t, const BoardConfiguration&)> callback);
