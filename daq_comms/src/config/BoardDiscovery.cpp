...
bool BoardDiscovery::initialize(const std::string& network_interface,
    ...
    std::cout << "[BoardDiscovery] IP range: " << base_ip_ << "." 
              << (int)ip_range_start_ << "-" << (int)ip_range_end_ << std::endl;
...
void BoardDiscovery::start_discovery(DiscoveryMode mode) {
    ...
    std::cout << "[BoardDiscovery] Starting discovery (mode=" 
              << (mode == DiscoveryMode::PASSIVE ? "PASSIVE" :
                  mode == DiscoveryMode::ACTIVE ? "ACTIVE" : "HYBRID")
              << ")" << std::endl;
...
void BoardDiscovery::process_board_announcement(const uint8_t* data, size_t size, const std::string& source_ip) {
    ...
    signature.board_id = (static_cast<uint32_t>(heartbeat->heartbeat.board_type) << 8) | heartbeat->heartbeat.board_id;
    ...
    mac << std::hex << std::setw(2) << std::setfill('0') << ((ip_hash >> 16) & 0xFF) << ":"
        ...
        << std::hex << std::setw(2) << std::setfill('0') << (ip_hash & 0xFF) << ":"
        << std::hex << std::setw(2) << std::setfill('0') << ((ip_hash >> 24) & 0xFF) << ":"
        << std::hex << std::setw(2) << std::setfill('0') << ((signature.board_id >> 8) & 0xFF) << ":"
    ...
    if (signature_to_ip_.find(signature) == signature_to_ip_.end()) {
        ...
        if (is_ip_available(assigned_ip)) {
            ...
            std::cout << "[BoardDiscovery] Assigned IP " << assigned_ip 
                      << " to " << signature.to_string() << " (MAC: " << board.mac_address << ")" << std::endl;
        }
    }
...
void BoardDiscovery::process_sensor_data(const uint8_t* data, size_t size, const std::string& source_ip) {
    ...
    protocol::DiabloBoardPacketParser::BoardType board_type = protocol::DiabloBoardPacketParser::BoardType::UNKNOWN;
...
std::vector<SensorInfo> BoardDiscovery::detect_sensors_from_packet(const uint8_t* data, size_t size) const {
    std::vector<SensorInfo> sensors;
...
BoardDiscovery::BoardType BoardDiscovery::detect_board_type(const std::vector<SensorInfo>& sensors) const {
    if (sensors.empty()) {
    ...
    if (type_counts.size() == 1) {
        ...
        switch (type_counts.begin()->first) {
            case 1: return BoardType::PT_BOARD;   // PRESSURE_TRANSDUCER = 1
            case 2: return BoardType::LC_BOARD;    // LOAD_CELL = 2
            case 3: return BoardType::RTD_BOARD;   // RTD = 3
            case 4: return BoardType::TC_BOARD;   // THERMOCOUPLE = 4
            case 5: return BoardType::MIXED_BOARD; // ACTUATOR = 5 (repurposed)
...
std::map<std::string, std::string> BoardDiscovery::generate_board_config(const DiscoveredBoard& board) const {
...
std::map<std::string, std::string> BoardDiscovery::generate_sensor_config(const std::vector<SensorInfo>& sensors) const {
    std::map<std::string, std::string> config;
    ...
    for (const auto& [type, sensor_list] : sensors_by_type) {
        ...
        switch (type) {
            case 0x01: type_name = "pt"; break;
            case 0x02: type_name = "tc"; break;
            case 0x03: type_name = "rtd"; break;
            case 0x04: type_name = "lc"; break;
            default: type_name = "unknown_" + std::to_string(type); break;
        }
...
void BoardDiscovery::register_discovery_callback(std::function<void(const DiscoveredBoard&)> callback) {
