...
TCPClient::TCPClient() 
    : socket_fd_(-1), connected_(false), buffer_pos_(0) {
...
bool TCPClient::_write_all(const void* data, size_t len) {
    ...
    while (total_sent < len) {
        ssize_t sent = 0;
        // Handle EINTR (interrupted system call) - retry automatically (matching FSW's Socket::write)
        // CRITICAL: Use ::write() instead of ::send() to match FSW exactly!
        do {
