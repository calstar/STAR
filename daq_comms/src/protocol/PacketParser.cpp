...
PacketParser::PacketParser()
    : healthy_(true)
    , checksum_validation_enabled_(true)
    , next_sequence_(1)
...
std::optional<PacketParser::ParsedPacket> PacketParser::parse_packet(const uint8_t* data, size_t size) {
    if (size < PACKET_HEADER_SIZE) {
        handle_malformed_packet("Packet too small: " + std::to_string(size) + " < " + std::to_string(PACKET_HEADER_SIZE));
        return std::nullopt;
    }
    ...
    if (checksum_validation_enabled_) {
        ...
        if (calculated_checksum != received_checksum) {
            handle_malformed_packet("Checksum mismatch: calculated=" + 
    ...
    if (!header.is_valid()) {
        handle_malformed_packet("Invalid packet header (magic=" + 
                               std::to_string(header.magic_number) + 
    ...
    if (expected_payload_size != header.payload_size) {
        handle_malformed_packet("Payload size mismatch: expected " + 
                               ...
                               ", parsed " + std::to_string(expected_payload_size));
    }
    
    // Validate checksum if enabled
    if (checksum_validation_enabled_ && packet.is_valid) {
        uint16_t calculated_checksum = calculate_checksum(packet.raw_packet.data(), HEADER_DATA_SIZE);
...
std::optional<PacketParser::ParsedPacket> PacketParser::parse_packet(const std::vector<uint8_t>& data) {
    return parse_packet(data.data(), data.size());
}

bool PacketParser::parse_header(const uint8_t* data, size_t size, size_t& offset, PacketHeader& header) const {
...
bool PacketParser::parse_sensor_data(const uint8_t* data, size_t size, size_t& offset, SensorDataEntry& entry) const {
    if (offset + 1 > size) {
        return false;
    }
...
std::vector<uint8_t> PacketParser::construct_sensor_packet(
    ...
    // Add header
    add_header_to_packet(packet, PacketType::SENSOR_DATA, priority, 
                        payload_size, static_cast<uint16_t>(sensor_data.size()));
...
std::vector<uint8_t> PacketParser::construct_calibration_request(
    ...
    return construct_command_packet(0x01, command_data, Priority::HIGH); // 0x01 = calibration command
}

void PacketParser::add_header_to_packet(std::vector<uint8_t>& packet, PacketType type, Priority priority,
    ...
    uint16_t checksum = calculate_checksum(packet.data() + (packet.size() - HEADER_DATA_SIZE), HEADER_DATA_SIZE);
...
void PacketParser::add_sensor_entry_to_packet(std::vector<uint8_t>& packet, const SensorDataEntry& entry) const {
...
PacketToCommsMessageConverter::convert_pt_entry(
...
PacketToCommsMessageConverter::convert_tc_entry(
    const PacketParser::SensorDataEntry& entry, uint64_t receive_timestamp_ns) const {
...
PacketToCommsMessageConverter::convert_rtd_entry(
    const PacketParser::SensorDataEntry& entry, uint64_t receive_timestamp_ns) const {
...
PacketToCommsMessageConverter::convert_lc_entry(
    const PacketParser::SensorDataEntry& entry, uint64_t receive_timestamp_ns) const {
