...
IPAddress staticIP(
    192, 168, 2,
    101);  // 101 so PT board doesn't conflict with Actuator_Testing (100)
IPAddress gateway(192, 168, 2, 1);
IPAddress subnet(255, 255, 255, 0);
IPAddress dns(192, 168, 2, 1);
IPAddress receiverIP(
    192, 168, 2, 20);  // Same PC as actuator; different port (5007) for PT data
const int receiverPort =
    5007;  // 5007 so PT and Actuator (5006) can run together
EthernetUDP udp;
...
void setup() {
    ...
    Serial.println(
        "PT_BOARD_Multi_Send – cycle PTs, send DAQv2-Comms over UDP");
