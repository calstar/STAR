...
def filtered_sensor_data(data, window=5, std_threshold=2):
    ...
    filtered_data = [
        ...
        if (mean - std_threshold * std) <= d <= (mean + std_threshold * std)
    ]
...
def apply_dual_filter(data, ema_alpha=0.1, ma_window=250, std_threshold=0.001):
    ...
    ma_filtered = filtered_sensor_data(
        [ema_filtered], window=ma_window, std_threshold=std_threshold
    )
    return ma_filtered
...
instrument_to_connector = (
    []
)  # Filled at startup: instrument_to_connector[i] = board connector (1-10) for PT i+1
...
def read_udp(port=5006, bind_address="0.0.0.0"):
    ...
    try:
        ...
        print(
            "Make sure pt_multi_gui is not already running, or that SO_REUSEPORT is supported."
        )
        return
                    ...
                    if result:
                        ...
                        for j in range(len(instrument_deques)):
                            if len(instrument_deques[j]) > 0:
                                filtered_code = apply_dual_filter(
                                    list(instrument_deques[j])
                                )
                                filtered_code_deques[j].append(filtered_code)
                                filtered_codes.append(filtered_code)

                                calculated_pressure = (
                                    calculate_pressure_from_polynomial(
                                        filtered_code, calibration_polynomials[j]
                                    )
                                )
                                pressure_deques[j].append(calculated_pressure)
                                filtered_pressure = apply_dual_filter(
                                    list(pressure_deques[j])
                                )
                        ...
                        code_output = " | ".join(
                            [
                                f"PT {j+1}: {code:.0f}"
                                for j, code in enumerate(filtered_codes)
                            ]
                        )
                        print("Filtered ADC code readings:", code_output)

                        pressure_output = " | ".join(
                            [
                                f"PT {j+1}: {pressure:.3f} psi"
                                for j, pressure in enumerate(pressure_readings)
                            ]
                        )
    ...
    while True:
        try:
            pressure_input = input(
                f"Enter the {num_gauges} pressure gauge readings (comma-separated): "
            )
...
def interrupt():
    ...
    if all(len(Y[gauge]) > 1 for gauge in Y):
        for pt_num in range(len(instrument_deques)):
            try:
                gauge_num = gauge_mappings[
                    pt_num + 1
                ]  # Get the gauge number for this PT
                epsilon = 1e-10
                ...
                print(
                    f"Updated polynomial coefficients for PT {pt_num + 1}: {calibration_polynomials[pt_num]}"
                )
            except np.RankWarning:
                print(
                    f"Warning: The fit may be poorly conditioned for PT {pt_num + 1}."
                )
            except Exception as e:
        ...
        print(
            "Not enough data to fit a curve yet. Please enter at least one more data point."
        )
...
def save_calibration_data():
    with open(filename, "a", newline="") as f:
        ...
        for pt_num in range(len(instrument_deques)):
            ...
            row.extend([X[pt_num][-1], Y[gauge_num][-1]])  # ADC Code  # Pressure
    ...
    def update(frame):
        ...
        for j in range(len(instrument_deques)):
            if instrument_deques[j]:  # Check if there's data in the deque
                ...
                calculated_pressure = calculate_pressure_from_polynomial(
                    filtered_value, calibration_polynomials[j]
                )
            ...
            axs[row, col].plot(
                list(times),
                list(pressure_data[j]),
                label=f"Instrument {j+1} Pressure",
                color="blue",
            )
...
if __name__ == "__main__":
    ...
    while True:
        try:
            instrument_count = int(
                input("Please input the number of PTs for calibration: ")
            )
            break
        except ValueError:
    ...
    for pt_num in range(1, instrument_count + 1):
        while True:
            try:
                gauge_num = int(
                    input(f"Enter gauge number (1-{gauge_count}) for PT {pt_num}: ")
                )
                if 1 <= gauge_num <= gauge_count:
                    ...
                    print(
                        f"Please enter a valid gauge number between 1 and {gauge_count}"
                    )
            except ValueError:
                print("Invalid input. Please enter an integer.")
    ...
    while True:
        try:
            poly_order = int(
                input(
                    "Enter the polynomial order for the fit (e.g., 1 for linear, 2 for quadratic, etc.): "
                )
            )
            break
        except ValueError:
    ...
    filtered_code_deques = [
        deque(maxlen=data_point_num) for _ in range(instrument_count)
    ]
    ...
    with open(filename, "w", newline="") as f:
        ...
        for pt_index in range(instrument_count):
            ...
            pt_header.extend(
                [f"{base_label} Coefficient {k}" for k in range(poly_order + 1)]
            )
    ...
    while continue_calibration:
        try:
            ...
            while True:
                try:
                    user_input = (
                        input("Would you like to continue calibrating? (y/n): ")
                        .strip()
                        .lower()
                    )
                    if user_input == "y":
    ...
    show_live_feed = (
        input(
            "Would you like to see a live feed of the calibrated pressure readings vs time? (y/n): "
        )
        .strip()
        .lower()
    )
