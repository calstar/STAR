...
void setup() {
    ...
    if (!g_imu.begin(SPI, SPISettings(4000000, MSBFIRST, SPI_MODE3),
                     PIN_IMU_CS)) {
    ...
    if (!g_imu.configureWakeUpOnInt1(ISM330_WKUP_THS, ISM330_WAKEUP_DUR,
                                     ISM330_ROUTE_DRDY_TO_INT1)) {
...
void loop() {
    ...
    if (now - s_lastMcpMs >= 10U) {
        ...
        const float psi =
            (static_cast<float>(raw) / ADC_COUNTS) * PT_FULL_SCALE_PSI;
        if (sensorsShouldLog())
    ...
    if (g_imu.checkStatus()) {
        if (g_imu.getAccel(&g_accel) && g_imu.getGyro(&g_gyro)) {
            if (sensorsShouldLog()) {
                logIsm330(g_accel.xData / 1000.0f, g_accel.yData / 1000.0f,
                          g_accel.zData / 1000.0f, g_gyro.xData / 1000.0f,
                          g_gyro.yData / 1000.0f, g_gyro.zData / 1000.0f, now);
            }
        }
    }
