...
// Record: [0xDA][0x4E][ver=1][flags=0] [tag u16 LE] [t_ms u32 LE] [payload_len
// u16 LE] [payload...]
constexpr uint8_t REC_VER = 1;
constexpr size_t REC_HEADER = 12;

bool appendRecord(uint16_t tag, uint32_t t_ms, const void* payload,
...
void logFlightStateTransition(FlightState from, FlightState to, uint32_t t_ms,
                              uint8_t reason) {
#if !FLIGHT_LOG_ENABLE
    (void)from;
    (void)to;
...
void logIsm330(float ax, float ay, float az, float gx, float gy, float gz,
               uint32_t t_ms) {
    if (!sensorsShouldLog() || !s_flash)
        return;
