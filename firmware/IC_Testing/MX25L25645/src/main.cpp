...
void setup() {
    ...
    if (jedec != MX25_EXPECTED_JEDEC)
        Serial.printf(
            "WARN: expected 0x%06lX for MX25L25645G; continuing anyway.\r\n",
    ...
    // Use last 4 KiB sector so we stay away from low addresses other tooling
    // might use.
    ...
    // Second check: Arduino String round-trip (same idea as storage-test
    // Flash_Storage.ino)
    const char* msg = "MX25 OK";
    String sIn = msg;
    const uint16_t strBytes =
        static_cast<uint16_t>(sizeof(char) * (sIn.length() + 1U));
