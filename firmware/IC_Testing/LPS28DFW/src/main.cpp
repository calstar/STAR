...
/** Barometric altitude (m) from static pressure using a simple ISA troposphere
 * fit. */
...
void setup() {
    ...
    while (pressureSensor.begin(LPS28DFW_I2C_ADDR, Wire) != LPS28DFW_OK) {
        Serial.println(
            "ERROR: LPS28DFW not found. Check wiring and I2C address (0x5C / "
            "0x5D).");
        delay(1000);
    }
    ...
    Serial.printf(
        "Altitude uses ISA P0 = %.2f hPa (change LPS28DFW_SEA_LEVEL_HPA to "
