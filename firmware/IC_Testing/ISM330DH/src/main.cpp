...
void setup() {
    ...
    const uint8_t testPins[] = {ISM330_SCLK_PIN, ISM330_MOSI_PIN,
                                ISM330_CS_PIN};
    ...
    if (!imu.begin(SPI, ismSpiSettings, ISM330_CS_PIN)) {
        Serial.println(
            "ERROR: ISM330DHCX not found. Check SPI wiring / CS / power.");
        while (true)
            delay(1000);
    }
    ...
    if (!imu.configureWakeUpOnInt1(ISM330_WKUP_THS, ISM330_WAKEUP_DUR,
    ...
    Serial.printf(
        "Accel FS: %.0f g  wake-up threshold: %.2f g  WK_THS LSBs: %u  "
        "wake_dur: %u\r\n",
