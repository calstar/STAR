 ...
 * sends NO_CONNECTION_ABORT to PTs, runs PT Abort / No PT Abort /
 * StandaloneAbort. Uses DAQv2-Comms over Ethernet/UDP. Based on
 * Avionics_State_Machine_Drafts/actuator_c.cpp.
 */
...
// For ConnectionLossDetected -> StandaloneAbort: last time we got sensor data
// from a config PT
...
//-----------------------------------------------------------------------------
// Helpers: map state to daq::BoardState for heartbeats
//-----------------------------------------------------------------------------
...
static IncomingPacketKind processIncomingPacket(const uint8_t* buffer,
    ...
    switch (hdr.packet_type) {
        ...
        case daq::PacketType::ACTUATOR_CONFIG: {
            ...
            if (daq::parse_actuator_config_packet(
                    buffer, len, dummy, is_controller, act_locs, pt_locs,
...
static void run_NoConnectionAbort() {
    ...
    if (elapsed >= NO_CONN_ABORT_PT_WAIT_MS) {
        no_conn_abort_pt_sent =
            false;  // reset for next time we enter NoConnectionAbort
...
static void run_NoPTAbort() {
    // TODO: Run slightly different procedure involving receiving PT data and
    // actuating solenoids. When procedure finishes, transition to
    // AbortFinished.
    if ((millis() - state_enter_ms) >= NO_CONNECTION_ABORT_DONE_MS) {
        state = ActuatorControllerState::AbortFinished;
        state_enter_ms = millis();
...
static void run_ConnectionLossDetected() {
    ...
    // If connection to one or more necessary-for-abort boards is lost ->
    // StandaloneAbort
    if (is_abort_controller && config_valid && !abort_pt_locations.empty()) {
        if ((millis() - last_pt_data_from_config_ms) >
            STANDALONE_ABORT_PT_LOSS_MS) {
