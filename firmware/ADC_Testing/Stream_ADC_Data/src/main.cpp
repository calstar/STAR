...
// Change the following line to automatically use the correct pins for the board
// being tested (PT_Board, LC_Board, RTD_Board, or TC_Board)
#define PINS_ACTIVE_LAYOUT sense_board_pins::PT_Board

// These lines MUST be after the #define PINS_ACTIVE_LAYOUT or they will
// overwrite it with the default value!
#include "connector_adc_map.h"
#include "sense_board_pins.h"

// Connectors 1-10 on the sense board. Each chunk = one full scan (all
// connectors). Packet format expects num_sensors datapoints per chunk; max 9
// chunks fits in 512 bytes.
#define TEST_PIN 1
...
// One chunk = full scan (10 datapoints). Accumulate up to
...
// Set the reference voltage based on configuration in main.h
    ...
    // Set the datarate. You can change this, but the options depends on the
    // filter I do not know what happens if you program an invalid data rate for
    // a given filter
    ...
    ESP_ERROR_CHECK(
        esp_read_mac(mac, ESP_MAC_ETH));  // Derived from base eFuse MAC
...
// One chunk = full scan of all 10 connectors (packet format expects num_sensors
// per chunk)
void collect_chunk() {
    Diablo::SensorDataChunkCollection chunk(millis(), NUM_CONNECTORS);
    for (uint8_t connector_id = 1; connector_id <= NUM_CONNECTORS;
         connector_id++) {
        ads126x.setInputMux(getAdcChannel(connector_id, TEST_PIN),
                            ADS126X_AINCOM);
