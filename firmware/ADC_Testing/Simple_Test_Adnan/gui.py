...
MAGIC = b"AD26"
BUFFER_SIZE = (
    1996  # 4 magic + 83 samples × 3 channels × (4 bytes timestamp + 4 bytes reading)
)
MAGIC_SIZE = 4
SAMPLES_PER_BUFFER = 83
NUM_CHANNELS = 3
DATA_SIZE = (
    SAMPLES_PER_BUFFER * NUM_CHANNELS * 8
)  # 1992 bytes (timestamp + reading per channel)
...
class Reader(QtCore.QThread):
    samples_ready = QtCore.pyqtSignal(
        list
    )  # Emits list of (ch, timestamp_us, volts) tuples for a packet
    ...
    def _find_and_parse_packets(self):
        ...
        while True:
            ...
            for sample_idx in range(SAMPLES_PER_BUFFER):
                base = (
                    sample_idx * NUM_CHANNELS * 8
                )  # 8 bytes per channel (timestamp + reading)
            ...
            try:
                ...
                if data:
                    ...
                    if now - last_status_time >= 0.5:
                        self.status.emit(
                            f"Reader: {self.packets_received} pkts, {self.total_samples} samples, buf={len(self.buf)}B"
                        )
...
class App(QtWidgets.QMainWindow):
    ...
    def _connect(self, port, baud):
        ...
        self.reader.samples_ready.connect(
            self._on_samples, QtCore.Qt.ConnectionType.QueuedConnection
        )
        self.reader.status.connect(
            self.set_status, QtCore.Qt.ConnectionType.QueuedConnection
        )
        self.reader.raw_bytes.connect(
            self._on_raw_bytes, QtCore.Qt.ConnectionType.QueuedConnection
        )
        self.reader.start()
        ...
        if self.last_sps_time is not None:
            ...
            if elapsed >= 1.0:
                ...
                self.lbl_sps.setText(
                    f"SPS: {sps:.1f} | Pkts: {self.packets_received} | Pts: {pts}"
                )
