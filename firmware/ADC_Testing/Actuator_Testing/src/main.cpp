...
// Configuration
const unsigned long ADC_READ_INTERVAL_MS =
    1000;  // Read ADC every 100ms (adjust as needed)
        ...
        if (bytesRead > 0) {
            ...
            if (header->packet_type == Diablo::PacketType::ACTUATOR_COMMAND) {
                ...
                if (Diablo::parse_pwm_actuator_packet(packetBuffer, bytesRead,
                    ...
                    Serial.println(
                        "Error: Failed to parse PWM actuator command packet");
                }
            } else {
                // Unknown packet type
...
void processPWMActuatorCommand(
    ...
    for (const auto& cmd : commands) {
        ...
        // Frequency in Hz, resolution in bits (14 bits = 0-16383 for better
        // low-frequency support) 10Hz with 14-bit resolution -> div = 80M / (10
        // * 16384) = ~488 (fits in divider)
        ...
        // Update effective state (non-zero duty = ON for state tracking
        // purposes, though it's PWM)
...
void updatePWM() {
    ...
    for (int i = 0; i < NUM_ACTUATORS; i++) {
        if (pwm_states[i].active) {
            if (current_time - pwm_states[i].start_time >=
                pwm_states[i].duration) {
