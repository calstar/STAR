...
float NoiseAnalyzer::noiseFreeBits(uint8_t adcBits) const {
    if (_count < 50)
        return NAN;  // Need at least 50 samples for good statistics
    ...
    // Peak-to-peak noise ≈ 6.6 × RMS for Gaussian distribution (99.99%
    // confidence) NFB = log2(2^adcBits / noise_pp) = adcBits - log2(noise_pp)
...
float NoiseAnalyzer::enob(uint8_t adcBits) const {
    ...
    if (enobValue < 0.0)
        enobValue = 0.0;
    if (enobValue > (double)adcBits)
        enobValue = (double)adcBits;
...
bool NoiseAnalyzer::isStable(float cvThreshold) const {
    if (_count < 10)
        return false;  // Need reasonable sample size
