...
_file_dir = os.path.dirname(
    os.path.abspath(__file__)
)  # external/DiabloAvionics/test_guis
...
try:
    ...
    print(
        f"⚠️  Robust calibration framework not available: {e}. Using simple polynomial fit."
    )
...
ACTUATOR_DISPLAY_ORDER_WHEN_ROLES = [
    "Fuel Vent",
    "LOX Vent",
    "Fuel Press",
    "LOX Press",
...
class ConfigManager:
    ...
    def _backfill_role_names(self):
        ...
        if "actuator_abbrev_to_role" not in self.config:
            self.config["actuator_abbrev_to_role"] = dict(
                DEFAULT_ACTUATOR_ABBREV_TO_ROLE
            )
    ...
    def get_actuator_abbrev_to_role(self):
        ...
        return self.config.get("actuator_abbrev_to_role") or dict(
            DEFAULT_ACTUATOR_ABBREV_TO_ROLE
        )
    ...
    def get_pt_calibration_csv_paths(self):
        ...
        return [
            str(_SCRIPT_DIR / path) if not os.path.isabs(path) else path for path in raw
        ]
    ...
    def set_actuator_role(self, role_name, actuator_id):
        ...
        roles = self.config.setdefault(
            "actuator_roles", {name: 0 for name in role_names}
        )
        ...
        if isinstance(existing_value, list) and len(existing_value) == 2:
            ...
            roles[role_name] = [
                existing_value[0],
                int(actuator_id) if actuator_id else 0,
            ]
        else:
...
def load_state_machine_csv(csv_path: Path) -> tuple[Dict[str, Dict[str, str]], int]:
    ...
    try:
        with open(csv_path, "r", newline="", encoding="utf-8") as f:
            ...
            header = rows[0]
            states = [
                col.strip() for col in header[1:] if col.strip()
            ]  # Skip first empty column
                ...
                for i, state in enumerate(states):
                    if i + 1 < len(row):
                        ...
                        if value in ["OPEN", "CLOSE", "CLOSED"]:
                            state_machine[state][actuator_abbrev] = (
                                "OPEN" if value == "OPEN" else "CLOSE"
                            )
...
def apply_state_from_csv(
    actuator_widget, state_name: str, state_machine: Dict[str, Dict[str, str]]
):
    """
    ...
    desired_by_actuator_id: Dict[int, int] = {
        aid: 0 for aid in range(1, get_num_actuators() + 1)
    }
    ...
    for abbrev, action in state_config.items():
        ...
        if actuator_id == 0:
            print(
                f"Warning: Actuator role '{role_name}' ({abbrev}) not assigned to any actuator"
            )
            continue
    ...
    for actuator_id in range(1, get_num_actuators() + 1):
        ...
        current = (
            actuator_widget.actuator_states[array_idx]
            if 0 <= array_idx < len(actuator_widget.actuator_states)
            else None
        )
...
class SensorPlotWidget(QtWidgets.QWidget):
    ...
    def __init__(self, receiver, bind_address: str = "0.0.0.0", parent=None):
        ...
        self.graph_moving_avg_samples = (
            1  # Moving average for graphed lines (1 = no smoothing)
        )
        ...
        self.sensor_adc_codes: Dict[int, deque] = (
            {}
        )  # Store ADC codes for pressure calculation
        ...
        self.sensor_psi_history: Dict[int, list] = (
            {}
        )  # PSI data for CSV saving (full history)
        self.sensor_plots: Dict[int, pg.PlotDataItem] = {}
        self.plot_enabled: Dict[int, bool] = {
            i: True for i in range(1, NUM_CONNECTORS + 1)
        }
        ...
        self.pt_calibration, self.pt_calibration_error = load_pt_calibration(
            CONFIG.get_pt_calibration_csv_paths()
        )
        ...
        self.sensor_labels = {
            i: CONFIG.get_sensor_label(i) for i in range(1, NUM_CONNECTORS + 1)
        }
        ...
        self.graph_moving_avg_samples = disp.get(
            "graph_ma_samples", self.graph_moving_avg_samples
        )
        self.display_moving_avg_samples = disp.get(
            "display_ma_samples", self.display_moving_avg_samples
        )
    ...
    def init_ui(self):
        ...
        self.status_label.setFixedWidth(
            400
        )  # Fixed width to prevent layout resize on text change
        ...
        self.under_graph_name_labels: Dict[int, QtWidgets.QLabel] = (
            {}
        )  # name label (PT role)
        self.under_graph_containers: Dict[int, QtWidgets.QWidget] = (
            {}
        )  # cell to show/hide
        for i in range(1, NUM_CONNECTORS + 1):
        ...
        self.plot_item = self.plot_widget.addPlot(
            title="Pressure Data Over Time (Calibrated Sensors)"
        )

        # Set title color and size to white for visibility on black background
        self.plot_item.setTitle(
            "Pressure Data Over Time (Calibrated Sensors)", color="w", size="14pt"
        )
        ...
        self.sps_text_item = pg.TextItem(
            text="SPS: 0", color=(200, 200, 200), anchor=(0, 1)
        )
        self.sps_text_item.setPos(
            0, 0
        )  # Initial position, will be updated in update_plots/resize
    ...
    def init_connector_plots(self):
        ...
        for connector_id in range(1, NUM_CONNECTORS + 1):
            ...
            if connector_id in self.pt_calibration:
                self.sensor_psi_plot_t[connector_id] = deque(
                    maxlen=self.plot_buffer_size
                )
                self.sensor_psi_plot_v[connector_id] = deque(
                    maxlen=self.plot_buffer_size
                )
    ...
    def reload_pt_calibration(self):
        ...
        for connector_id in range(1, NUM_CONNECTORS + 1):
            if (
                connector_id in self.pt_calibration
                and connector_id not in self.sensor_psi_plot_t
            ):
                self.sensor_psi_plot_t[connector_id] = deque(
                    maxlen=self.plot_buffer_size
                )
    ...
    def update_buffer_size(self):
        ...
        if new_size != self.plot_buffer_size:
            ...
            for k in list(self.sensor_psi_plot_t.keys()):
                self.sensor_psi_plot_t[k] = deque(
                    self.sensor_psi_plot_t[k], maxlen=self.plot_buffer_size
                )
                self.sensor_psi_plot_v[k] = deque(
                    self.sensor_psi_plot_v[k], maxlen=self.plot_buffer_size
                )
            ...
            for dp in chunk["datapoints"]:
                ...
                if sensor_id not in self.sensor_data:
                    ...
                    if sensor_id in self.pt_calibration:
                        self.sensor_psi_plot_t[sensor_id] = deque(
                            maxlen=self.plot_buffer_size
                        )
                        self.sensor_psi_plot_v[sensor_id] = deque(
                            maxlen=self.plot_buffer_size
                        )
                ...
                if sensor_id in self.pt_calibration:
                    ...
                    self.sensor_psi_plot_v[sensor_id].append(
                        raw_psi - offset
                    )  # plot shows zeroed
                    self.sensor_psi_history[sensor_id].append(
                        (relative_time, raw_psi)
                    )  # history stores raw
    ...
    def _send_demo_packet(self):
        ...
        if (
            not self.demo_mode
            or self.demo_start_time is None
            or not self.pt_calibration
        ):
            return
        ...
        if packet and self.demo_send_socket is not None:
            try:
                self.demo_send_socket.sendto(packet, ("127.0.0.1", self.receiver.port))
            except OSError:
                pass
    ...
    def update_plots(self):
        ...
        for sensor_id in self.sensor_psi_plot_t:
            ...
            try:
                ...
                if len(times_array) > MAX_PLOT_POINTS:
                    indices = np.linspace(
                        0, len(times_array) - 1, MAX_PLOT_POINTS, dtype=int
                    )
                ...
                if (
                    self.graph_moving_avg_samples > 1
                    and len(psi_array) >= self.graph_moving_avg_samples
                ):
                    kernel = (
                        np.ones(self.graph_moving_avg_samples)
                        / self.graph_moving_avg_samples
                    )
        ...
        if self.y_axis_auto_scale:
            ...
            for sid in self.sensor_psi_plot_t:
                if (
                    sid not in self.sensor_plots
                    or not self.sensor_plots[sid].isVisible()
                ):
                    continue
    ...
    def update_statistics(self):
        ...
        for connector_id in range(1, NUM_CONNECTORS + 1):
            display_text = "---"
            self.debug_values[connector_id] = {
                "current": None,
                "moving_avg": None,
                "psi": None,
            }
            ...
            if d and len(d) > 0:
                ...
                if connector_id in self.pt_calibration and adc_d and len(adc_d) > 0:
                    ...
                    psi_val = calculate_pressure(
                        moving_avg_adc, a, b, c, dd
                    ) - self.pt_offsets.get(connector_id, 0.0)
    ...
    def save_pressures_csv(self):
        ...
        if not self.sensor_psi_history:
            QtWidgets.QMessageBox.warning(
                self, "No Data", "No pressure data available to export."
            )
            return
        ...
        if not sensors_to_save:
            QtWidgets.QMessageBox.warning(
                self, "No Data", "No sensors with roles available to export."
            )
            return
        ...
        try:
            with open(filename, "w", newline="", encoding="utf-8") as csvfile:
                ...
                if not all_times:
                    QtWidgets.QMessageBox.warning(
                        self, "No Data", "No timestamp data available."
                    )
                    return
            ...
            QtWidgets.QMessageBox.information(
                self, "Success", f"Saved pressures CSV to {filename}"
            )
                ...
                try:
                    with open(zeroed_filename, "w", newline="", encoding="utf-8") as zf:
                        ...
                        for sensor_id in sensors_to_save:
                            label = self.sensor_labels.get(sensor_id, "")
                            col = (
                                f"PT{sensor_id}: {label} (psi, zeroed)"
                                if label
                                else f"PT{sensor_id} (psi, zeroed)"
                            )
                            zheader.append(col)
                        ...
                        for t in sorted(all_times):
                            ...
                            for sensor_id in sensors_to_save:
                                val = row_data.get(sensor_id, "")
                                if val != "":
                                    zeroed_val = val - self.pt_offsets.get(
                                        sensor_id, 0.0
                                    )
                    ...
                    QtWidgets.QMessageBox.warning(
                        self, "Zeroed CSV Error", f"Could not save zeroed CSV: {ze}"
                    )
        except Exception as e:
    ...
    def save_events_csv(self):
        ...
        if not events:
            QtWidgets.QMessageBox.warning(
                self, "No Data", "No events available to export."
            )
            return
        ...
        try:
            ...
            QtWidgets.QMessageBox.information(
                self, "Success", f"Saved events CSV to {filename}"
            )
        except Exception as e:
            QtWidgets.QMessageBox.critical(self, "Error", f"CSV save error: {e}")
...
class ActuatorControlWidget(QtWidgets.QWidget):
    ...
    def __init__(
        self, receiver, device_ip: str = None, device_port: int = None, parent=None
    ):
        super().__init__(parent)
        self.receiver = receiver
        ...
        self.actuator_types = {
            i: CONFIG.get_actuator_type_by_id(i)
            for i in range(1, get_num_actuators() + 1)
        }
        ...
        self.actuator_labels = {
            i: CONFIG.get_actuator_label(i) for i in range(1, get_num_actuators() + 1)
        }

        # Load display settings from Config
        disp = CONFIG.config["display"]
        self.only_show_actuators_with_roles = disp.get(
            "only_show_actuators_with_roles", False
        )
    ...
    def init_ui(self):
        ...
        for i in range(num_actuators):
            ...
            actuator_frame.setStyleSheet(
                "padding: 2px 4px 4px 4px; margin: 1px; background-color: #353535;"
            )
            ...
            label_text = (
                self.actuator_labels.get(actuator_id, "") or f"Actuator {actuator_id}"
            )
            ...
            id_label.setStyleSheet(
                "font-weight: bold; font-size: 10pt; padding: 1px 2px; color: white; background-color: transparent;"
            )
            ...
            on_btn.clicked.connect(
                lambda checked=False, aid=actuator_id: self.set_actuator_state(aid, 1)
            )
            ...
            off_btn.clicked.connect(
                lambda checked=False, aid=actuator_id: self.set_actuator_state(aid, 0)
            )
            ...
            voltage_label.setStyleSheet(
                "font-size: 7pt; padding: 0px 2px; color: white; background-color: transparent;"
            )
    ...
    def on_sensor_data(self, header: dict, chunks: List[dict], source_ip: str):
        ...
        for dp in latest_chunk["datapoints"]:
            ...
            try:
                voltage = struct.unpack("<f", struct.pack("<I", data_u32 & 0xFFFFFFFF))[
                    0
                ]
    ...
    def update_button_highlight(self, array_idx: int, actuator_state: int):
        ...
        widget = self.actuator_widgets[array_idx]
        bg_color = (
            widget["frame"].palette().color(QtGui.QPalette.ColorRole.Window).name()
        )
    ...
    def send_actuator_command(self, actuator_id: int, hardware_command: int):
        ...
        try:
            ...
            if len(packet) > 0:
                ...
                print(
                    f"Sent command: Actuator {actuator_id} -> {'OPEN' if gui_state else 'CLOSED'} (hardware: {'ON' if hardware_command else 'OFF'})"
                )
            else:
...
class PWMControlWindow(QtWidgets.QWidget):
    ...
    def init_ui(self):
        ...
        for i in range(get_num_actuators()):
            ...
            freq_spin.setRange(
                0.1, 1000000.0
            )  # Match extended range from previous task
            ...
            btn_exec.clicked.connect(
                lambda checked=False, aid=actuator_id: self.execute_now(aid)
            )
            ...
            chk_arm.toggled.connect(
                lambda checked, aid=actuator_id: self.update_arm_status(aid, checked)
            )
        ...
        btn_disarm_all.setStyleSheet(
            "background-color: #e74c3c; color: white; padding: 5px 15px;"
        )
        ...
        if self.parent_window and self.parent_window.actuator_widget:
            try:
                ...
                if sock:
                    ...
                    print(
                        f"Manual PWM: ID={actuator_id} F={freq}Hz D={duty*100}% T={dur_s}s"
                    )
            except Exception as e:
    ...
    def disarm_all(self):
        ...
        for aid, widgets in self.actuator_rows.items():
            widgets["arm_chk"].setChecked(
                False
            )  # This triggers update_arm_status via signal
...
class PressureBarWidget(QtWidgets.QWidget):
    ...
    BOTTOM_CLEARANCE = 4
    BOTTOM_MARGIN = (
        BOTTOM_GAP + VALUE_BOX_HEIGHT + BOTTOM_CLEARANCE
    )  # total bottom region (parent uses for bar extent)
    ...
    def paintEvent(self, event):
        ...
        draw_w = max(
            28, int(bar_w * 0.85)
        )  # bar width (slightly thinner than full slot)
        ...
        painter.drawText(
            QtCore.QRect(bar_x, 0, draw_w, top_margin),
        ...
        if v <= thresh:
            ...
            segment = (
                0.20 + (v - thresh) / (nop - thresh) * 0.20 if nop > thresh else 0.20
            )
        elif v <= meop:
        ...
        if self.fixed_color:
            ...
            if self.current_value > self.meop:
                ...
                painter.setBrush(
                    QtGui.QColor("#27ae60")
                )  # jungle green (darker for white text)
        ...
        white_dash = QtGui.QPen(
            QtCore.Qt.GlobalColor.white, 1, QtCore.Qt.PenStyle.DashLine
        )
        orange_dash = QtGui.QPen(
            QtGui.QColor("#ff9f43"), 1, QtCore.Qt.PenStyle.DashLine
        )
        ...
        box_color = (
            ...
            if self.fixed_color
                ...
                if self.current_value > self.meop
                    ...
                    QtGui.QColor("#fdcb6e")
                    if self.current_value > self.nop
...
class TopBarWidget(QtWidgets.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        def limits(fluid: str):
            ...
            return (
                float(d.get("NOP", 600)),
                float(d.get("MEOP", 650)),
                float(d.get("POP", 750)),
                float(d.get("THRESH", 400)),
            )
        ...
        self.bar_gn2 = PressureBarWidget(
            "GN2",
            nop_gn2,
            meop_gn2,
            pop_gn2,
            thresh=thresh_gn2,
            fixed_color=QtGui.QColor("#27ae60"),
        )  # jungle green (darker for white text)
        self.bar_eth = PressureBarWidget(
            "ETH",
            nop_eth,
            meop_eth,
            pop_eth,
            thresh=thresh_eth,
            fixed_color=QtGui.QColor("#ff9f43"),
        )  # vibrant pastel orange (fuel)
        self.bar_lox = PressureBarWidget(
            "LOX",
            nop_lox,
            meop_lox,
            pop_lox,
            thresh=thresh_lox,
            fixed_color=QtGui.QColor("#74b9ff"),
        )  # vibrant pastel blue
        ...
        self.state_transitions = load_state_transitions_csv(
            CONFIG.get_state_transitions_csv_path()
        )
        self.current_state = None  # Track current state for transition validation (None = no state selected)
        self.unlock_states = (
            False  # Track whether state transitions are unlocked (via DEBUG button)
        )
        ...
        for i, label in enumerate(mode_labels):
            ...
            btn.clicked.connect(
                lambda checked=False, idx=i: self.on_mode_button_clicked(idx)
            )
...
class SettingsWidget(QtWidgets.QWidget):
    ...
    def __init__(self, sensor_widget, actuator_widget, parent=None):
        ...
        # Default mappings
        self.gauge_mappings = {"GN2": 0, "ETH": 0, "LOX": 0}
    ...
    def init_ui(self):
        ...
        self.sim_pressure_chk.setToolTip(
            "If enabled: In FIRE state, transitions to ARMED if LOX or Fuel Upstream < 350 psi"
        )
        ...
        for i in range(1, NUM_CONNECTORS + 1):
            ...
            cb.stateChanged.connect(
                lambda s, cid=i: self.sensor_widget._on_plot_toggle(cid, s)
            )
            cb.setFixedWidth(24)
        ...
        self.only_show_pt_with_roles_chk = QtWidgets.QCheckBox(
            "Only show PTs with roles"
        )
        self.only_show_pt_with_roles_chk.toggled.connect(
            self.on_only_show_pt_with_roles_changed
        )
        ...
        self.zero_pts_btn.setToolTip(
            "Set current PSI readings as zero reference (tare all PTs)"
        )
        self.zero_pts_btn.setStyleSheet(
            "QPushButton { background-color: #e67e22; color: white; font-weight: bold; padding: 4px 8px; border-radius: 4px; }"
            "QPushButton:hover { background-color: #d35400; }"
        )
        self.zero_pts_btn.clicked.connect(self.on_zero_pts)
        self.clear_zeros_btn = QtWidgets.QPushButton("Clear Zeros")
        self.clear_zeros_btn.setToolTip(
            "Remove zero offset — restore raw calibration values"
        )
        self.clear_zeros_btn.setStyleSheet(
            "QPushButton { background-color: #555; color: white; padding: 4px 8px; border-radius: 4px; }"
        ...
        self.only_show_actuators_with_roles_chk = QtWidgets.QCheckBox(
            "Only show actuators with roles"
        )
        self.only_show_actuators_with_roles_chk.toggled.connect(
            self.on_only_show_actuators_with_roles_changed
        )
        ...
        for role_name in CONFIG.get_actuator_role_names():
            ...
            combo.currentIndexChanged.connect(
                lambda idx, rn=role_name, c=combo: self.on_actuator_role_changed(rn, c)
            )
        ...
        for name, combo in self.combos.items():
            ...
            combo.currentIndexChanged.connect(
                lambda idx, n=name, c=combo: self.on_mapping_changed(n, c)
            )
        ...
        self.gn2_pop_spin.valueChanged.connect(
            lambda v: self._on_threshold_changed("gn2_pop_psi", v)
        )
        ...
        self.fuel_pop_spin.valueChanged.connect(
            lambda v: self._on_threshold_changed("fuel_pop_psi", v)
        )
        ...
        self.ox_pop_spin.valueChanged.connect(
            lambda v: self._on_threshold_changed("ox_pop_psi", v)
        )
        ...
        self.gn2_thresh_spin.valueChanged.connect(
            lambda v: self._on_threshold_changed("gn2_thresh_psi", v)
        )
        ...
        self.fuel_thresh_spin.valueChanged.connect(
            lambda v: self._on_threshold_changed("fuel_thresh_psi", v)
        )
        ...
        self.ox_thresh_spin.valueChanged.connect(
            lambda v: self._on_threshold_changed("ox_thresh_psi", v)
        )
        ...
        for role_name in CONFIG.get_sensor_role_names():
            ...
            for pt_id in range(1, NUM_CONNECTORS + 1):
                combo.addItem(f"PT {pt_id}", pt_id)
            combo.currentIndexChanged.connect(
                lambda idx, rn=role_name, c=combo: self.on_pt_role_changed(rn, c)
            )
    ...
    def load_values(self):
        ...
        with QtCore.QSignalBlocker(self.sim_pressure_chk):
            self.sim_pressure_chk.setChecked(
                CONFIG.config.get("simulate_chamber_pressure", False)
            )
        ...
        if (
            hasattr(self.sensor_widget, "pt_calibration_error")
            and self.sensor_widget.pt_calibration_error
        ):
            self.pt_calibration_error_label.setText(
                self.sensor_widget.pt_calibration_error
            )
        ...
        with QtCore.QSignalBlocker(self.only_show_pt_with_roles_chk):
            self.only_show_pt_with_roles_chk.setChecked(
                self.sensor_widget.only_show_pt_with_roles
            )
        ...
        for i in range(1, NUM_CONNECTORS + 1):
            if i in self.plot_visibility_cbs:
                with QtCore.QSignalBlocker(self.plot_visibility_cbs[i]):
                    self.plot_visibility_cbs[i].setChecked(
                        self.sensor_widget.plot_enabled.get(i, True)
                    )
        ...
        with QtCore.QSignalBlocker(self.only_show_actuators_with_roles_chk):
            self.only_show_actuators_with_roles_chk.setChecked(
                self.actuator_widget.only_show_actuators_with_roles
            )
    ...
    def _update_pt_calibration_csv_config(self):
        ...
        CONFIG.config.setdefault("paths", {})["pt_calibration_csv"] = (
            paths if paths else []
        )
        CONFIG.save()
    ...
    def on_pt_calibration_csv_add(self):
        ...
        if self.pt_calibration_csv_list.count() > 0:
            last_item = self.pt_calibration_csv_list.item(
                self.pt_calibration_csv_list.count() - 1
            )
            if last_item:
                last_path = (
                    os.path.dirname(last_item.text())
                    if os.path.dirname(last_item.text())
    ...
    def _on_threshold_changed(self, key: str, value: float):
        ...
        if fluid is not None:
            CONFIG.config.setdefault("pressure_limits", {}).setdefault(fluid, {})[
                limit
            ] = value
            CONFIG.save()
    ...
    def on_sim_pressure_toggled(self, checked):
        ...
        if hasattr(self.parent(), "parent") and hasattr(
            self.parent().parent(), "update_sim_pressure_state"
        ):
    ...
    def _on_reset_all_data(self):
        ...
        main_win = (
            sw.main_window_ref
            if hasattr(sw, "main_window_ref") and sw.main_window_ref
            else None
        )
...
class ActuatorControlWindow(QtWidgets.QMainWindow):
    ...
    def __init__(
        self,
        receiver,
        device_ip: str = DEFAULT_ACTUATOR_IP,
        ...
        self.setCentralWidget(
            ActuatorControlWidget(receiver, device_ip, device_port, self)
        )
...
class CalibrationWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, sensor_widget, parent=None):
        ...
        if ROBUST_CALIBRATION_AVAILABLE:
            for sensor_id in range(1, NUM_CONNECTORS + 1):
                self.robust_frameworks[sensor_id] = RobustCalibrationFramework(
                    sensor_id
                )
    ...
    def init_ui(self, parent):
        ...
        instructions.setStyleSheet(
            "color: #555; font-size: 10pt; padding: 10px; background: #f0f0f0; border-radius: 5px;"
        )
        ...
        for sensor_id in range(1, NUM_CONNECTORS + 1):
            ...
            frame.setStyleSheet(
                "QFrame { border: 1px solid #ddd; border-radius: 3px; padding: 3px; }"
            )
            ...
            calibrated_label.setStyleSheet(
                "font-family: monospace; font-size: 10pt; font-weight: bold; color: #27ae60;"
            )
        ...
        for sensor_id in range(1, NUM_CONNECTORS + 1):
            ...
            if adc_label:
                ...
                if adc_value is not None:
                    ...
                    adc_label.setStyleSheet(
                        "font-family: 'Courier New', monospace; font-size: 11pt; font-weight: bold; color: #2c3e50;"
                    )

                    # Calculate and display calibrated pressure in real-time
                    calibrated_label = self.findChild(
                        QtWidgets.QLabel, f"calibrated_{sensor_id}"
                    )
                    if calibrated_label:
                        calibrated_pressure = self.calculate_calibrated_pressure(
                            sensor_id, adc_value
                        )
                        if calibrated_pressure is not None:
                            calibrated_label.setText(f"{calibrated_pressure:.2f}")
                            calibrated_label.setStyleSheet(
                                "font-family: 'Courier New', monospace; font-size: 11pt; font-weight: bold; color: #27ae60;"
                            )
                        else:
                            calibrated_label.setText("--")
                            calibrated_label.setStyleSheet(
                                "font-family: 'Courier New', monospace; font-size: 11pt; color: #95a5a6;"
                            )
                else:
                    adc_label.setText("--")
                    adc_label.setStyleSheet(
        ...
        if hasattr(self.sensor_widget, "sensor_adc_codes"):
            ...
            if available_keys:
                sample_data = [
                    (
                        k,
                        len(self.sensor_widget.sensor_adc_codes[k]),
                        (
                            self.sensor_widget.sensor_adc_codes[k][-1][1]
                            if len(self.sensor_widget.sensor_adc_codes[k]) > 0
                            else None
                        ),
                    )
    ...
    def record_calibration_point(self):
            ...
            for sensor_id, ref_pressure in reference_pressures.items():
                if sensor_id in adc_codes and sensor_id in self.robust_frameworks:
                    ...
                    cal_point = CalibrationPoint(
                        ...
                        pressure=float(
                            ref_pressure
                        ),  # 0.0 is valid for zero-point calibration
                        timestamp=time.time(),
                        environmental_state=env_state,
                        uncertainty=0.01,  # 1% default uncertainty
                    )
                    self.robust_frameworks[sensor_id].calibration_points.append(
                        cal_point
                    )
                    ...
                    if len(framework.calibration_points) >= 1:
                        ...
                        if len(framework.calibration_points) == 1:
                            ...
                            try:
                                theta_tls, cov_tls = (
                                    framework.total_least_squares_calibration(
                                        framework.calibration_points
                                    )
                                )
                                theta_post, cov_post = framework.bayesian_update(
                                    framework.calibration_points
                                )
                                ...
                                adc_list = [
                                    cp.adc_code for cp in framework.calibration_points
                                ]
                                pressure_list = [
                                    cp.pressure for cp in framework.calibration_points
                                ]
                                if len(adc_list) >= 2:
                                    poly_coeffs = np.polyfit(
                                        adc_list,
                                        pressure_list,
                                        min(3, len(adc_list) - 1),
                                    )
                                    self.calibration_polynomials[sensor_id] = tuple(
                                        poly_coeffs
                                    )
                            except Exception as e:
                                print(
                                    f"Warning: Could not fit calibration immediately for PT {sensor_id}: {e}"
                                )
        ...
        self.status_label.setText(
            f"Recorded point #{len(self.calibration_points)} - Calibrated pressure will update immediately"
        )
    ...
    def update_points_table(self):
        ...
        for i, point in enumerate(self.calibration_points):
            ...
            sensors_str = ", ".join(
                [f"PT{sid}" for sid in sorted(point["adc_codes"].keys())]
            )
    ...
    def fit_calibration(self):
        ...
        if len(self.calibration_points) < 3:
            QtWidgets.QMessageBox.warning(
                self, "Insufficient Data", "Need at least 3 points."
            )
            return
        ...
        if ROBUST_CALIBRATION_AVAILABLE:
            ...
            for sensor_id in range(1, NUM_CONNECTORS + 1):
                ...
                if len(cal_points) >= 3:
                    try:
                        ...
                        theta_tls, cov_tls = framework.total_least_squares_calibration(
                            cal_points
                        )
                        ...
                        calib_status = self.findChild(
                            QtWidgets.QLabel, f"calib_status_{sensor_id}"
                        )
                        if calib_status:
                            calib_status.setText(
                                f"Calibrated (R²={r_squared:.3f}, {conf_level})"
                            )
                        ...
                        print(
                            f"Failed to fit PT {sensor_id} with robust framework: {e}"
                        )
        else:
            ...
            for sensor_id in range(1, NUM_CONNECTORS + 1):
                ...
                for point in self.calibration_points:
                    if (
                        sensor_id in point["adc_codes"]
                        and sensor_id in point["reference_pressures"]
                    ):
        ...
        phase_text = (
            " (Phase 1: TLS + Bayesian)" if ROBUST_CALIBRATION_AVAILABLE else ""
        )
        self.status_label.setText(f"Fitted {fitted_count} sensor(s){phase_text}")
        QtWidgets.QMessageBox.information(
            self, "Complete", f"Fitted {fitted_count} sensor(s)!"
        )
        ...
        if filename:
            try:
                ...
                self.calibration_polynomials = {
                    ...
                    for sensor_id, coeffs in calibration_data.get(
                        "calibration_polynomials", {}
                    ).items()
                }
                ...
                if (
                    ROBUST_CALIBRATION_AVAILABLE
                    and "robust_frameworks" in calibration_data
                ):
    ...
    def load_existing_calibration(self):
        ...
        if calib_dir.exists():
            json_files = sorted(
                calib_dir.glob("*.json"), key=lambda p: p.stat().st_mtime, reverse=True
            )
            if json_files:
                try:
    ...
    def calculate_calibrated_pressure(self, sensor_id: int, adc_code: int):
        ...
        if ROBUST_CALIBRATION_AVAILABLE and sensor_id in self.robust_frameworks:
            ...
            if len(framework.calibration_points) >= 1:  # At least one human input
                env_state = EnvironmentalState()
                phi = framework.environmental_robust_basis_functions(
                    float(adc_code), env_state
                )
    ...
    def perform_zero_point_calibration(self):
        ...
        if not hasattr(self.sensor_widget, "sensor_adc_codes"):
            QtWidgets.QMessageBox.warning(
                self, "No Data", "No sensor data available yet."
            )
            return
        ...
        self.update_points_table()
        self.status_label.setText(
            "Zero-point calibration complete (0 PSI for all sensors)"
        )
        QtWidgets.QMessageBox.information(
            self,
            "Zero-Point Calibration Complete",
    ...
    def consensus_based_recalibration(self):
        ...
        for sensor_id in calibrated_sensors:
            if sensor_id in current_adcs:
                framework = self.robust_frameworks[sensor_id]
                phi = framework.environmental_robust_basis_functions(
                    float(current_adcs[sensor_id]), env_state
                )
    ...
    def toggle_phase2(self, checked):
        ...
        if not ROBUST_CALIBRATION_AVAILABLE:
            QtWidgets.QMessageBox.warning(
                self, "Not Available", "Robust calibration framework not available."
            )
    ...
    def phase2_update(self):
        ...
        for sensor_id in range(1, NUM_CONNECTORS + 1):
            ...
            if (
                hasattr(self.sensor_widget, "sensor_adc_codes")
                and sensor_id in self.sensor_widget.sensor_adc_codes
            ):
                adc_deque = self.sensor_widget.sensor_adc_codes[sensor_id]
                if len(adc_deque) > 0:
                    ...
                    # GLR drift detection
                    glr_stat, drift_detected = (
                        framework.generalized_likelihood_ratio_test([cal_point])
                    )
                    ...
                    if drift_detected:
                        ...
                        if calib_status:
                            calib_status.setText(
                                "⚠️ DRIFT DETECTED - Recalibration needed"
                            )
                            calib_status.setStyleSheet(
...
class CombinedMainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(
        ...
        self.state_machine, _ = load_state_machine_csv(
            CONFIG.get_state_machine_csv_path()
        )
        ...
        self.actuator_widget = ActuatorControlWidget(
            receiver, device_ip, device_port, self
        )
    ...
    def _apply_bar_limits(self):
        ...
        for fluid, bar in [
            ("GN2", self.top_bar_widget.bar_gn2),
            ("ETH", self.top_bar_widget.bar_eth),
            ("LOX", self.top_bar_widget.bar_lox),
        ]:
            d = pl.get(fluid, {})
    ...
    def on_mode_changed(self, mode_name: str):
        ...
        if self.state_machine:
            apply_state_from_csv(
                self.actuator_widget, csv_state_name, self.state_machine
            )
        else:
            print(
                f"Warning: State machine not loaded, cannot apply state for '{mode_name}'"
            )
        ...
        if self.pwm_window is not None:
            ...
            if armed:
                ...
                try:
                    ...
                    sock.sendto(packet, (ip, port))
                    self.log_event(
                        "PWM Armed Execute", f"{len(armed)} commands on '{mode_name}'"
                    )
                    print(
                        f"Sent {len(armed)} armed PWM commands on state '{mode_name}'"
                    )
                except Exception as e:
    ...
    def get_event_log(self):
        ...
        for event_time, event_type, details in self.event_log:
            ...
            if (
                event_type == "Actuator State Change"
                and self.actuator_widget.only_show_actuators_with_roles
            ):
    ...
    def _check_auto_state_transitions(self):
        ...
        if current == "Fire" and CONFIG.config.get("simulate_chamber_pressure", False):
            ...
            ox_upstream_pt = CONFIG.get_sensor_role(
                "Ox Upstream"
            )  # "Ox Upstream" in config vs "LOX Upstream" in code?
            ...
            if p_fuel < 350.0 or p_lox < 350.0:
                ...
                self.log_event(
                    "Auto Transition",
                    f"Simulate Chamber Pressure triggered: Fuel={p_fuel:.1f}, LOX={p_lox:.1f} (<350)",
                )
                return
