...
static const std::string ACTUATOR_CONTROLLER_ADDRESS =
    "ACTUATOR_CONTROLLER_ADDRESS";
...
enum class BoardState { inactive, active, disconnected, abort, abortDone };
BoardState myBoardState = BoardState::inactive;

void sendHeartbeat(int boardID, BoardState boardState,
...
void provideAbortData(BoardState boardState, int boardID, int IPAddress,
                      std::string destination) {
...
void abortFinish(BoardState myBoardState, int boardID) {
    ...
    sendHeartbeat(boardID, myBoardState,
                  SERVER_ADDRESS);  // send heartbeat to server with abort state
    mainloop(myBoardState, boardID);  // example of looping mainloop after abort
}
void broadcastConnectionLoss() {
    // mock implementation of broadcasting connection loss to server and
    // actuator controller
}
void ping() {
...
bool responseReceived() {
    // mock implementation of checking for response from server and actuator
    // controller
    return false;  // assume no response received for example
}
bool abortDonePacketReceived() {
    // mock implementation of checking for abort done packet from controller
    // board
...
void setup() {
    ...
    // Waiting for Server
    // send heartbeats including board ID until it recieves server's heartbeat
    // response or timeout
...
void mainloop(BoardState myBoardState, int boardID) {  // ACTIVE STATE
    sendHeartbeat(boardID, myBoardState,
                  SERVER_ADDRESS);  // send heartbeat to server
    storeAddress();                 // store server address in server heartbeat
...
void connectionLoss(BoardState myBoardState, int boardID) {
    ...
    while (currentTime < CONNECTION_LOSS_GRACE_MS) {
        broadcastConnectionLoss();  // broadcast connection loss to server and
                                    // actuator controller
        ping();
    ...
    if (connectionResume() == true) {
        mainloop(myBoardState,
                 boardID);  // resume mainloop if connection is resumed
    }
    if (responseReceived() == true) {
        noConnectionAbort(myBoardState,
                          boardID);  // abort if connection not resumed
    }

    else {                                 // STANDALONE ABORT
        myBoardState = BoardState::abort;  // abort if necessary
        provideAbortData(myBoardState, boardID, IPAddress,
                         ACTUATOR_CONTROLLER_ADDRESS);  // provide abort data to
                                                        // actuator controller
...
void standardAbort(BoardState myBoardState, int boardID) {
    while (true) {
        sendHeartbeat(
            boardID, BoardState::abort,
            SERVER_ADDRESS);  // send heartbeat to server with abort state
        delay(HEARTBEAT_SEND_INTERVAL_MS);
        if (abortDonePacketReceived() == true) {
            abortFinish(myBoardState,
                        boardID);  // finish abort if abort done packet received
...
void noConnectionAbort(BoardState myBoardState, int boardID) {
    if (responseReceived()) {  // NO CONNECTION ABORT
        ...
        while (abortDonePacketReceived() == false) {
            executeCommands();  // execute commands from controller board as
                                // they come
            delay(HEARTBEAT_SEND_INTERVAL_MS);
        }
