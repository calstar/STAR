...
// Stored config from server (placeholder until full ACTUATOR_CONFIG parse
// exists)
struct StoredConfig {
    bool valid;
    uint8_t raw[64];
...
//-----------------------------------------------------------------------------
// Status LED: non-blocking blink count = state number (timing in
// actuator_config.h)
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Process one incoming packet: update last heartbeat time and return what was
// received
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Actuator commands from server (Active and Standard Abort only), like
// Actuator_Testing
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Sensor data: read current sense pins and send one packet (like
// Actuator_Testing)
//-----------------------------------------------------------------------------
...
// 1: Waiting for Server — heartbeat sent at fixed interval in loop; on config
// -> Active
static void run_WaitingForServer() {
    // Incoming packets processed in loop(); transition to Active happens when
    // Config received
}

// 2: Active — stream sensor data, watchdog -> Connection Loss; heartbeat at
// fixed interval
...
// 3: Standard Abort — Abort Done packet -> Abort Finished; watchdog -> No
// Connection Abort; heartbeat at fixed interval
...
// 4: No Connection Abort — complete abort sequence (TODO); after timeout ->
// Abort Finished; heartbeat at fixed interval
...
// 6: Connection Loss Detected — if server heartbeat received (in loop) ->
// Active; else after grace time -> No Connection Abort
...
//-----------------------------------------------------------------------------
// Apply transitions that depend on received packet (called from loop after
...
void setup() {
    ...
    // Mount read-only: do not format on fail, so we never overwrite burned
    // value
    ...
    if (!spiffs_ok)
        Serial.println(
            "SPIFFS read skipped or failed, using default board ID 1 / "
            "192.168.2.1");
...
void loop() {
    ...
    // 1) Read all pending UDP packets and update heartbeat time / apply
    // transitions
    ...
    if (packetSize > 0) {
        ...
        if (bytesRead > 0) {
            IncomingPacketKind kind =
                processIncomingPacket(packetBuffer, bytesRead);
