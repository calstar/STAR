...
// ---------------------------------------------------------------------------
// Network: primary = 192.168.2.100, listen 5005; send to secondary
// 192.168.2.101:5006
// ---------------------------------------------------------------------------
byte mac[6];
...
void setup() {
    ...
    SPI.begin(pins::Actuator_Board.ETH_SCLK, pins::Actuator_Board.ETH_MISO,
              pins::Actuator_Board.ETH_MOSI, pins::Actuator_Board.ETH_CS);
    delay(500);
...
void loop() {
    ...
    if (n > 0) {
        ...
        if (len > 0)
            printReceivedPacket(packetBuffer, (size_t)len, udp.remoteIP(),
                                udp.remotePort());
    }
