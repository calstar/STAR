...
/*!
 *   @brief  Configure INT1 for high-G threshold interrupt with high-pass filter.
 *
 *   Sets up CTRL_REG2 (HPF for INT1), performs the mandatory REFERENCE dummy
 *   read to zero the HPF baseline (prevents spurious interrupt at startup),
 *   programs INT1_CFG for high-event detection on all three axes (OR mode),
 *   writes threshold and duration registers, routes IA1 to the INT1 pin via
 *   CTRL_REG3, and latches the interrupt in CTRL_REG5 until INT1_SRC is read.
 *
 *   @param  ths  INT1_THS value (7-bit). LSB size depends on full-scale range:
 *                ±2g=16mg, ±4g=32mg, ±8g=62mg, ±16g=186mg per datasheet.
 *   @param  dur  INT1_DUR value (7-bit). Each count = 1/ODR.
 */
void Adafruit_LIS3DH::configureHighGInterrupt(uint8_t ths, uint8_t dur) {
  // CTRL_REG2: HPM=00 (normal, reset by REFERENCE read), HP_IA1=1
  // Explicit value avoids relying on reset state.
  Adafruit_BusIO_Register ctrl2 = Adafruit_BusIO_Register(
      i2c_dev, spi_dev, ADDRBIT8_HIGH_TOREAD, LIS3DH_REG_CTRL2, 1);
  ctrl2.write(0x01);

  // Dummy read of REFERENCE resets HPF to current acceleration baseline,
  // preventing a spurious interrupt while the filter settles at startup.
  Adafruit_BusIO_Register ref = Adafruit_BusIO_Register(
      i2c_dev, spi_dev, ADDRBIT8_HIGH_TOREAD, LIS3DH_REG_REFERENCE, 1);
  ref.read();

  // INT1_CFG: enable high event on X, Y, Z axes (OR combination)
  // XHIE=0x02, YHIE=0x08, ZHIE=0x20
  Adafruit_BusIO_Register int1_cfg = Adafruit_BusIO_Register(
      i2c_dev, spi_dev, ADDRBIT8_HIGH_TOREAD, LIS3DH_REG_INT1CFG, 1);
  int1_cfg.write(0x2A);

  // Threshold and duration
  Adafruit_BusIO_Register int1_ths = Adafruit_BusIO_Register(
      i2c_dev, spi_dev, ADDRBIT8_HIGH_TOREAD, LIS3DH_REG_INT1THS, 1);
  int1_ths.write(ths & 0x7F);

  Adafruit_BusIO_Register int1_dur = Adafruit_BusIO_Register(
      i2c_dev, spi_dev, ADDRBIT8_HIGH_TOREAD, LIS3DH_REG_INT1DUR, 1);
  int1_dur.write(dur & 0x7F);

  // CTRL_REG3: route IA1 to INT1 physical pin (bit 6 = I1_IA1)
  Adafruit_BusIO_Register ctrl3 = Adafruit_BusIO_Register(
      i2c_dev, spi_dev, ADDRBIT8_HIGH_TOREAD, LIS3DH_REG_CTRL3, 1);
  ctrl3.write(0x40);

  // CTRL_REG5: latch INT1 until INT1_SRC is read (bit 3 = LIR_INT1)
  Adafruit_BusIO_Register ctrl5 = Adafruit_BusIO_Register(
      i2c_dev, spi_dev, ADDRBIT8_HIGH_TOREAD, LIS3DH_REG_CTRL5, 1);
  ctrl5.write(0x08);
}
