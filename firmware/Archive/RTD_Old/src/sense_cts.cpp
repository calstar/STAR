...
struct Rec18 {    // TODO: What does Rec18 mean? Is adding second channel ok?
  uint8_t ch_pos; // channel id
  uint8_t ch_neg; // channel id
  uint8_t ok;     // 0 or 1
  int32_t raw;
...
static void sendSweepPacket(const adcs::AllResults &sweep) {
  ...
  uint8_t *buf =
      standby; // use standby as a scratch area so we don't disturb "active"
  size_t off = 0;
  ...
  h.flags = 0x01;
  h.count = n_fixed;
  h.failures = sweep.failures +
               static_cast<uint16_t>(
                   (n_fixed > sweep.count) ? (n_fixed - sweep.count) : 0);
  ...
  for (size_t i = 0; i < n_real; ++i) {
    ...
    Rec18 r;
    r.ch_pos = s.channel_pos;
    r.ch_neg = s.channel_neg;
