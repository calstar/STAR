...
const int CPR_4X_MOTOR =
    64;  // encoder counts per MOTOR rev at 4x (set true value)
...
// One-shot polarity calibration: apply a small forward nudge and measure
// Δcounts
...
void setup() {
    ...
    Serial.println(
        "Ready. Calibrating direction, then toggling 0 <-> +1000 counts.");
...
void loop() {
    ...
    if (millis() - print_t0 > 200) {
        ...
        Serial.println(msg);
        if (ethernetReady()) {
            size_t payloadLen = (n < 0) ? strlen(msg)
                                        : ((n >= static_cast<int>(sizeof(msg)))
                                               ? sizeof(msg) - 1
