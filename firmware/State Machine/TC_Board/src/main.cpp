 ...
 *
 * Same state machine as PT Hotfire: Waiting for Server → Active → Standalone
 * Abort (and back to Active on clear). Sends heartbeats, streams
 * thermocouple/sensor data to server or to actuator controller in abort. Uses
 * TC_Board pin layout and BoardType::THERMOCOUPLE.
 */
...
// Server address: updated when we receive a server heartbeat (store server
// address in server heartbeat)
...
// State machine
//-----------------------------------------------------------------------------
enum class PTHotfireState { WaitingForServer, Active, StandaloneAbort };

static PTHotfireState state = PTHotfireState::WaitingForServer;

// Stored config from SENSOR_CONFIG packet (necessary_for_abort, actuator
// controller IP)
...
// remote_ip/remote_port: set when we receive SERVER_HEARTBEAT (store server
// address)
//-----------------------------------------------------------------------------
        ...
        case Diablo::PacketType::SENSOR_CONFIG:
            ...
            if (len >= 6 + 1) {
                ...
                if (stored_config.necessary_for_abort && len >= 6 + 5) {
                    stored_config.actuator_controller_ip =
...
// 3: Standalone Abort — stream sensor data to actuator controller; heartbeat at
// fixed interval
static void run_StandaloneAbort() {
    if (!stored_config.valid || stored_config.actuator_controller_ip == 0)
        return;
    IPAddress actuatorIP((stored_config.actuator_controller_ip >> 24) & 0xFF,
    ...
    switch (state) {
        ...
        case PTHotfireState::Active:
            if (kind == IncomingPacketKind::NoConnAbort &&
                stored_config.necessary_for_abort)
