...
// use begin(). Official Ethernet 2.x EthernetServer only implements void
// begin(), so EthernetServer can be abstract on CI. Implement begin(uint16_t)
// without 'override' so it matches newer Server.h on CI; omitting 'override'
// avoids an error on older cores whose Server has no begin(uint16_t).
...
// On failure: prints error, closes the client, and returns (board keeps
// running).
