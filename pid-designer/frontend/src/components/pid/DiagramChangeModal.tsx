/**
 * The "Change" dialog: everything you can do to the set of diagrams, in one
 ...
 *
 * Two tabs, because there are two genuinely different relationships to a diagram:
 *
...
import type { BrowseGroup, DiagramMeta, DocRef, TeamUser } from '../../api/diagrams';
import { btn, ghostBtn, primaryBtn, relativeTime } from '../../lib/ui';
...
export function DiagramChangeModal({
  ...
  useEffect(() => {
    let cancelled = false;
    void api.browseDiagrams().then((t) => !cancelled && setTree(t)).catch(() => !cancelled && setTree([]));
  ...
  const startShare = (d: DiagramMeta) => {
    reset();
  ...
  return (
    <Modal open={open} onClose={onClose} title="Diagrams" width="w-[560px]">
      ...
      {tab === 'editable' ? (
        <div className="max-h-[55vh] overflow-y-auto">
          {creating ? (
            ...
            <button
              ...
              onClick={() => { reset(); setCreating(true); setNewName(`Diagram ${documents.length + 1}`); }}
            >
              + New diagram
            </button>
          )}
          ...
          {documents.length === 0 && (
            <p className="py-3 text-xs text-[var(--color-text-muted)]">No diagrams yet.</p>
          )}
                    ...
                    <>
                      ...
                      {!d.mine && (
                        <button
                          ...
                          title="Remove yourself from this diagram. It is not deleted — you can copy it from View only whenever you like."
                  ...
                  <div className="mx-2 mb-2 rounded border border-[var(--color-border)] p-2">
                    ...
                    {removed(d).length > 0 && (
                      <p className="mt-2 text-[10px] text-amber-500">
                        Removing: {removed(d).join(', ')} — they lose edit access, but can still copy this diagram.
                      </p>
                    )}
        ...
        <div className="max-h-[55vh] overflow-y-auto">
          <p className="mb-2 text-[10px] text-[var(--color-text-muted)]">
            Everyone else's diagrams. Opening one takes your own copy of it — the original is untouched.
          </p>
          {tree === null && <p className="py-3 text-xs text-[var(--color-text-muted)]">Loading…</p>}
          {tree?.length === 0 && (
            <p className="py-3 text-xs text-[var(--color-text-muted)]">Nobody else has diagrams yet.</p>
          )}
          {tree?.map((g) => (
