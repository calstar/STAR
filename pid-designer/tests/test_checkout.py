...
def _save(client, doc_id, headers, params=None, nodes=None):
    return client.post(f"{BASE}/{doc_id}/autosave", headers=headers,
                       params=params if params is not None else {},
                       json={"nodes": nodes or [{"id": "n1"}], "edges": []})
...
def test_only_one_of_many_simultaneous_takes_wins(client, tmp_path, attempt):
    ...
    index = json.loads((tmp_path / "alice@berkeley.edu" / "pid" / "index.json").read_text())
...
@pytest.mark.parametrize("path,method,body", [
    ("/autosave", "POST", {"nodes": [], "edges": []}),
    ("/flush", "POST", {"nodes": [], "edges": []}),
...
def test_the_previous_holder_is_refused_rather_than_silently_overwriting(client, monkeypatch):
    ...
    assert client.get(f"{BASE}/{doc_id}/load", headers=A).json()["nodes"] == [{"id": "fresh"}]
...
def test_saving_keeps_the_checkout_alive(client):
    ...
    _save(client, doc_id, A, nodes=[{"id": "later"}])
...
def test_every_endpoint_that_returns_a_record_returns_a_decorated_one(client):
    ...
    created = client.post(BASE, headers=A, json={"name": "Shaped"}).json()
...
def test_a_returned_record_says_the_design_is_yours(client):
    ...
    doc_id = _create(client)
    for label, resp in [
        ("create", client.post(BASE, headers=A, json={"name": "Shaped"})),
