"""Who a P&ID diagram can be shared with: the team roster.
...
def roster(request: Request) -> list[dict]:
    """Everyone a diagram can be shared with: ``[{email, name}]``, sorted."""
    return _shared.roster(request, userdata.store)


def display_names(request: Request) -> dict[str, str]:
    """``slug -> display name`` for labelling diagram owners; ``""`` when unknown."""
