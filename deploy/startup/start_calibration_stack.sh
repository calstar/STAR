...
# Build
DAQ_BIN="$REPO_ROOT/build/bin/daq_bridge"
[ ! -x "$DAQ_BIN" ] && DAQ_BIN="$REPO_ROOT/build/bin/daq_bridge"
if [ ! -x "$DAQ_BIN" ]; then
...
tmux send-keys -t calibration_stack:0.1 "cd $REPO_ROOT/diablo_server/backend" C-m
...
if [ "${USE_ROBUST_CALIBRATION:-1}" = "0" ]; then
  CAL_BIN="$REPO_ROOT/build/bin/calibration_service"
  [ ! -x "$CAL_BIN" ] && CAL_BIN="$REPO_ROOT/build/bin/calibration_service"
  if [ -x "$CAL_BIN" ]; then
    ...
    tmux send-keys -t calibration_stack:0.3 "cd $REPO_ROOT" C-m
    tmux send-keys -t calibration_stack:0.3 "PYTHONPATH=$REPO_ROOT python3 tools/calibration/calibration_server.py" C-m
