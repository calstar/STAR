...
class RTDCalibrator(SensorCalibrator):
    ...
    def perform_calibration(self) -> CalibrationResult:
        ...
        A_matrix = np.column_stack(
            [np.ones(len(resistances)), resistances, resistances**2]
        )
...
class CalibrationSequence:
    ...
    def run_pressure_transducer_calibration(
        self,
        sensor_id: str,
        ...
        logger.info(
            f"Quality metrics: RMSE={result.quality_metrics['rmse']:.2f} Pa, "
            f"NRMSE={result.quality_metrics['nrmse']:.4f}"
        )

        return result

    def run_rtd_calibration(
        self,
        sensor_id: str,
        ...
        for temperature, resistance in zip(temperature_points, resistance_readings):
            ...
            logger.info(
                f"Added calibration point: {resistance:.2f}Ω -> {temperature:.1f}°C"
            )
    ...
    def generate_report(self) -> str:
        ...
        for sensor_id, result in self.results.items():
            ...
            if "coverage_95" in result.quality_metrics:
                report.append(
                    f"95% Coverage: {result.quality_metrics['coverage_95']:.2%}"
                )
            report.append("")
...
def main():
    ...
    parser.add_argument(
        "--config", default="config_engine.toml", help="Configuration file"
    )
