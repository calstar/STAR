...
#
# Browser / Playwright E2E is separate: bash test/e2e_guitest_playwright.sh (not run by this script).
#
# Usage:
...
kill_stale_integration_processes() {
  ...
  pkill -f "$REPO_ROOT/diablo_server/backend.*server\.ts" 2>/dev/null && killed=$((killed + 1)) || true
  ...
  if [ "$killed" -gt 0 ]; then
    ...
    pkill -9 -f "$REPO_ROOT/diablo_server/backend.*server\.ts" 2>/dev/null || true
...
# Find DAQ bridge
DAQ_BRIDGE="$REPO_ROOT/build/bin/daq_bridge"
[ -x "$DAQ_BRIDGE" ] || fail "daq_bridge not found. Build with: cd build && cmake .. && make daq_bridge"
echo "  ✅ daq_bridge: $DAQ_BRIDGE"

CONFIG_BROADCAST_SVC="$REPO_ROOT/build/bin/config_broadcast_service"
if [ -x "$CONFIG_BROADCAST_SVC" ]; then
...
HEARTBEAT_SVC="$REPO_ROOT/build/bin/heartbeat_service"
if [ -x "$HEARTBEAT_SVC" ]; then
...
SEQ_SVC="$REPO_ROOT/build/bin/sequencer_service"
if [ -x "$SEQ_SVC" ]; then
...
CONTROLLER_SVC="$REPO_ROOT/build/bin/controller_service"
if [ -x "$CONTROLLER_SVC" ]; then
...
if [ ! -d "$REPO_ROOT/diablo_server/backend/node_modules/ws" ]; then
...
if [ "${INTEGRATION_SKIP_STARTUP_E2E:-0}" != "1" ]; then
  ...
  if [ -n "$CONFIG_BROADCAST_SVC" ]; then
    ...
    (cd "$REPO_ROOT" && PYTHONPATH="$REPO_ROOT/scripts/services:$REPO_ROOT/scripts/calibration" \
      "$PYTHON_BIN" archive/legacy/python-services/config_broadcast_service.py --config "$TEST_CONFIG" --interval-ms 1500 \
...
if [ "$BACKEND" = "thin" ]; then
  ...
  (cd "$REPO_ROOT/diablo_server/backend" && \
...
CALIB_SVC="$REPO_ROOT/build/bin/calibration_service"
...
(cd "$REPO_ROOT/diablo_server/backend" && \
  NODE_PATH="$REPO_ROOT/diablo_server/backend/node_modules" \
