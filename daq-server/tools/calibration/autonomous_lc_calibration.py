#!/usr/bin/env python3
"""
Autonomous Load Cell (LC) Calibration System
Reads raw ADC counts from LC boards via UDP, collects calibration points
at known reference forces (lbs), fits polynomials, and saves coefficients.

All board IPs, ports, and sensor counts come from config/config.toml.

Polynomial model: force_lbs = A·adc³ + B·adc² + C·adc + D

Usage:
  python3 autonomous_lc_calibration.py --interactive
  python3 autonomous_lc_calibration.py --ref-all 0.0 --collect-time 10
"""
...
def _default_calibration_dir():
    if _cfg_loaded:
        cc = get_calibration_config("lc")
        jd = cc.get("json_dir")
        if jd:
            return resolve_path(jd)
    return Path(__file__).parent / "calibrations" / "lc"
...
class AutonomousLCCalibrator:
    """Autonomous Load-Cell calibration via direct UDP board packets."""
    ...
    def __init__(self, udp_port=None, board_ip=None):
        ...
        self.reference_forces: Dict[int, float] = {}
        ...
        logger.info("⚖️  Autonomous LC Calibrator initialized")
    ...
    def set_reference_force(self, ch, lbs):
    ...
    def collect_samples(self, dur=5.0):
        if not self.reference_forces:
            logger.warning("No reference forces set")
            return
        logger.info(f"📊 Collecting LC data for {dur:.1f}s")
    ...
    def _listen(self):
        ...
        try:
            ...
            logger.info(f"📡 LC listening UDP :{self.udp_port}")
        except OSError as e:
        ...
        while self.running:
            ...
            for sid, signed, raw in samples:
                ...
                if self.collecting and sid in self.reference_forces:
                    pt = CalibrationPoint(
                        sid, signed, self.reference_forces[sid], time.time()
                    )
                    self._add_point(pt)
        sock.close()
        self.udp_connected = False
    ...
    def auto_fit(self):
        for sid, s in self.sensor_states.items():
            if len(s.calibration_points) < self.min_calibration_points:
                continue
            counts = np.array(
                [p.adc_counts for p in s.calibration_points], dtype=np.float64
            )
            ref = np.array(
                [p.reference_force_lbs for p in s.calibration_points], dtype=np.float64
            )
            if np.std(ref) < 1e-9:
    ...
    def save_calibrations(self):
        ...
        jp = CALIBRATION_DIR / f"lc_calibration_{ts}.json"
        data = {
            "sensor_type": "LC",
            "unit": "lbs",
            "polynomial_order": 3,
    ...
    def print_status(self):
        print(f"\n{'═'*70}")
        print("⚖️  Autonomous LC Calibration Status")
        print(f"{'═'*70}")
        ...
        for ch in sorted(
            ...
            adc_str = (
                f"ADC={int(np.mean(list(buf))):>10d}" if buf and len(buf) else "ADC=---"
            )
            ref = self.reference_forces.get(ch)
            ref_str = f"  ref={ref:.2f}lbs" if ref is not None else ""
            if s:
                cal = (
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Autonomous LC Calibration (UDP)")
    parser.add_argument(
    ...
    parser.add_argument("--force", type=float, action="append")
    parser.add_argument("--ref-all", type=float, default=None)
    parser.add_argument("--collect-time", type=float, default=5.0)
    parser.add_argument("--interactive", "-i", action="store_true")
    args = parser.parse_args()

    cal = AutonomousLCCalibrator(args.port, args.board_ip)
    if args.ref_all is not None:
        cal.set_reference_all(args.ref_all)
    if args.sensor and args.force:
        for s, f in zip(args.sensor, args.force):
            cal.set_reference_force(s, f)
    cal.start()
    time.sleep(1)
    ...
    if args.interactive:
        print(
            "Commands: ref <ch> <lbs> | ref-all <lbs> | collect [s] | fit | save | status | quit"
        )
        while True:
            try:
                cmd = input("  [LC] > ").strip()
            ...
            if v == "ref-all" and len(p) >= 2:
                ...
                cal.set_reference_force(int(p[1]), float(p[2]))
            elif v == "collect":
