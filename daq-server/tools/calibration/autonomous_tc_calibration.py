#!/usr/bin/env python3
"""
Autonomous Thermocouple (TC) Calibration System
Reads raw ADC data from TC boards via UDP, collects calibration points at known
reference temperatures, fits polynomials, and saves coefficients for the C++ DAQ bridge.

All board IPs, ports, and sensor counts come from config/config.toml.

Calibration model:  temp_°C = A·adc³ + B·adc² + C·adc + D

Usage:
  python3 autonomous_tc_calibration.py --interactive
  python3 autonomous_tc_calibration.py --ref-all 25.0 --collect-time 10
  python3 autonomous_tc_calibration.py --sensor 1 --temp 100.0 --sensor 2 --temp 200.0
"""
...
def _default_calibration_dir():
    if _cfg_loaded:
        cc = get_calibration_config("tc")
        jd = cc.get("json_dir")
        if jd:
            return resolve_path(jd)
    return Path(__file__).parent / "calibrations" / "tc"
...
class AutonomousTCCalibrator:
    """Autonomous thermocouple calibration via direct UDP board packets."""
    ...
    def __init__(self, udp_port=None, board_ip=None):
        ...
        logger.info("🌡️  Autonomous TC Calibrator initialized")
    ...
    def collect_samples(self, dur=5.0):
        if not self.reference_temps:
            ...
            return
        logger.info(f"📊 Collecting TC data for {dur:.1f}s")
    ...
    def _listen(self):
        ...
        try:
            ...
            logger.info(f"📡 TC listening UDP :{self.udp_port}")
        except OSError as e:
    ...
    def auto_fit(self):
        for sid, s in self.sensor_states.items():
            if len(s.calibration_points) < self.min_calibration_points:
                continue
            adc = np.array([p.adc_code for p in s.calibration_points], dtype=np.float64)
            ref = np.array(
            ...
            if np.std(ref) < 1e-9:
                ...
                continue
            try:
                c = np.polyfit(adc, ref, min(3, len(s.calibration_points) - 1))
                while len(c) < 4:
                    c = np.insert(c, 0, 0.0)
                s.polynomial_coeffs = tuple(c)
                pred = np.polyval(c, adc)
    ...
    def save_calibrations(self):
        ...
        jp = CALIBRATION_DIR / f"tc_calibration_{ts}.json"
        data = {
            "sensor_type": "TC",
            "unit": "°C",
            "polynomial_order": 3,
    ...
    def print_status(self):
        print(f"\n{'═'*70}")
        print("🌡️  Autonomous TC Calibration Status")
        print(f"{'═'*70}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Autonomous TC Calibration (UDP)")
    parser.add_argument(
    ...
    cal = AutonomousTCCalibrator(args.port, args.board_ip)
    ...
    if args.interactive:
        print(
            "Commands: ref <ch> <°C> | ref-all <°C> | collect [s] | fit | save | status | live | quit"
        )
        while True:
            try:
                cmd = input("  [TC] > ").strip()
