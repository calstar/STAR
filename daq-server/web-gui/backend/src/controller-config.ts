...
export const DEFAULT_DUTY_SWEEP_STEPS: [number, number][] = [
  [0.1, 0.1],
  [0.3, 0.2],
  [0.5, 0.4],
  [0.3, 0.3],
 ...
 */
export function applyControllerDefaults(config: any): void {
  if (!config.controller) config.controller = {};
  const c = config.controller;
  if (
    c.duty_sweep_steps == null ||
  ...
  if (
    c.duty_sweep_step_duration_sec == null ||
