...
( [ -x ../../sim/setup_sim_network.sh ] && ../../sim/setup_sim_network.sh ) &
sleep 2
# If aliases fail (sudo prompt), simulator falls back to 127.0.0.x — data still flows

echo "🎭 Starting board_simulator (PT data → :5006)..."
cd ../..
python3 sim/board_simulator.py --config config/config.toml --target 127.0.0.1 --port 5006 --only-type PT &
SIMULATOR_PID=$!
echo "🚀 Starting controller_service..."
"$SCRIPT_DIR/../../build/bin/controller_service" --config "$SCRIPT_DIR/../../config/config.toml" --elodin-host 127.0.0.1 &
CONTROLLER_PID=$!

cd "$SCRIPT_DIR/../../diablo_server/frontend"
fi

# C++ Calibration Service (skip in replay/demo)
if ! $DEMO_MODE && ! $REPLAY_MODE; then
  sleep 2  # Ensure relay TCP forward is ready
  CAL_BIN="$SCRIPT_DIR/../../build/bin/calibration_service"
  if [ -x "$CAL_BIN" ]; then
    ...
    echo "   ⚠️ calibration_service not found — run: cd build && cmake .. && make calibration_service"
  fi
fi
