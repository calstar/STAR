...
if ! systemctl --user is-active --quiet sensor-backend.service; then
  ...
  echo "   Start them with: systemctl --user start sensor-backend sensor-frontend sensor-elodin"
...
tmux split-window -v -t "$SESSION:logs.1" \
  "journalctl --user-unit=sensor-calibration.service -f"
...
echo "│  2: Frontend logs   3: Calibration logs │"
