...
[ ! -x "$DAQ_BIN" ] && DAQ_BIN="$PROJECT/build/bin/daq_bridge"
if [ ! -x "$DAQ_BIN" ]; then
...
[ ! -x "$CAL_BIN" ] && CAL_BIN="$PROJECT/build/bin/calibration_service"
if [ -x "$CAL_BIN" ]; then
  cd "$PROJECT"
...
if [ -x "$PROJECT/sim/setup_sim_network.sh" ]; then
  "$PROJECT/sim/setup_sim_network.sh"
fi
# Add --low-noise for smoother PT signal when validating spike behavior
python3 sim/board_simulator.py --config config/config.toml --target 127.0.0.1 --port 5006 ${LOW_NOISE:+--low-noise} > /tmp/sim_fake.log 2>&1 &
sleep 1
...
[ -x "$PROJECT/build/bin/actuator_service" ] && ACTUATOR_SVC_ENV="ACTUATOR_SERVICE_ENABLED=true ACTUATOR_SERVICE_PORT=9998"
...
cd "$PROJECT"
PYTHONPATH="$PROJECT" python3 tools/calibration/calibration_server.py > /tmp/sidecar_fake.log 2>&1 &
sleep 1
echo "✅ Calibration sidecar"

# 7. Controller service (optional)
if [ -x "$PROJECT/build/bin/controller_service" ]; then
  $PROJECT/build/bin/controller_service --config config/config.toml --elodin-host 127.0.0.1 > /tmp/ctrl_fake.log 2>&1 &
  echo "✅ Controller service"
fi

# 8. Actuator service (optional)
if [ -x "$PROJECT/build/bin/actuator_service" ]; then
  $PROJECT/build/bin/actuator_service --config config/config.toml --port 9998 > /tmp/actuator_fake.log 2>&1 &
