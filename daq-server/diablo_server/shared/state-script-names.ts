...
export function isValidScriptFilename(name: string): boolean {
...
const PATTERNS: { re: RegExp; ns: ScriptNamespace }[] = [
...
export function scanScriptNames(src: string): ScriptNameRef[] {
  const out: ScriptNameRef[] = [];
  stripComments(src).forEach((text, i) => {
    for (const { re, ns } of PATTERNS) {
      re.lastIndex = 0;
      let m: RegExpExecArray | null;
      while ((m = re.exec(text)) !== null) out.push({ slug: m[1], ns, line: i + 1 });
    }
  });
  return out;
}
 ...
 */
export function renameScriptSlug(
  src: string,
  ns: ScriptNamespace,
  fromSlug: string,
  toSlug: string,
  ...
  const calls =
    ns === 'actuator' ? '(?:open_valve|close_valve)' : ns === 'sensor' ? 'pressure' : 'transition_to';
...
export function checkScriptNames(src: string, tables: ScriptNameTables): ScriptNameIssue[] {
  const out: ScriptNameIssue[] = [];
  for (const ref of scanScriptNames(src)) {
    const table =
      ref.ns === 'actuator' ? tables.actuators : ref.ns === 'sensor' ? tables.sensors : tables.states;
