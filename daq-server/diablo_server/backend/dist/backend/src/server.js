 ...
 */
function shouldThrottleSensorStreamPacket(high, _low) {
...
const lastRawAdc = new Map();
let ptCalibrationFilePath = null;
// Load existing calibration file if available
try {
    const loaded = loadPTCalibration();
    if (loaded.map.size > 0) {
        for (const [k, v] of loaded.map)
            ptCalibration.set(k, v);
...
function uniqueIdFromPtEntity(entity) {
    ...
    try {
        ...
        for (const [, raw] of Object.entries(boards)) {
            ...
            if (board.enabled === false)
                continue;
            const id = typeof board.board_id === 'number' ? board.board_id : undefined;
            if (id === undefined)
                continue;
            const type = board.type || 'UNKNOWN';
            const boardNumber = typeof board.board_number === 'number'
                ? board.board_number
...
const STATE_ACTUATOR_MAP = getStateActuatorMap();
/**
 ...
 */
function getExpectedPositions(state) {
...
function broadcastCommandedActuatorsForState(state) {
...
const actuatorSensedState = new Map();
const ACTUATOR_MISMATCH_DELAY_MS = 5000;
let actuatorMismatchTimer = null;
function scheduleActuatorMismatchCheck(state) {
    if (actuatorMismatchTimer)
        clearTimeout(actuatorMismatchTimer);
    actuatorMismatchTimer = setTimeout(() => {
        actuatorMismatchTimer = null;
        checkActuatorMismatch(state);
    }, ACTUATOR_MISMATCH_DELAY_MS);
}
function checkActuatorMismatch(state) {
    ...
    for (const [entity, expectedPos] of Object.entries(expected)) {
        ...
        if (sensed === undefined)
            continue; // no data yet — don't alert
...
function updateBoard(low, payload) {
    ...
    if (boardType === 1)
        ...
        typeStr = 'ACTUATOR';
    else if (boardType === 6 || boardId === 61)
        typeStr = 'ENCODER';
...
function markStaleBoards() {
    ...
    for (const [id, status] of boardsStatus) {
        ...
        if (inSetup) {
            if (!boardFirstSeenSetupMs.has(id))
                boardFirstSeenSetupMs.set(id, now);
...
function broadcastBoardStatus() {
    ...
    for (const b of boards) {
        ...
        if (times && times.length >= 2) {
            ...
            if (span > 0) {
                b.frequencyHz = (times.length - 1) / (span / 1000);
            }
        }
    }
...
function sendToActuatorService(line) {
...
function sendToControllerService(line) {
    return new Promise((resolve) => {
        let replyData = '';
        let resolved = false;
        const done = (ok, reply) => {
...
function scheduleResubscribe(attempt) {
    ...
    try {
        ...
        for (const parsed of parsedList) {
            ...
            if (parsed.component === 'raw_adc_counts') {
                ...
                if (uid != null)
                    lastRawAdc.set(uid, parsed.value);
            }
