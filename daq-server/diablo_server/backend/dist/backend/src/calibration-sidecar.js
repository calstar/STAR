...
export class CalibrationSidecarClient extends EventEmitter {
    ...
    async sendAdcSamples(samples) {
        if (!this.enabled || !this.isConnected || samples.length === 0)
            return;
        try {
    ...
    async calibrateChannel(channel, adcCode, referencePsi) {
        if (!this.enabled || !this.isConnected)
            return false;
        try {
