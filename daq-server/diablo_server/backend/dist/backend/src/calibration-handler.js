 ...
 */
function publishCalibrationCommand(host, type, sensorId, ref) {
    if (!host.elodin) {
...
function getActiveChannels(host) {
    ...
    try {
        ...
        for (const [, boardRaw] of Object.entries(boards)) {
            ...
            if (!Number.isFinite(boardId))
                continue;
            const activeChannels = Array.isArray(board.active_connectors) && board.active_connectors.length > 0
                ? board.active_connectors.map((v) => Number(v)).filter((v) => Number.isFinite(v) && v >= 1)
                : Array.from({ length: Math.max(0, Number(board.num_sensors) || 0) }, (_, i) => i + 1);
            for (const ch of activeChannels)
                channels.add(boardId * 100 + ch);
        }
    }
    catch {
 ...
 */
export function handleCalibrationCommand(host, ws, payload) {
    ...
    switch (commandType) {
        ...
        case 'save_coefficients': {
            ...
            try {
                ...
                const calibration_polynomials = {};
                const calibration_poly_coeffs = {};
                const calibration_adc_norm_min = {};
                const calibration_adc_norm_scale = {};
                ...
                for (const ch of channelsToSave) {
                    ...
                    if (coeffs) {
                        ...
                        if (coeffs.polyCoeffs?.length)
                            calibration_poly_coeffs[String(ch)] = coeffs.polyCoeffs;
                        if (coeffs.adcNormMin != null)
                            calibration_adc_norm_min[String(ch)] = coeffs.adcNormMin;
                        if (coeffs.adcNormScale != null)
                            calibration_adc_norm_scale[String(ch)] = coeffs.adcNormScale;
                    }
                }
                ...
                if (Object.keys(calibration_poly_coeffs).length > 0)
                    data.calibration_poly_coeffs = calibration_poly_coeffs;
                if (Object.keys(calibration_adc_norm_min).length > 0)
                    data.calibration_adc_norm_min = calibration_adc_norm_min;
                if (Object.keys(calibration_adc_norm_scale).length > 0)
                    data.calibration_adc_norm_scale = calibration_adc_norm_scale;
        ...
        case 'clear_calibration': {
            const defaultCoeffs = { A: 0, B: 0, C: 1e-8, D: 0 };
