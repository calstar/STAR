 ...
 */
export function computeMsgId(typeName) {
...
function encodeVTable(config) {
    ...
    for (const field of config.fields) {
        buffer.writeUInt32LE(field.offset, off);
        off += 4;
        buffer.writeUInt32LE(field.size, off);
        off += 4;
...
const subscribedVTableStreamPairs = new Set();
 ...
 */
function buildVTableStreamSubscriptionList() {
    const subscriptions = [];
    const seen = new Set();
    const addUnique = (high, low) => {
        const key = `${high},${low}`;
        if (!seen.has(key)) {
            seen.add(key);
    ...
    const addBoard = (typeHi, boardNumber, channels) => {
    ...
    try {
        ...
        for (const [, raw] of Object.entries(boards)) {
            ...
            const rawConnectors = Array.isArray(b.active_connectors) && b.active_connectors.length > 0
                ? b.active_connectors
                : Array.isArray(b.active_connections) && b.active_connections.length > 0
 ...
 */
export async function registerVTables(client) {
 ...
 */
export async function registerSensorVTables(client, ptMap, actMap) {
    if (!client.isConnected())
        return false;
    console.log('📡 Registering Sensor VTables (Raw + Cal)...');
    const vtableMsgId = computeMsgId('VTableMsg');
    let count = 0;
    const strip = (s) => s.includes('.') ? s.split('.').slice(1).join('.') : s;
    ...
    for (let ch = 1; ch <= 14; ch++) {
        ...
        if (client.sendRawMessage(vtableMsgId, ElodinPacketType.MSG, vtRaw))
            count++;
        // Cal: 21 bytes
        ...
        if (client.sendRawMessage(vtableMsgId, ElodinPacketType.MSG, vtCal))
            count++;
    }
    ...
    for (let ch = 1; ch <= 20; ch++) {
        ...
        if (client.sendRawMessage(vtableMsgId, ElodinPacketType.MSG, vtState))
            count++;
    }
...
export async function registerControllerVTables(client) {
    if (!client.isConnected())
        return false;
...
export async function registerActuatorCommandedVTables(client, actuatorChannelToEntityMap) {
    if (!client.isConnected())
        return false;
    const vtableMsgId = computeMsgId('VTableMsg');
    let count = 0;
