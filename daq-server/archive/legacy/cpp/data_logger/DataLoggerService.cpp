...
bool DataLoggerService::initialize(const std::string& config_path) {
    ...
    if (!elodin_client_.connect(db_host_, db_port_)) {
        std::cerr << "[DataLogger] ❌ Failed to connect to Elodin DB at " << db_host_ << ":"
                  << db_port_ << std::endl;
        return false;
    }
    ...
    for (int ch = 1; ch <= 14; ch++) {
        ...
        channel_map_[{0x20, packet_ch}] = {"PT.CH" + std::to_string(ch), 0, 12,
                                           ChannelInfo::Type::F32};
    }
    // TC Calibrated (0x21, 0x11..0x24)
    for (int ch = 1; ch <= 20; ch++) {
        uint8_t packet_ch = 0x10 + ch;
        channel_map_[{0x21, packet_ch}] = {"TC.CH" + std::to_string(ch), 0, 12,
                                           ChannelInfo::Type::F32};
    }
    // RTD Calibrated (0x22, 0x11..0x24)
    for (int ch = 1; ch <= 20; ch++) {
        uint8_t packet_ch = 0x10 + ch;
        channel_map_[{0x22, packet_ch}] = {"RTD.CH" + std::to_string(ch), 0, 12,
                                           ChannelInfo::Type::F32};
    }
    // LC Calibrated (0x23, 0x11..0x1A)
    for (int ch = 1; ch <= 10; ch++) {
        uint8_t packet_ch = 0x10 + ch;
        channel_map_[{0x23, packet_ch}] = {"LC.CH" + std::to_string(ch), 0, 12,
                                           ChannelInfo::Type::F32};
    }
    // Actuator State (0x31, 1..20)
    for (int ch = 1; ch <= 20; ch++) {
        channel_map_[{0x31, (uint8_t)ch}] = {"ACT.CH" + std::to_string(ch), 0, 9,
                                             ChannelInfo::Type::U8};
    }
    ...
    std::cout << "[DataLogger] Ready to log " << active_channels_.size() << " channels"
              << std::endl;
    return true;
}
...
void DataLoggerService::run() {
    uint8_t buffer[2048];
    while (true) {
        // ElodinClient::read_packet returns total length (header+payload), 0 if no data, -1 on
        // error
...
void DataLoggerService::start_logging() {
    ...
    std::stringstream ss;
    ss << "data/runs/run_" << std::put_time(std::localtime(&in_time_t), "%Y-%m-%d_%H-%M-%S")
       << ".sensorlog";
...
void DataLoggerService::process_packet(const std::array<uint8_t, 2>& packet_id,
